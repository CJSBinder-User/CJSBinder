...
//
//[code snippet 1]
void MAXIMUM_SEGMENT_VALUE_PUTTING_K_BREAKPOINTS_NUMBER_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    MAXIMUM_SEGMENT_VALUE_PUTTING_K_BREAKPOINTS_NUMBER* obj = new MAXIMUM_SEGMENT_VALUE_PUTTING_K_BREAKPOINTS_NUMBER();
...
//
//[code snippet 2]
void MAXIMUM_SEGMENT_VALUE_PUTTING_K_BREAKPOINTS_NUMBER_M_S_V_P_K_B_N_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 2 || !args[0]->IsString() || !args[1]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }

    // Extracting string argument
    v8::String::Utf8Value str(args.GetIsolate(), args[0]);
    std::string s(*str);
    
    CCLASS* obj = static_cast<MAXIMUM_SEGMENT_VALUE_PUTTING_K_BREAKPOINTS_NUMBER*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int k = args[1]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    
    int result = obj->M_S_V_P_K_B_N_f_gold(s, k);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, MAXIMUM_SEGMENT_VALUE_PUTTING_K_BREAKPOINTS_NUMBER_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "MAXIMUM_SEGMENT_VALUE_PUTTING_K_BREAKPOINTS_NUMBER").ToLocalChecked());
    
    // Registering the method
    tpl->PrototypeTemplate()->Set(isolate, "M_S_V_P_K_B_N_f_gold", v8::FunctionTemplate::New(isolate, MAXIMUM_SEGMENT_VALUE_PUTTING_K_BREAKPOINTS_NUMBER_M_S_V_P_K_B_N_f_gold));

    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "MAXIMUM_SEGMENT_VALUE_PUTTING_K_BREAKPOINTS_NUMBER").ToLocalChecked(), constructor);
