...
//
//[code snippet 1]
void C_PROGRAM_CONCATENATE_STRING_GIVEN_NUMBER_TIMES_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    C_PROGRAM_CONCATENATE_STRING_GIVEN_NUMBER_TIMES* obj = new C_PROGRAM_CONCATENATE_STRING_GIVEN_NUMBER_TIMES();
...
//
//[code snippet 2]
void C_PROGRAM_CONCATENATE_STRING_GIVEN_NUMBER_TIMES_C_P_C_S_G_N_T_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 2 || !args[0]->IsString() || !args[1]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    
    C_PROGRAM_CONCATENATE_STRING_GIVEN_NUMBER_TIMES* obj = static_cast<C_PROGRAM_CONCATENATE_STRING_GIVEN_NUMBER_TIMES*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());

    v8::String::Utf8Value str(args.GetIsolate(), args[0]);
    int n = args[1]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    
    std::string result = obj->C_P_C_S_G_N_T_f_gold(*str, n);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, C_PROGRAM_CONCATENATE_STRING_GIVEN_NUMBER_TIMES_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "C_PROGRAM_CONCATENATE_STRING_GIVEN_NUMBER_TIMES").ToLocalChecked());
    
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, C_PROGRAM_CONCATENATE_STRING_GIVEN_NUMBER_TIMES_C_P_C_S_G_N_T_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "C_P_C_S_G_N_T_f_gold", method_tpl);
    
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "C_PROGRAM_CONCATENATE_STRING_GIVEN_NUMBER_TIMES").ToLocalChecked(), constructor);
