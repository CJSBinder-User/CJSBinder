...
//
//[code snippet 1]
void FIND_A_TRIPLET_THAT_SUM_TO_A_GIVEN_VALUE_1_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    FIND_A_TRIPLET_THAT_SUM_TO_A_GIVEN_VALUE_1* obj = new FIND_A_TRIPLET_THAT_SUM_TO_A_GIVEN_VALUE_1();
...
//
//[code snippet 2]
void FIND_A_TRIPLET_THAT_SUM_TO_A_GIVEN_VALUE_1_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 3 || !args[1]->IsInt32() || !args[2]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }

    FIND_A_TRIPLET_THAT_SUM_TO_A_GIVEN_VALUE_1* obj = static_cast<FIND_A_TRIPLET_THAT_SUM_TO_A_GIVEN_VALUE_1*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    
    // Extract the sum
    int sum = args[0]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    ...
    // Call the C++ method
    bool result = obj->F_A_T_T_S_T_A_G_V_1_f_gold(A, arr_size, sum);
    delete[] A; // Clean up
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, FIND_A_TRIPLET_THAT_SUM_TO_A_GIVEN_VALUE_1_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "FIND_A_TRIPLET_THAT_SUM_TO_A_GIVEN_VALUE_1").ToLocalChecked());
    tpl->PrototypeTemplate()->Set(isolate, "F_A_T_T_S_T_A_G_V_1_f_gold", v8::FunctionTemplate::New(isolate, FIND_A_TRIPLET_THAT_SUM_TO_A_GIVEN_VALUE_1_f_gold));
    
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "FIND_A_TRIPLET_THAT_SUM_TO_A_GIVEN_VALUE_1").ToLocalChecked(), constructor);
