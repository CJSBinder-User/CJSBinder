...
//
//[code snippet 1]
void FIND_EQUAL_POINT_STRING_BRACKETS_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    FIND_EQUAL_POINT_STRING_BRACKETS* obj = new FIND_EQUAL_POINT_STRING_BRACKETS();
...
//
//[code snippet 2]
void FIND_EQUAL_POINT_STRING_BRACKETS_F_E_P_S_B_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsString()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }

    // Conversion of V8 string to std::string
    v8::String::Utf8Value utf8(args.GetIsolate(), args[0]);
    std::string str(*utf8);

    FIND_EQUAL_POINT_STRING_BRACKETS* obj = static_cast<FIND_EQUAL_POINT_STRING_BRACKETS*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int result = obj->F_E_P_S_B_f_gold(str);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, FIND_EQUAL_POINT_STRING_BRACKETS_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "FIND_EQUAL_POINT_STRING_BRACKETS").ToLocalChecked());

    // Register the method
    tpl->PrototypeTemplate()->Set(isolate, "F_E_P_S_B_f_gold", v8::FunctionTemplate::New(isolate, FIND_EQUAL_POINT_STRING_BRACKETS_F_E_P_S_B_f_gold));

    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "FIND_EQUAL_POINT_STRING_BRACKETS").ToLocalChecked(), constructor);
