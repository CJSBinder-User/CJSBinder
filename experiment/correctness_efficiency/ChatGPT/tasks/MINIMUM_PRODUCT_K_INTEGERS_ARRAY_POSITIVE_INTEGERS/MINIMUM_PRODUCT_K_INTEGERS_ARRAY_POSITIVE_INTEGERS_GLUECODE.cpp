...
//
//[code snippet 1]
void MINIMUM_PRODUCT_K_INTEGERS_ARRAY_POSITIVE_INTEGERS_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    MINIMUM_PRODUCT_K_INTEGERS_ARRAY_POSITIVE_INTEGERS* obj = new MINIMUM_PRODUCT_K_INTEGERS_ARRAY_POSITIVE_INTEGERS();
...
//
//[code snippet 2]
void MINIMUM_PRODUCT_K_INTEGERS_ARRAY_POSITIVE_INTEGERS_M_P_K_I_A_P_I_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 3 || !args[1]->IsInt32() || !args[2]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    
    MINIMUM_PRODUCT_K_INTEGERS_ARRAY_POSITIVE_INTEGERS* obj = static_cast<MINIMUM_PRODUCT_K_INTEGERS_ARRAY_POSITIVE_INTEGERS*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    ...
    for (int i = 0; i < n; i++) {
        ...
        arr[i] = element->IsInt32() ? element->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0) : 0; // Assuming default value if not an int
    }
    
    int result = obj->M_P_K_I_A_P_I_f_gold(arr, n, k);
    delete[] arr; // Free the allocated memory for the C++ array
...
// assumed that 'isolate' and 'context' have been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, MINIMUM_PRODUCT_K_INTEGERS_ARRAY_POSITIVE_INTEGERS_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "MINIMUM_PRODUCT_K_INTEGERS_ARRAY_POSITIVE_INTEGERS").ToLocalChecked());
    
    tpl->PrototypeTemplate()->Set(isolate, "M_P_K_I_A_P_I_f_gold", v8::FunctionTemplate::New(isolate, MINIMUM_PRODUCT_K_INTEGERS_ARRAY_POSITIVE_INTEGERS_M_P_K_I_A_P_I_f_gold));
    
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "MINIMUM_PRODUCT_K_INTEGERS_ARRAY_POSITIVE_INTEGERS").ToLocalChecked(), constructor);
