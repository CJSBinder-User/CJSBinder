...
//
//[code snippet 1]
void SUM_SERIES_ALTERNATE_SIGNED_SQUARES_AP_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    SUM_SERIES_ALTERNATE_SIGNED_SQUARES_AP* obj = new SUM_SERIES_ALTERNATE_SIGNED_SQUARES_AP();
...
//
//[code snippet 2]
void SUM_SERIES_ALTERNATE_SIGNED_SQUARES_AP_S_S_A_S_S_A_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 2 || !args[0]->IsInt32() || !args[1]->IsArray()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }

    SUM_SERIES_ALTERNATE_SIGNED_SQUARES_AP* obj = static_cast<SUM_SERIES_ALTERNATE_SIGNED_SQUARES_AP*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int n = args[0]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);

    v8::Local<v8::Array> jsArray = v8::Local<v8::Array>::Cast(args[1]);
    int* a = new int[n];
    ...
    int result = obj->S_S_A_S_S_A_f_gold(n, a);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, SUM_SERIES_ALTERNATE_SIGNED_SQUARES_AP_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "SUM_SERIES_ALTERNATE_SIGNED_SQUARES_AP").ToLocalChecked());
    
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, SUM_SERIES_ALTERNATE_SIGNED_SQUARES_AP_S_S_A_S_S_A_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "S_S_A_S_S_A_f_gold", method_tpl);
    
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "SUM_SERIES_ALTERNATE_SIGNED_SQUARES_AP").ToLocalChecked(), constructor);
