...
//
//[code snippet 1]
void SWAP_TWO_NIBBLES_BYTE_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    SWAP_TWO_NIBBLES_BYTE* obj = new SWAP_TWO_NIBBLES_BYTE();
...
//
//[code snippet 2]
void SWAP_TWO_NIBBLES_BYTE_S_T_N_B_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    SWAP_TWO_NIBBLES_BYTE* obj = static_cast<SWAP_TWO_NIBBLES_BYTE*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int x = args[0]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    int result = obj->S_T_N_B_f_gold(x);
...
// assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, SWAP_TWO_NIBBLES_BYTE_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "SWAP_TWO_NIBBLES_BYTE").ToLocalChecked());
    tpl->PrototypeTemplate()->Set(isolate, "S_T_N_B_f_gold", v8::FunctionTemplate::New(isolate, SWAP_TWO_NIBBLES_BYTE_S_T_N_B_f_gold));
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "SWAP_TWO_NIBBLES_BYTE").ToLocalChecked(), constructor);
