...
//
//[code snippet 1]
void SPACE_OPTIMIZED_DP_SOLUTION_0_1_KNAPSACK_PROBLEM_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    SPACE_OPTIMIZED_DP_SOLUTION_0_1_KNAPSACK_PROBLEM* obj = new SPACE_OPTIMIZED_DP_SOLUTION_0_1_KNAPSACK_PROBLEM();
...
//
//[code snippet 2]
void SPACE_OPTIMIZED_DP_SOLUTION_0_1_KNAPSACK_PROBLEM_S_O_D_S_0_1_K_P_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 4 || !args[0]->IsArray() || !args[1]->IsArray() || !args[2]->IsInt32() || !args[3]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }

    // Extracting the arrays and parameters
    ...
    int* wt = new int[n];
    for (int i = 0; i < n; ++i) {
        val[i] = valArray->Get(args.GetIsolate()->GetCurrentContext(), i).ToLocalChecked()->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
        wt[i] = wtArray->Get(args.GetIsolate()->GetCurrentContext(), i).ToLocalChecked()->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    }

    SPACE_OPTIMIZED_DP_SOLUTION_0_1_KNAPSACK_PROBLEM* obj = static_cast<SPACE_OPTIMIZED_DP_SOLUTION_0_1_KNAPSACK_PROBLEM*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int result = obj->S_O_D_S_0_1_K_P_f_gold(val, wt, n, W);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, SPACE_OPTIMIZED_DP_SOLUTION_0_1_KNAPSACK_PROBLEM_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "SPACE_OPTIMIZED_DP_SOLUTION_0_1_KNAPSACK_PROBLEM").ToLocalChecked());
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, SPACE_OPTIMIZED_DP_SOLUTION_0_1_KNAPSACK_PROBLEM_S_O_D_S_0_1_K_P_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "S_O_D_S_0_1_K_P_f_gold", method_tpl);
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "SPACE_OPTIMIZED_DP_SOLUTION_0_1_KNAPSACK_PROBLEM").ToLocalChecked(), constructor);
