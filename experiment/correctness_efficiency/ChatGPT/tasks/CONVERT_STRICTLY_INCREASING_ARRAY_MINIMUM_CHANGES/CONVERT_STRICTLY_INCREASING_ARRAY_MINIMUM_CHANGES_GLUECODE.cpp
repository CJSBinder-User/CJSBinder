...
//
//[code snippet 1]
void CONVERT_STRICTLY_INCREASING_ARRAY_MINIMUM_CHANGES_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    CONVERT_STRICTLY_INCREASING_ARRAY_MINIMUM_CHANGES* obj = new CONVERT_STRICTLY_INCREASING_ARRAY_MINIMUM_CHANGES();
...
//
//[code snippet 2]
void CONVERT_STRICTLY_INCREASING_ARRAY_MINIMUM_CHANGES_C_S_I_A_M_C_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 2 || !args[0]->IsArray() || !args[1]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    CONVERT_STRICTLY_INCREASING_ARRAY_MINIMUM_CHANGES* obj = static_cast<CONVERT_STRICTLY_INCREASING_ARRAY_MINIMUM_CHANGES*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());

    // Read the array from JavaScript
    ...
    for (int i = 0; i < n; ++i) {
        ...
        cppArray[i] = value->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    }

    int result = obj->C_S_I_A_M_C_f_gold(cppArray, n);
    delete[] cppArray; // Remember to clean up
...
//assumed that 'isolate' and 'context' have been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, CONVERT_STRICTLY_INCREASING_ARRAY_MINIMUM_CHANGES_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "CONVERT_STRICTLY_INCREASING_ARRAY_MINIMUM_CHANGES").ToLocalChecked());
    
    // Wrap the method
    tpl->PrototypeTemplate()->Set(isolate, "C_S_I_A_M_C_f_gold", v8::FunctionTemplate::New(isolate, CONVERT_STRICTLY_INCREASING_ARRAY_MINIMUM_CHANGES_C_S_I_A_M_C_f_gold));
    
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "CONVERT_STRICTLY_INCREASING_ARRAY_MINIMUM_CHANGES").ToLocalChecked(), constructor);
