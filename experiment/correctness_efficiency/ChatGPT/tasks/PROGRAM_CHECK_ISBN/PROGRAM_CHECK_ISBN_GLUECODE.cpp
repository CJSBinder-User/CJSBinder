...
//
//[code snippet 1]
void PROGRAM_CHECK_ISBN_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    PROGRAM_CHECK_ISBN* obj = new PROGRAM_CHECK_ISBN();
...
//
//[code snippet 2]
void PROGRAM_CHECK_ISBN_P_C_I_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    ...
    std::string isbn = std::string(*str);

    PROGRAM_CHECK_ISBN* obj = static_cast<PROGRAM_CHECK_ISBN*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    bool result = obj->P_C_I_f_gold(isbn);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, PROGRAM_CHECK_ISBN_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "PROGRAM_CHECK_ISBN").ToLocalChecked());
    
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, PROGRAM_CHECK_ISBN_P_C_I_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "P_C_I_f_gold", method_tpl);
    
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "PROGRAM_CHECK_ISBN").ToLocalChecked(), constructor);
