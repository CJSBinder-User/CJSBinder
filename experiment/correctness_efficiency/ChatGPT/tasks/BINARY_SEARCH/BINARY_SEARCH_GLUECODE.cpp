...
//
//[code snippet 1]
void BINARY_SEARCH_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
...
//
//[code snippet 2]
void BINARY_SEARCH_B_S_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    ...
    // Convert JavaScript array to a C++ array
    int arraySize = arr->Length();
    int* cArr = new int[arraySize];
    for (int i = 0; i < arraySize; ++i) {
        cArr[i] = arr->Get(args.GetIsolate()->GetCurrentContext(), i).ToLocalChecked()->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    }

    BINARY_SEARCH* obj = static_cast<BINARY_SEARCH*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int result = obj->B_S_f_gold(cArr, l, r, x);
    delete[] cArr; // Clean up the allocated array
...
//assumed that 'isolate' and 'context' have been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, BINARY_SEARCH_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "BINARY_SEARCH").ToLocalChecked());
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, BINARY_SEARCH_B_S_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "B_S_f_gold", method_tpl);
    
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "BINARY_SEARCH").ToLocalChecked(), constructor);
