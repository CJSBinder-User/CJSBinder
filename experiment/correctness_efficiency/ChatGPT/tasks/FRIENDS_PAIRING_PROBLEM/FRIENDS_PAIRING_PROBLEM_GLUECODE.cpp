...
//
//[code snippet 1]
void FRIENDS_PAIRING_PROBLEM_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    FRIENDS_PAIRING_PROBLEM* obj = new FRIENDS_PAIRING_PROBLEM();
...
//
//[code snippet 2]
void FRIENDS_PAIRING_PROBLEM_F_P_P_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    FRIENDS_PAIRING_PROBLEM* obj = static_cast<FRIENDS_PAIRING_PROBLEM*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int n = args[0]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    int result = obj->F_P_P_f_gold(n);
...
//assumed that 'isolate' and 'context' have been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, FRIENDS_PAIRING_PROBLEM_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "FRIENDS_PAIRING_PROBLEM").ToLocalChecked());
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, FRIENDS_PAIRING_PROBLEM_F_P_P_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "F_P_P_f_gold", method_tpl);
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "FRIENDS_PAIRING_PROBLEM").ToLocalChecked(), constructor);
