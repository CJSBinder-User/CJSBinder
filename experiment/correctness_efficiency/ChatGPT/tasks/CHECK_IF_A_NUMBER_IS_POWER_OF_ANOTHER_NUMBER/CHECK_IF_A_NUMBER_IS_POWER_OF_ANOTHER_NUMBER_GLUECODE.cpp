...
//
//[code snippet 1]
void CHECK_IF_A_NUMBER_IS_POWER_OF_ANOTHER_NUMBER_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    CHECK_IF_A_NUMBER_IS_POWER_OF_ANOTHER_NUMBER* obj = new CHECK_IF_A_NUMBER_IS_POWER_OF_ANOTHER_NUMBER();
...
//
//[code snippet 2]
void CHECK_IF_A_NUMBER_IS_POWER_OF_ANOTHER_NUMBER_C_I_A_N_I_P_O_A_N_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 2 || !args[0]->IsInt32() || !args[1]->IsNumber()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    CHECK_IF_A_NUMBER_IS_POWER_OF_ANOTHER_NUMBER* obj = static_cast<CHECK_IF_A_NUMBER_IS_POWER_OF_ANOTHER_NUMBER*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int x = args[0]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    long y = static_cast<long>(args[1]->NumberValue(args.GetIsolate()->GetCurrentContext()).FromMaybe(0));
    bool result = obj->C_I_A_N_I_P_O_A_N_f_gold(x, y);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, CHECK_IF_A_NUMBER_IS_POWER_OF_ANOTHER_NUMBER_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "CHECK_IF_A_NUMBER_IS_POWER_OF_ANOTHER_NUMBER").ToLocalChecked());
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, CHECK_IF_A_NUMBER_IS_POWER_OF_ANOTHER_NUMBER_C_I_A_N_I_P_O_A_N_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "C_I_A_N_I_P_O_A_N_f_gold", method_tpl);
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "CHECK_IF_A_NUMBER_IS_POWER_OF_ANOTHER_NUMBER").ToLocalChecked(), constructor);
