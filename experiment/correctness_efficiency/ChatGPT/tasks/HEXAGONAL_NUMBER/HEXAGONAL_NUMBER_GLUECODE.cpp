...
//
//[code snippet 1]
void HEXAGONAL_NUMBER_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    HEXAGONAL_NUMBER* obj = new HEXAGONAL_NUMBER();
...
//
//[code snippet 2]
void HEXAGONAL_NUMBER_H_N_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    HEXAGONAL_NUMBER* obj = static_cast<HEXAGONAL_NUMBER*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int n = args[0]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    int result = obj->H_N_f_gold(n);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, HEXAGONAL_NUMBER_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "HEXAGONAL_NUMBER").ToLocalChecked());
    tpl->PrototypeTemplate()->Set(isolate, "H_N_f_gold", v8::FunctionTemplate::New(isolate, HEXAGONAL_NUMBER_H_N_f_gold));
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "HEXAGONAL_NUMBER").ToLocalChecked(), constructor);
