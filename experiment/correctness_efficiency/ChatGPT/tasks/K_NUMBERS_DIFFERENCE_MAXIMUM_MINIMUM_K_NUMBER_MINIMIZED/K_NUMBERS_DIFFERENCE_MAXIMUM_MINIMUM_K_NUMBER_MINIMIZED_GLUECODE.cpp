...
//
//[code snippet 1]
void K_NUMBERS_DIFFERENCE_MAXIMUM_MINIMUM_K_NUMBER_MINIMIZED_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    K_NUMBERS_DIFFERENCE_MAXIMUM_MINIMUM_K_NUMBER_MINIMIZED* obj = new K_NUMBERS_DIFFERENCE_MAXIMUM_MINIMUM_K_NUMBER_MINIMIZED();
...
//
//[code snippet 2]
void K_N_D_M_M_K_N_M_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 3 || !args[1]->IsInt32() || !args[2]->IsInt32() || !args[0]->IsArray()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    
    K_NUMBERS_DIFFERENCE_MAXIMUM_MINIMUM_K_NUMBER_MINIMIZED* obj = static_cast<K_NUMBERS_DIFFERENCE_MAXIMUM_MINIMUM_K_NUMBER_MINIMIZED*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    ...
    // Converting the JavaScript array to a C++ array
    int* c_arr = new int[n];
    for (int i = 0; i < n; ++i) {
    ...
    int result = obj->K_N_D_M_M_K_N_M_f_gold(c_arr, n, k);
...
// assumed that 'isolate' and 'context' have been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, K_NUMBERS_DIFFERENCE_MAXIMUM_MINIMUM_K_NUMBER_MINIMIZED_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "K_NUMBERS_DIFFERENCE_MAXIMUM_MINIMUM_K_NUMBER_MINIMIZED").ToLocalChecked());
    tpl->PrototypeTemplate()->Set(isolate, "K_N_D_M_M_K_N_M_f_gold", v8::FunctionTemplate::New(isolate, K_N_D_M_M_K_N_M_f_gold));
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "K_NUMBERS_DIFFERENCE_MAXIMUM_MINIMUM_K_NUMBER_MINIMIZED").ToLocalChecked(), constructor);
