...
//
//[code snippet 1]
void LONGEST_COMMON_INCREASING_SUBSEQUENCE_LCS_LIS_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    LONGEST_COMMON_INCREASING_SUBSEQUENCE_LCS_LIS* obj = new LONGEST_COMMON_INCREASING_SUBSEQUENCE_LCS_LIS();
...
//
//[code snippet 2]
void LONGEST_COMMON_INCREASING_SUBSEQUENCE_LCS_LIS_L_C_I_S_L_L_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 3 || !args[0]->IsArray() || !args[1]->IsInt32() || !args[2]->IsArray()) {
    ...
    v8::Local<v8::Array> arr2 = v8::Local<v8::Array>::Cast(args[2]);
    ...
    int* c_arr2 = new int[m];

    // Filling the C++ arrays with values from the JavaScript arrays
    for (int i = 0; i < n; i++) {
    ...
    LONGEST_COMMON_INCREASING_SUBSEQUENCE_LCS_LIS* obj = static_cast<LONGEST_COMMON_INCREASING_SUBSEQUENCE_LCS_LIS*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int result = obj->L_C_I_S_L_L_f_gold(c_arr1, n, c_arr2, m);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, LONGEST_COMMON_INCREASING_SUBSEQUENCE_LCS_LIS_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "LONGEST_COMMON_INCREASING_SUBSEQUENCE_LCS_LIS").ToLocalChecked());
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, LONGEST_COMMON_INCREASING_SUBSEQUENCE_LCS_LIS_L_C_I_S_L_L_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "L_C_I_S_L_L_f_gold", method_tpl);
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "LONGEST_COMMON_INCREASING_SUBSEQUENCE_LCS_LIS").ToLocalChecked(), constructor);
