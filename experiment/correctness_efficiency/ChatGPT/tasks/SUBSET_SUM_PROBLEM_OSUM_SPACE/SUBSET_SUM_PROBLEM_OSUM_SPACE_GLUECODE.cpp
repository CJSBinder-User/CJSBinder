...
//
//[code snippet 1]
void SUBSET_SUM_PROBLEM_OSUM_SPACE_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    SUBSET_SUM_PROBLEM_OSUM_SPACE* obj = new SUBSET_SUM_PROBLEM_OSUM_SPACE();
...
//
//[code snippet 2]
void SUBSET_SUM_PROBLEM_OSUM_SPACE_S_S_P_O_S_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 3 || !args[0]->IsArray() || !args[1]->IsInt32() || !args[2]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }

    // Extract the array from the JavaScript argument
    ...
    SUBSET_SUM_PROBLEM_OSUM_SPACE* obj = static_cast<SUBSET_SUM_PROBLEM_OSUM_SPACE*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    bool result = obj->S_S_P_O_S_f_gold(arr, n, sum);
    delete[] arr; // Remember to free the allocated array
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, SUBSET_SUM_PROBLEM_OSUM_SPACE_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "SUBSET_SUM_PROBLEM_OSUM_SPACE").ToLocalChecked());
    tpl->PrototypeTemplate()->Set(isolate, "S_S_P_O_S_f_gold", v8::FunctionTemplate::New(isolate, SUBSET_SUM_PROBLEM_OSUM_SPACE_S_S_P_O_S_f_gold));
    
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "SUBSET_SUM_PROBLEM_OSUM_SPACE").ToLocalChecked(), constructor);
