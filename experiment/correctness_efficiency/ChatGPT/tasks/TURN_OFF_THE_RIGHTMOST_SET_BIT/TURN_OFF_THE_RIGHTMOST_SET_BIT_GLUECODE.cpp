...
//
//[code snippet 1]
void TURN_OFF_THE_RIGHTMOST_SET_BIT_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    TURN_OFF_THE_RIGHTMOST_SET_BIT* obj = new TURN_OFF_THE_RIGHTMOST_SET_BIT();
...
//
//[code snippet 2]
void TURN_OFF_THE_RIGHTMOST_SET_BIT_T_O_T_R_S_B_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsUint32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    TURN_OFF_THE_RIGHTMOST_SET_BIT* obj = static_cast<TURN_OFF_THE_RIGHTMOST_SET_BIT*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    unsigned int n = args[0]->Uint32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    int result = obj->T_O_T_R_S_B_f_gold(n);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, TURN_OFF_THE_RIGHTMOST_SET_BIT_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "TURN_OFF_THE_RIGHTMOST_SET_BIT").ToLocalChecked());
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, TURN_OFF_THE_RIGHTMOST_SET_BIT_T_O_T_R_S_B_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "T_O_T_R_S_B_f_gold", method_tpl);
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "TURN_OFF_THE_RIGHTMOST_SET_BIT").ToLocalChecked(), constructor);
