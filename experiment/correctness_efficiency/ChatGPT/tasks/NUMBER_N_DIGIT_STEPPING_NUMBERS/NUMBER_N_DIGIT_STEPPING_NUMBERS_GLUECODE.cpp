...
//
//[code snippet 1]
void NUMBER_N_DIGIT_STEPPING_NUMBERS_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    NUMBER_N_DIGIT_STEPPING_NUMBERS* obj = new NUMBER_N_DIGIT_STEPPING_NUMBERS();
...
//
//[code snippet 2]
void NUMBER_N_DIGIT_STEPPING_NUMBERS_N_N_D_S_N_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    NUMBER_N_DIGIT_STEPPING_NUMBERS* obj = static_cast<NUMBER_N_DIGIT_STEPPING_NUMBERS*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int n = args[0]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    long long result = obj->N_N_D_S_N_f_gold(n);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, NUMBER_N_DIGIT_STEPPING_NUMBERS_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "NUMBER_N_DIGIT_STEPPING_NUMBERS").ToLocalChecked());
    
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, NUMBER_N_DIGIT_STEPPING_NUMBERS_N_N_D_S_N_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "N_N_D_S_N_f_gold", method_tpl);
    
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "NUMBER_N_DIGIT_STEPPING_NUMBERS").ToLocalChecked(), constructor);
