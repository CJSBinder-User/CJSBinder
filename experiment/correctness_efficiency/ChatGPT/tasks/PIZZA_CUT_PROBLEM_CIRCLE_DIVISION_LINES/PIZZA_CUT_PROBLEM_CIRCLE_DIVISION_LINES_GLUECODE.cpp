...
//
//[code snippet 1]
void PIZZA_CUT_PROBLEM_CIRCLE_DIVISION_LINES_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    PIZZA_CUT_PROBLEM_CIRCLE_DIVISION_LINES* obj = new PIZZA_CUT_PROBLEM_CIRCLE_DIVISION_LINES();
...
//
//[code snippet 2]
void PIZZA_CUT_PROBLEM_CIRCLE_DIVISION_LINES_P_C_P_C_D_L_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    PIZZA_CUT_PROBLEM_CIRCLE_DIVISION_LINES* obj = static_cast<PIZZA_CUT_PROBLEM_CIRCLE_DIVISION_LINES*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int n = args[0]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    int result = obj->P_C_P_C_D_L_f_gold(n);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, PIZZA_CUT_PROBLEM_CIRCLE_DIVISION_LINES_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "PIZZA_CUT_PROBLEM_CIRCLE_DIVISION_LINES").ToLocalChecked());
    tpl->PrototypeTemplate()->Set(isolate, "P_C_P_C_D_L_f_gold", v8::FunctionTemplate::New(isolate, PIZZA_CUT_PROBLEM_CIRCLE_DIVISION_LINES_P_C_P_C_D_L_f_gold));
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "PIZZA_CUT_PROBLEM_CIRCLE_DIVISION_LINES").ToLocalChecked(), constructor);
