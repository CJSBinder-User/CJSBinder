...
//
//[code snippet 1]
void HEIGHT_COMPLETE_BINARY_TREE_HEAP_N_NODES_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    HEIGHT_COMPLETE_BINARY_TREE_HEAP_N_NODES* obj = new HEIGHT_COMPLETE_BINARY_TREE_HEAP_N_NODES();
...
//
//[code snippet 2]
void HEIGHT_COMPLETE_BINARY_TREE_HEAP_N_NODES_H_C_B_T_H_N_N_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    HEIGHT_COMPLETE_BINARY_TREE_HEAP_N_NODES* obj = static_cast<HEIGHT_COMPLETE_BINARY_TREE_HEAP_N_NODES*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int N = args[0]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    int result = obj->H_C_B_T_H_N_N_f_gold(N);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, HEIGHT_COMPLETE_BINARY_TREE_HEAP_N_NODES_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "HEIGHT_COMPLETE_BINARY_TREE_HEAP_N_NODES").ToLocalChecked());

    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, HEIGHT_COMPLETE_BINARY_TREE_HEAP_N_NODES_H_C_B_T_H_N_N_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "H_C_B_T_H_N_N_f_gold", method_tpl);

    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "HEIGHT_COMPLETE_BINARY_TREE_HEAP_N_NODES").ToLocalChecked(), constructor);
