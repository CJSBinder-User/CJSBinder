...
//
//[code snippet 1]
void FIND_SUBARRAY_WITH_GIVEN_SUM_1_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    FIND_SUBARRAY_WITH_GIVEN_SUM_1* obj = new FIND_SUBARRAY_WITH_GIVEN_SUM_1();
...
//
//[code snippet 2]
void FIND_SUBARRAY_WITH_GIVEN_SUM_1_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 3 || !args[0]->IsArray() || !args[1]->IsInt32() || !args[2]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }

    // Retrieve the array from JavaScript
    ...
    // Convert the JavaScript array to C++ array
    int* arr = new int[n];
    for (int i = 0; i < n; i++) {
        v8::MaybeLocal<v8::Value> maybeValue = jsArray->Get(args.GetIsolate()->GetCurrentContext(), i);
        if (maybeValue.IsEmpty() || !maybeValue.ToLocal(&v8::Value)) {
            delete[] arr;
            args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
            return;
        }
        arr[i] = maybeValue.ToLocalChecked()->Int32Value(args.GetIsolate()->GetCurrentContext()).ToChecked();
    }

    FIND_SUBARRAY_WITH_GIVEN_SUM_1* obj = static_cast<FIND_SUBARRAY_WITH_GIVEN_SUM_1*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int result = obj->F_S_W_G_S_1_f_gold(arr, n, sum);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, FIND_SUBARRAY_WITH_GIVEN_SUM_1_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "FIND_SUBARRAY_WITH_GIVEN_SUM_1").ToLocalChecked());
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, FIND_SUBARRAY_WITH_GIVEN_SUM_1_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "F_S_W_G_S_1_f_gold", method_tpl);
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "FIND_SUBARRAY_WITH_GIVEN_SUM_1").ToLocalChecked(), constructor);
