...
//
//[code snippet 1]
void CHECK_ARRAY_MAJORITY_ELEMENT_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    CHECK_ARRAY_MAJORITY_ELEMENT* obj = new CHECK_ARRAY_MAJORITY_ELEMENT();
...
//
//[code snippet 2]
void CHECK_ARRAY_MAJORITY_ELEMENT_C_A_M_E_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        c_array[i] = value->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    }
    
    CHECK_ARRAY_MAJORITY_ELEMENT* obj = static_cast<CHECK_ARRAY_MAJORITY_ELEMENT*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    bool result = obj->C_A_M_E_f_gold(c_array, n);
    
    delete[] c_array; // Clean up heap-allocated memory
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, CHECK_ARRAY_MAJORITY_ELEMENT_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "CHECK_ARRAY_MAJORITY_ELEMENT").ToLocalChecked());
    
    tpl->PrototypeTemplate()->Set(isolate, "C_A_M_E_f_gold", v8::FunctionTemplate::New(isolate, CHECK_ARRAY_MAJORITY_ELEMENT_C_A_M_E_f_gold));
    
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "CHECK_ARRAY_MAJORITY_ELEMENT").ToLocalChecked(), constructor);
