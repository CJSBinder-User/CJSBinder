...
//
//[code snippet 1]
void MINIMUM_PERIMETER_N_BLOCKS_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    MINIMUM_PERIMETER_N_BLOCKS* obj = new MINIMUM_PERIMETER_N_BLOCKS();
...
//
//[code snippet 2]
void MINIMUM_PERIMETER_N_BLOCKS_M_P_N_B_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    MINIMUM_PERIMETER_N_BLOCKS* obj = static_cast<MINIMUM_PERIMETER_N_BLOCKS*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int n = args[0]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    int result = obj->M_P_N_B_f_gold(n);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, MINIMUM_PERIMETER_N_BLOCKS_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "MINIMUM_PERIMETER_N_BLOCKS").ToLocalChecked());
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, MINIMUM_PERIMETER_N_BLOCKS_M_P_N_B_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "M_P_N_B_f_gold", method_tpl);
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "MINIMUM_PERIMETER_N_BLOCKS").ToLocalChecked(), constructor);
