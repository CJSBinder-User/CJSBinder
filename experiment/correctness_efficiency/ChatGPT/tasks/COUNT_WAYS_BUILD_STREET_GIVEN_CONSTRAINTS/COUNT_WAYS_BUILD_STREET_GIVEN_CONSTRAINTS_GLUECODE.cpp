...
//
//[code snippet 1]
void COUNT_WAYS_BUILD_STREET_GIVEN_CONSTRAINTS_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    COUNT_WAYS_BUILD_STREET_GIVEN_CONSTRAINTS* obj = new COUNT_WAYS_BUILD_STREET_GIVEN_CONSTRAINTS();
...
//
//[code snippet 2]
void COUNT_WAYS_BUILD_STREET_GIVEN_CONSTRAINTS_C_W_B_S_G_C_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    COUNT_WAYS_BUILD_STREET_GIVEN_CONSTRAINTS* obj = static_cast<COUNT_WAYS_BUILD_STREET_GIVEN_CONSTRAINTS*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int n = args[0]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    long result = obj->C_W_B_S_G_C_f_gold(n);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, COUNT_WAYS_BUILD_STREET_GIVEN_CONSTRAINTS_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "COUNT_WAYS_BUILD_STREET_GIVEN_CONSTRAINTS").ToLocalChecked());
    
    // Registering the method
    tpl->PrototypeTemplate()->Set(isolate, "C_W_B_S_G_C_f_gold", v8::FunctionTemplate::New(isolate, COUNT_WAYS_BUILD_STREET_GIVEN_CONSTRAINTS_C_W_B_S_G_C_f_gold));

    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "COUNT_WAYS_BUILD_STREET_GIVEN_CONSTRAINTS").ToLocalChecked(), constructor);
