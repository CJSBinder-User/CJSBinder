...
//
//[code snippet 1]
void SUM_NODES_K_TH_LEVEL_TREE_REPRESENTED_STRING_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    SUM_NODES_K_TH_LEVEL_TREE_REPRESENTED_STRING* obj = new SUM_NODES_K_TH_LEVEL_TREE_REPRESENTED_STRING();
...
//
//[code snippet 2]
void SUM_NODES_K_TH_LEVEL_TREE_REPRESENTED_STRING_S_N_K_T_L_T_R_S_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 2 || !args[0]->IsString() || !args[1]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    
    v8::String::Utf8Value tree(args.GetIsolate(), args[0]);
    int k = args[1]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);

    SUM_NODES_K_TH_LEVEL_TREE_REPRESENTED_STRING* obj = static_cast<SUM_NODES_K_TH_LEVEL_TREE_REPRESENTED_STRING*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int result = obj->S_N_K_T_L_T_R_S_f_gold(std::string(*tree), k);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, SUM_NODES_K_TH_LEVEL_TREE_REPRESENTED_STRING_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "SUM_NODES_K_TH_LEVEL_TREE_REPRESENTED_STRING").ToLocalChecked());
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, SUM_NODES_K_TH_LEVEL_TREE_REPRESENTED_STRING_S_N_K_T_L_T_R_S_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "S_N_K_T_L_T_R_S_f_gold", method_tpl);
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "SUM_NODES_K_TH_LEVEL_TREE_REPRESENTED_STRING").ToLocalChecked(), constructor);
