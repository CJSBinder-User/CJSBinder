...
//
//[code snippet 1]
void MAXIMUM_SUBARRAY_SUM_USING_PREFIX_SUM_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    MAXIMUM_SUBARRAY_SUM_USING_PREFIX_SUM* obj = new MAXIMUM_SUBARRAY_SUM_USING_PREFIX_SUM();
...
//
//[code snippet 2]
void MAXIMUM_SUBARRAY_SUM_USING_PREFIX_SUM_M_S_S_U_P_S_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (value->IsInt32()) {
            ...
            delete[] arr; // cleanup on error
            args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
            return;
        }
    }

    MAXIMUM_SUBARRAY_SUM_USING_PREFIX_SUM* obj = static_cast<MAXIMUM_SUBARRAY_SUM_USING_PREFIX_SUM*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int result = obj->M_S_S_U_P_S_f_gold(arr, n);
    delete[] arr; // cleanup
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, MAXIMUM_SUBARRAY_SUM_USING_PREFIX_SUM_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "MAXIMUM_SUBARRAY_SUM_USING_PREFIX_SUM").ToLocalChecked());
    
    tpl->PrototypeTemplate()->Set(isolate, "M_S_S_U_P_S_f_gold", v8::FunctionTemplate::New(isolate, MAXIMUM_SUBARRAY_SUM_USING_PREFIX_SUM_M_S_S_U_P_S_f_gold));
    
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "MAXIMUM_SUBARRAY_SUM_USING_PREFIX_SUM").ToLocalChecked(), constructor);
