...
//
//[code snippet 1]
void FIND_A_TRIPLET_THAT_SUM_TO_A_GIVEN_VALUE_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    FIND_A_TRIPLET_THAT_SUM_TO_A_GIVEN_VALUE* obj = new FIND_A_TRIPLET_THAT_SUM_TO_A_GIVEN_VALUE();
...
//
//[code snippet 2]
void FIND_A_TRIPLET_THAT_SUM_TO_A_GIVEN_VALUE_F_A_T_T_S_T_A_G_V_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    ...
    FIND_A_TRIPLET_THAT_SUM_TO_A_GIVEN_VALUE* obj = 
        static_cast<FIND_A_TRIPLET_THAT_SUM_TO_A_GIVEN_VALUE*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    ...
    for (int i = 0; i < arr_size; i++) {
        A[i] = jsArray->Get(args.GetIsolate()->GetCurrentContext(), i).ToLocalChecked()->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    }

    // Call the C++ method
    bool result = obj->F_A_T_T_S_T_A_G_V_f_gold(A, arr_size, sum);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, FIND_A_TRIPLET_THAT_SUM_TO_A_GIVEN_VALUE_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "FIND_A_TRIPLET_THAT_SUM_TO_A_GIVEN_VALUE").ToLocalChecked());
    
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, FIND_A_TRIPLET_THAT_SUM_TO_A_GIVEN_VALUE_F_A_T_T_S_T_A_G_V_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "F_A_T_T_S_T_A_G_V_f_gold", method_tpl);
    
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "FIND_A_TRIPLET_THAT_SUM_TO_A_GIVEN_VALUE").ToLocalChecked(), constructor);
