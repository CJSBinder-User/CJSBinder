...
//
//[code snippet 1]
void COUNT_SET_BITS_IN_AN_INTEGER_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    COUNT_SET_BITS_IN_AN_INTEGER* obj = new COUNT_SET_BITS_IN_AN_INTEGER();
...
//
//[code snippet 2]
void COUNT_SET_BITS_IN_AN_INTEGER_C_S_B_I_A_I_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsUint32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    COUNT_SET_BITS_IN_AN_INTEGER* obj = static_cast<COUNT_SET_BITS_IN_AN_INTEGER*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    unsigned int n = args[0]->Uint32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    unsigned int result = obj->C_S_B_I_A_I_f_gold(n);
...
// assumed that 'isolate' and 'context' have been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, COUNT_SET_BITS_IN_AN_INTEGER_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "COUNT_SET_BITS_IN_AN_INTEGER").ToLocalChecked());
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, COUNT_SET_BITS_IN_AN_INTEGER_C_S_B_I_A_I_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "C_S_B_I_A_I_f_gold", method_tpl);
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "COUNT_SET_BITS_IN_AN_INTEGER").ToLocalChecked(), constructor);
