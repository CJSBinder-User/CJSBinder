...
//
//[code snippet 1]
void PROGRAM_AREA_SQUARE_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    PROGRAM_AREA_SQUARE* obj = new PROGRAM_AREA_SQUARE();
...
//
//[code snippet 2]
void PROGRAM_AREA_SQUARE_P_A_S_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    PROGRAM_AREA_SQUARE* obj = static_cast<PROGRAM_AREA_SQUARE*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int side = args[0]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    int result = obj->P_A_S_f_gold(side);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, PROGRAM_AREA_SQUARE_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "PROGRAM_AREA_SQUARE").ToLocalChecked());
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, PROGRAM_AREA_SQUARE_P_A_S_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "P_A_S_f_gold", method_tpl);
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "PROGRAM_AREA_SQUARE").ToLocalChecked(), constructor);
