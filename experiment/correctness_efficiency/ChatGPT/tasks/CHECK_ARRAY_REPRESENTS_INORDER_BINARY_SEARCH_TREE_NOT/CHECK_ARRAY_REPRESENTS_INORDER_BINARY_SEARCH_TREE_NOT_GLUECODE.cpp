...
//
//[code snippet 1]
void CHECK_ARRAY_REPRESENTS_INORDER_BINARY_SEARCH_TREE_NOT_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    CHECK_ARRAY_REPRESENTS_INORDER_BINARY_SEARCH_TREE_NOT* obj = new CHECK_ARRAY_REPRESENTS_INORDER_BINARY_SEARCH_TREE_NOT();
...
//
//[code snippet 2]
void CHECK_ARRAY_REPRESENTS_INORDER_BINARY_SEARCH_TREE_NOT_C_A_R_I_B_S_T_N_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 2 || !args[0]->IsArray() || !args[1]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    CHECK_ARRAY_REPRESENTS_INORDER_BINARY_SEARCH_TREE_NOT* obj = static_cast<CHECK_ARRAY_REPRESENTS_INORDER_BINARY_SEARCH_TREE_NOT*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    ...
    bool result = obj->C_A_R_I_B_S_T_N_f_gold(c_arr, n);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, CHECK_ARRAY_REPRESENTS_INORDER_BINARY_SEARCH_TREE_NOT_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "CHECK_ARRAY_REPRESENTS_INORDER_BINARY_SEARCH_TREE_NOT").ToLocalChecked());
    
    tpl->PrototypeTemplate()->Set(isolate, "C_A_R_I_B_S_T_N_f_gold",
        v8::FunctionTemplate::New(isolate, CHECK_ARRAY_REPRESENTS_INORDER_BINARY_SEARCH_TREE_NOT_C_A_R_I_B_S_T_N_f_gold));

    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "CHECK_ARRAY_REPRESENTS_INORDER_BINARY_SEARCH_TREE_NOT").ToLocalChecked(), constructor);
