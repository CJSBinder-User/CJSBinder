...
//
//[code snippet 1]
void PROGRAM_CALCULATE_VOLUME_OCTAHEDRON_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    PROGRAM_CALCULATE_VOLUME_OCTAHEDRON* obj = new PROGRAM_CALCULATE_VOLUME_OCTAHEDRON();
...
//
//[code snippet 2]
void PROGRAM_CALCULATE_VOLUME_OCTAHEDRON_P_C_V_O_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsNumber()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    PROGRAM_CALCULATE_VOLUME_OCTAHEDRON* obj = static_cast<PROGRAM_CALCULATE_VOLUME_OCTAHEDRON*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    double side = args[0]->NumberValue(args.GetIsolate()->GetCurrentContext()).FromMaybe(0.0);
    double result = obj->P_C_V_O_f_gold(side);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, PROGRAM_CALCULATE_VOLUME_OCTAHEDRON_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "PROGRAM_CALCULATE_VOLUME_OCTAHEDRON").ToLocalChecked());
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, PROGRAM_CALCULATE_VOLUME_OCTAHEDRON_P_C_V_O_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "P_C_V_O_f_gold", method_tpl);
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "PROGRAM_CALCULATE_VOLUME_OCTAHEDRON").ToLocalChecked(), constructor);
