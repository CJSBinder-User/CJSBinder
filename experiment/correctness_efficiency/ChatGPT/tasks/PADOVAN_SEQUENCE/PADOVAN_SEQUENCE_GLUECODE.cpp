...
//
//[code snippet 1]
void PADOVAN_SEQUENCE_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    PADOVAN_SEQUENCE* obj = new PADOVAN_SEQUENCE();
...
//
//[code snippet 2]
void PADOVAN_SEQUENCE_P_S_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    PADOVAN_SEQUENCE* obj = static_cast<PADOVAN_SEQUENCE*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int n = args[0]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    int result = obj->P_S_f_gold(n);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, PADOVAN_SEQUENCE_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "PADOVAN_SEQUENCE").ToLocalChecked());
    
    tpl->PrototypeTemplate()->Set(isolate, "P_S_f_gold", v8::FunctionTemplate::New(isolate, PADOVAN_SEQUENCE_P_S_f_gold));
    
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "PADOVAN_SEQUENCE").ToLocalChecked(), constructor);
