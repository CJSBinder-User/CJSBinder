...
//
//[code snippet 1]
void FIBONACCI_MODULO_P_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    FIBONACCI_MODULO_P* obj = new FIBONACCI_MODULO_P();
...
//
//[code snippet 2]
void FIBONACCI_MODULO_P_F_M_P_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    FIBONACCI_MODULO_P* obj = static_cast<FIBONACCI_MODULO_P*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int p = args[0]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    int result = obj->F_M_P_f_gold(p);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, FIBONACCI_MODULO_P_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "FIBONACCI_MODULO_P").ToLocalChecked());
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, FIBONACCI_MODULO_P_F_M_P_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "F_M_P_f_gold", method_tpl);
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "FIBONACCI_MODULO_P").ToLocalChecked(), constructor);
