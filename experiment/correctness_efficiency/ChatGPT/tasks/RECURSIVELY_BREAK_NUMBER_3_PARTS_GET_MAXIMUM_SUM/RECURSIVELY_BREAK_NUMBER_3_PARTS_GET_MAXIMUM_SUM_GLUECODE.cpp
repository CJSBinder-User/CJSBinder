...
//
//[code snippet 1]
void RECURSIVELY_BREAK_NUMBER_3_PARTS_GET_MAXIMUM_SUM_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    RECURSIVELY_BREAK_NUMBER_3_PARTS_GET_MAXIMUM_SUM* obj = new RECURSIVELY_BREAK_NUMBER_3_PARTS_GET_MAXIMUM_SUM();
...
//
//[code snippet 2]
void RECURSIVELY_BREAK_NUMBER_3_PARTS_GET_MAXIMUM_SUM_R_B_N_3_P_G_M_S_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    RECURSIVELY_BREAK_NUMBER_3_PARTS_GET_MAXIMUM_SUM* obj = static_cast<RECURSIVELY_BREAK_NUMBER_3_PARTS_GET_MAXIMUM_SUM*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int n = args[0]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    int result = obj->R_B_N_3_P_G_M_S_f_gold(n);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, RECURSIVELY_BREAK_NUMBER_3_PARTS_GET_MAXIMUM_SUM_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "RECURSIVELY_BREAK_NUMBER_3_PARTS_GET_MAXIMUM_SUM").ToLocalChecked());
    tpl->PrototypeTemplate()->Set(isolate, "R_B_N_3_P_G_M_S_f_gold", v8::FunctionTemplate::New(isolate, RECURSIVELY_BREAK_NUMBER_3_PARTS_GET_MAXIMUM_SUM_R_B_N_3_P_G_M_S_f_gold));
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "RECURSIVELY_BREAK_NUMBER_3_PARTS_GET_MAXIMUM_SUM").ToLocalChecked(), constructor);
