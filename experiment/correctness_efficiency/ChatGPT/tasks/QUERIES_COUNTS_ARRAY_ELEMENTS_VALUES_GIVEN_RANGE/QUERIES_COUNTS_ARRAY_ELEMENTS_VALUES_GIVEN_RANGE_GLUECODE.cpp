...
//
//[code snippet 1]
void QUERIES_COUNTS_ARRAY_ELEMENTS_VALUES_GIVEN_RANGE_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    QUERIES_COUNTS_ARRAY_ELEMENTS_VALUES_GIVEN_RANGE* obj = new QUERIES_COUNTS_ARRAY_ELEMENTS_VALUES_GIVEN_RANGE();
...
//
//[code snippet 2]
void QUERIES_COUNTS_ARRAY_ELEMENTS_VALUES_GIVEN_RANGE_Q_C_A_E_V_G_R_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 4 || 
        !args[0]->IsArray() ||
        !args[1]->IsInt32() ||
        !args[2]->IsInt32() ||
    ...
    int y = args[3]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);

    int* cArray = new int[n];
    for (int i = 0; i < n; ++i) {
        v8::Local<v8::Value> value = arr->Get(args.GetIsolate()->GetCurrentContext(), i).ToLocalChecked();
        cArray[i] = value->IsInt32() ? value->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0) : 0; // Handle non-integer values
    }

    QUERIES_COUNTS_ARRAY_ELEMENTS_VALUES_GIVEN_RANGE* obj = static_cast<QUERIES_COUNTS_ARRAY_ELEMENTS_VALUES_GIVEN_RANGE*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int result = obj->Q_C_A_E_V_G_R_f_gold(cArray, n, x, y);
    delete[] cArray; // Clean up allocated memory
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, QUERIES_COUNTS_ARRAY_ELEMENTS_VALUES_GIVEN_RANGE_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "QUERIES_COUNTS_ARRAY_ELEMENTS_VALUES_GIVEN_RANGE").ToLocalChecked());
    tpl->PrototypeTemplate()->Set(isolate, "Q_C_A_E_V_G_R_f_gold", v8::FunctionTemplate::New(isolate, QUERIES_COUNTS_ARRAY_ELEMENTS_VALUES_GIVEN_RANGE_Q_C_A_E_V_G_R_f_gold));
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "QUERIES_COUNTS_ARRAY_ELEMENTS_VALUES_GIVEN_RANGE").ToLocalChecked(), constructor);
