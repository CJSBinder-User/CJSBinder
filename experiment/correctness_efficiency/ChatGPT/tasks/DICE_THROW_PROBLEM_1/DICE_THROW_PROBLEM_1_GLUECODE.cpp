...
//
//[code snippet 1]
void DICE_THROW_PROBLEM_1_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    DICE_THROW_PROBLEM_1* obj = new DICE_THROW_PROBLEM_1();
...
//
//[code snippet 2]
void DICE_THROW_PROBLEM_1_D_T_P_1_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 3 || !args[0]->IsInt32() || !args[1]->IsInt32() || !args[2]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    DICE_THROW_PROBLEM_1* obj = static_cast<DICE_THROW_PROBLEM_1*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int f = args[0]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    int d = args[1]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    int s = args[2]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    long result = obj->D_T_P_1_f_gold(f, d, s);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, DICE_THROW_PROBLEM_1_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "DICE_THROW_PROBLEM_1").ToLocalChecked());
    tpl->PrototypeTemplate()->Set(isolate, "D_T_P_1_f_gold", v8::FunctionTemplate::New(isolate, DICE_THROW_PROBLEM_1_D_T_P_1_f_gold));
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "DICE_THROW_PROBLEM_1").ToLocalChecked(), constructor);
