...
//
//[code snippet 1]
void FIND_LARGEST_PRIME_FACTOR_NUMBER_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    FIND_LARGEST_PRIME_FACTOR_NUMBER* obj = new FIND_LARGEST_PRIME_FACTOR_NUMBER();
...
//
//[code snippet 2]
void FIND_LARGEST_PRIME_FACTOR_NUMBER_F_L_P_F_N_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsNumber()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    FIND_LARGEST_PRIME_FACTOR_NUMBER* obj = static_cast<FIND_LARGEST_PRIME_FACTOR_NUMBER*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    long long n = args[0]->IntegerValue(args.GetIsolate()->GetCurrentContext()).ToChecked();
    long long result = obj->F_L_P_F_N_f_gold(n);
...
//assumed that 'isolate' and 'context' have been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, FIND_LARGEST_PRIME_FACTOR_NUMBER_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "FIND_LARGEST_PRIME_FACTOR_NUMBER").ToLocalChecked());
    tpl->PrototypeTemplate()->Set(isolate, "F_L_P_F_N_f_gold", v8::FunctionTemplate::New(isolate, FIND_LARGEST_PRIME_FACTOR_NUMBER_F_L_P_F_N_f_gold));
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "FIND_LARGEST_PRIME_FACTOR_NUMBER").ToLocalChecked(), constructor);
