...
//
//[code snippet 1]
void UGLY_NUMBERS_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    UGLY_NUMBERS* obj = new UGLY_NUMBERS();
...
//
//[code snippet 2]
void UGLY_NUMBERS_U_N_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsUint32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    UGLY_NUMBERS* obj = static_cast<UGLY_NUMBERS*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    unsigned int n = args[0]->Uint32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    unsigned int result = obj->U_N_f_gold(n);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, UGLY_NUMBERS_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "UGLY_NUMBERS").ToLocalChecked());
    
    tpl->PrototypeTemplate()->Set(isolate, "U_N_f_gold", v8::FunctionTemplate::New(isolate, UGLY_NUMBERS_U_N_f_gold));

    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "UGLY_NUMBERS").ToLocalChecked(), constructor);
