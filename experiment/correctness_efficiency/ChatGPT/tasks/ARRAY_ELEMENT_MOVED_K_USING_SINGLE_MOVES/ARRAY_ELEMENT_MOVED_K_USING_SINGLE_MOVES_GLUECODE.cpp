...
//
//[code snippet 1]
void ARRAY_ELEMENT_MOVED_K_USING_SINGLE_MOVES_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    ARRAY_ELEMENT_MOVED_K_USING_SINGLE_MOVES* obj = new ARRAY_ELEMENT_MOVED_K_USING_SINGLE_MOVES();
...
//
//[code snippet 2]
void ARRAY_ELEMENT_MOVED_K_USING_SINGLE_MOVES_A_E_M_K_U_S_M_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 3 || !args[0]->IsArray() || !args[1]->IsInt32() || !args[2]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }

    // Extracting array from JavaScript
    ...
    // Converting JavaScript array to C++ array
    int* a = new int[n];
    for (int i = 0; i < n; i++) {
    ...
    ARRAY_ELEMENT_MOVED_K_USING_SINGLE_MOVES* obj = static_cast<ARRAY_ELEMENT_MOVED_K_USING_SINGLE_MOVES*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int result = obj->A_E_M_K_U_S_M_f_gold(a, n, k);
    
    delete[] a; // Clean up allocated array
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, ARRAY_ELEMENT_MOVED_K_USING_SINGLE_MOVES_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "ARRAY_ELEMENT_MOVED_K_USING_SINGLE_MOVES").ToLocalChecked());
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, ARRAY_ELEMENT_MOVED_K_USING_SINGLE_MOVES_A_E_M_K_U_S_M_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "A_E_M_K_U_S_M_f_gold", method_tpl);
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "ARRAY_ELEMENT_MOVED_K_USING_SINGLE_MOVES").ToLocalChecked(), constructor);
