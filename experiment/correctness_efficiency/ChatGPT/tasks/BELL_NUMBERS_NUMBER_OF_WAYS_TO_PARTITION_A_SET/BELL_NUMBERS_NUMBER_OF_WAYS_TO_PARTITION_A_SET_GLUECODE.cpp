...
//
//[code snippet 1]
void BELL_NUMBERS_NUMBER_OF_WAYS_TO_PARTITION_A_SET_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    BELL_NUMBERS_NUMBER_OF_WAYS_TO_PARTITION_A_SET* obj = new BELL_NUMBERS_NUMBER_OF_WAYS_TO_PARTITION_A_SET();
...
//
//[code snippet 2]
void BELL_NUMBERS_NUMBER_OF_WAYS_TO_PARTITION_A_SET_B_N_N_O_W_T_P_A_S_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    BELL_NUMBERS_NUMBER_OF_WAYS_TO_PARTITION_A_SET* obj = static_cast<BELL_NUMBERS_NUMBER_OF_WAYS_TO_PARTITION_A_SET*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int n = args[0]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    int result = obj->B_N_N_O_W_T_P_A_S_f_gold(n);
...
//assumed that 'isolate' and 'context' have been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, BELL_NUMBERS_NUMBER_OF_WAYS_TO_PARTITION_A_SET_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "BELL_NUMBERS_NUMBER_OF_WAYS_TO_PARTITION_A_SET").ToLocalChecked());
    
    // Registering the method
    tpl->PrototypeTemplate()->Set(isolate, "B_N_N_O_W_T_P_A_S_f_gold", v8::FunctionTemplate::New(isolate, BELL_NUMBERS_NUMBER_OF_WAYS_TO_PARTITION_A_SET_B_N_N_O_W_T_P_A_S_f_gold));
    
    // Creating the constructor function
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    
    // Registering the constructor in the global context
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "BELL_NUMBERS_NUMBER_OF_WAYS_TO_PARTITION_A_SET").ToLocalChecked(), constructor);
