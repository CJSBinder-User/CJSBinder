...
//
//[code snippet 1]
void PROGRAM_PRINT_SUM_GIVEN_NTH_TERM_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    PROGRAM_PRINT_SUM_GIVEN_NTH_TERM* obj = new PROGRAM_PRINT_SUM_GIVEN_NTH_TERM();
...
//
//[code snippet 2]
void PROGRAM_PRINT_SUM_GIVEN_NTH_TERM_P_P_S_G_N_T_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsNumber()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    PROGRAM_PRINT_SUM_GIVEN_NTH_TERM* obj = static_cast<PROGRAM_PRINT_SUM_GIVEN_NTH_TERM*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    long n = static_cast<long>(args[0]->NumberValue(args.GetIsolate()->GetCurrentContext()).FromMaybe(0));
    int result = obj->P_P_S_G_N_T_f_gold(n);
...
// assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, PROGRAM_PRINT_SUM_GIVEN_NTH_TERM_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "PROGRAM_PRINT_SUM_GIVEN_NTH_TERM").ToLocalChecked());
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, PROGRAM_PRINT_SUM_GIVEN_NTH_TERM_P_P_S_G_N_T_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "P_P_S_G_N_T_f_gold", method_tpl);
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "PROGRAM_PRINT_SUM_GIVEN_NTH_TERM").ToLocalChecked(), constructor);
