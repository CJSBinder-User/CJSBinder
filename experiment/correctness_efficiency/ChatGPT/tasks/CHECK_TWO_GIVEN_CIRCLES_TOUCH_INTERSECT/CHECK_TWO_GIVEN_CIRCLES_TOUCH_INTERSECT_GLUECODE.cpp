...
//
//[code snippet 1]
void CHECK_TWO_GIVEN_CIRCLES_TOUCH_INTERSECT_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    CHECK_TWO_GIVEN_CIRCLES_TOUCH_INTERSECT* obj = new CHECK_TWO_GIVEN_CIRCLES_TOUCH_INTERSECT();
...
//
//[code snippet 2]
void CHECK_TWO_GIVEN_CIRCLES_TOUCH_INTERSECT_C_T_G_C_T_I_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 6 || !args[0]->IsInt32() || !args[1]->IsInt32() || 
        !args[2]->IsInt32() || !args[3]->IsInt32() || 
        !args[4]->IsInt32() || !args[5]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    CHECK_TWO_GIVEN_CIRCLES_TOUCH_INTERSECT* obj = 
        static_cast<CHECK_TWO_GIVEN_CIRCLES_TOUCH_INTERSECT*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    ...
    int r1 = args[4]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    int r2 = args[5]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    int result = obj->C_T_G_C_T_I_f_gold(x1, y1, x2, y2, r1, r2);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, CHECK_TWO_GIVEN_CIRCLES_TOUCH_INTERSECT_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "CHECK_TWO_GIVEN_CIRCLES_TOUCH_INTERSECT").ToLocalChecked());
    
    // Registering the function
    tpl->PrototypeTemplate()->Set(isolate, 
        "C_T_G_C_T_I_f_gold", 
        v8::FunctionTemplate::New(isolate, CHECK_TWO_GIVEN_CIRCLES_TOUCH_INTERSECT_C_T_G_C_T_I_f_gold));
    
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, 
        v8::String::NewFromUtf8(isolate, "CHECK_TWO_GIVEN_CIRCLES_TOUCH_INTERSECT").ToLocalChecked(), 
        constructor);
