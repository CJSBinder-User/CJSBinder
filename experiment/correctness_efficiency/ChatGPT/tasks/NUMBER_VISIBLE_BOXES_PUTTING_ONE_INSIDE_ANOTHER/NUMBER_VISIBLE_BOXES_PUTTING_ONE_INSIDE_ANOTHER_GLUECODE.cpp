...
//
//[code snippet 1]
void NUMBER_VISIBLE_BOXES_PUTTING_ONE_INSIDE_ANOTHER_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    NUMBER_VISIBLE_BOXES_PUTTING_ONE_INSIDE_ANOTHER* obj = new NUMBER_VISIBLE_BOXES_PUTTING_ONE_INSIDE_ANOTHER();
...
//
//[code snippet 2]
void NUMBER_VISIBLE_BOXES_PUTTING_ONE_INSIDE_ANOTHER_N_V_B_P_O_I_A_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 2 || !args[0]->IsArray() || !args[1]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    // Extract the C++ array from JavaScript array
    ...
    for (int i = 0; i < n; ++i) {
        ...
        cArray[i] = element->IsInt32() ? element->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0) : 0;
    }

    NUMBER_VISIBLE_BOXES_PUTTING_ONE_INSIDE_ANOTHER* obj = static_cast<NUMBER_VISIBLE_BOXES_PUTTING_ONE_INSIDE_ANOTHER*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int result = obj->N_V_B_P_O_I_A_f_gold(cArray, n);
    
    delete[] cArray; // Clean up the allocated memory
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, NUMBER_VISIBLE_BOXES_PUTTING_ONE_INSIDE_ANOTHER_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "NUMBER_VISIBLE_BOXES_PUTTING_ONE_INSIDE_ANOTHER").ToLocalChecked());
    
    // Register the method
    tpl->PrototypeTemplate()->Set(isolate, "N_V_B_P_O_I_A_f_gold", v8::FunctionTemplate::New(isolate, NUMBER_VISIBLE_BOXES_PUTTING_ONE_INSIDE_ANOTHER_N_V_B_P_O_I_A_f_gold));
    
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "NUMBER_VISIBLE_BOXES_PUTTING_ONE_INSIDE_ANOTHER").ToLocalChecked(), constructor);
