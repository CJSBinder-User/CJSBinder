...
//
//[code snippet 1]
void ROUND_THE_GIVEN_NUMBER_TO_NEAREST_MULTIPLE_OF_10_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    ROUND_THE_GIVEN_NUMBER_TO_NEAREST_MULTIPLE_OF_10* obj = new ROUND_THE_GIVEN_NUMBER_TO_NEAREST_MULTIPLE_OF_10();
...
//
//[code snippet 2]
void ROUND_THE_GIVEN_NUMBER_TO_NEAREST_MULTIPLE_OF_10_R_T_G_N_T_N_M_O_1_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    ROUND_THE_GIVEN_NUMBER_TO_NEAREST_MULTIPLE_OF_10* obj = static_cast<ROUND_THE_GIVEN_NUMBER_TO_NEAREST_MULTIPLE_OF_10*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int n = args[0]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    int result = obj->R_T_G_N_T_N_M_O_1_f_gold(n);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, ROUND_THE_GIVEN_NUMBER_TO_NEAREST_MULTIPLE_OF_10_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "ROUND_THE_GIVEN_NUMBER_TO_NEAREST_MULTIPLE_OF_10").ToLocalChecked());

    tpl->PrototypeTemplate()->Set(isolate, "R_T_G_N_T_N_M_O_1_f_gold", v8::FunctionTemplate::New(isolate, ROUND_THE_GIVEN_NUMBER_TO_NEAREST_MULTIPLE_OF_10_R_T_G_N_T_N_M_O_1_f_gold));

    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "ROUND_THE_GIVEN_NUMBER_TO_NEAREST_MULTIPLE_OF_10").ToLocalChecked(), constructor);
