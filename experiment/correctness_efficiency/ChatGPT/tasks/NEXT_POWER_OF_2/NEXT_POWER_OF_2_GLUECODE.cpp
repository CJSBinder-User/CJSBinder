...
//
//[code snippet 1]
void NEXT_POWER_OF_2_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    NEXT_POWER_OF_2* obj = new NEXT_POWER_OF_2();
...
//
//[code snippet 2]
void NEXT_POWER_OF_2_N_P_O_2_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsUint32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    NEXT_POWER_OF_2* obj = static_cast<NEXT_POWER_OF_2*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    unsigned int n = args[0]->Uint32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    unsigned int result = obj->N_P_O_2_f_gold(n);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, NEXT_POWER_OF_2_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "NEXT_POWER_OF_2").ToLocalChecked());
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, NEXT_POWER_OF_2_N_P_O_2_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "N_P_O_2_f_gold", method_tpl);
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "NEXT_POWER_OF_2").ToLocalChecked(), constructor);
