...
//
//[code snippet 1]
void TRIANGULAR_NUMBERS_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    TRIANGULAR_NUMBERS* obj = new TRIANGULAR_NUMBERS();
...
//
//[code snippet 2]
void TRIANGULAR_NUMBERS_T_N_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    TRIANGULAR_NUMBERS* obj = static_cast<TRIANGULAR_NUMBERS*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int num = args[0]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    bool result = obj->T_N_f_gold(num);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, TRIANGULAR_NUMBERS_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "TRIANGULAR_NUMBERS").ToLocalChecked());
    
    // Register the method
    tpl->PrototypeTemplate()->Set(isolate, "T_N_f_gold", v8::FunctionTemplate::New(isolate, TRIANGULAR_NUMBERS_T_N_f_gold));
    
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "TRIANGULAR_NUMBERS").ToLocalChecked(), constructor);
