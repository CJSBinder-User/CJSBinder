...
//
//[code snippet 1]
void CAESAR_CIPHER_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
...
//
//[code snippet 2]
void CAESAR_CIPHER_C_C_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 2 || !args[0]->IsString() || !args[1]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }

    // Convert the first argument from V8 String to C++ std::string
    v8::String::Utf8Value utf8_value(args.GetIsolate(), args[0]);
    std::string text(*utf8_value); // Create a C++ string from the v8 string

    // Get the second argument (an integer)
    int s = args[1]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);

    CAESAR_CIPHER* obj = static_cast<CAESAR_CIPHER*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    std::string result = obj->C_C_f_gold(text, s);
    
    // Convert the result back to V8 String and set the return value
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, CAESAR_CIPHER_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "CAESAR_CIPHER").ToLocalChecked());

    // Bind the method C_C_f_gold
    tpl->PrototypeTemplate()->Set(isolate, "C_C_f_gold", v8::FunctionTemplate::New(isolate, CAESAR_CIPHER_C_C_f_gold));

    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "CAESAR_CIPHER").ToLocalChecked(), constructor);
