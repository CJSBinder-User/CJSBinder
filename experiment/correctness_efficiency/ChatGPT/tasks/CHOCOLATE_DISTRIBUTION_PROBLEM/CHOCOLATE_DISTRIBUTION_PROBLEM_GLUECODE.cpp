...
//
//[code snippet 1]
void CHOCOLATE_DISTRIBUTION_PROBLEM_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    CHOCOLATE_DISTRIBUTION_PROBLEM* obj = new CHOCOLATE_DISTRIBUTION_PROBLEM();
...
//
//[code snippet 2]
void CHOCOLATE_DISTRIBUTION_PROBLEM_C_D_P_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 3 || !args[0]->IsArray() || !args[1]->IsInt32() || !args[2]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    
    // Retrieving the array from JavaScript
    ...
    CHOCOLATE_DISTRIBUTION_PROBLEM* obj = static_cast<CHOCOLATE_DISTRIBUTION_PROBLEM*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int result = obj->C_D_P_f_gold(arr, n, m);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, CHOCOLATE_DISTRIBUTION_PROBLEM_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "CHOCOLATE_DISTRIBUTION_PROBLEM").ToLocalChecked());
    
    tpl->PrototypeTemplate()->Set(isolate, "C_D_P_f_gold", v8::FunctionTemplate::New(isolate, CHOCOLATE_DISTRIBUTION_PROBLEM_C_D_P_f_gold));
    
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "CHOCOLATE_DISTRIBUTION_PROBLEM").ToLocalChecked(), constructor);
