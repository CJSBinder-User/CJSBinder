...
//
//[code snippet 1]
void CHECK_GIVEN_SENTENCE_GIVEN_SET_SIMPLE_GRAMMER_RULES_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    CHECK_GIVEN_SENTENCE_GIVEN_SET_SIMPLE_GRAMMER_RULES* obj = new CHECK_GIVEN_SENTENCE_GIVEN_SET_SIMPLE_GRAMMER_RULES();
...
//
//[code snippet 2]
void CHECK_GIVEN_SENTENCE_GIVEN_SET_SIMPLE_GRAMMER_RULES_C_G_S_G_S_S_G_R_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsString()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    
    CHECK_GIVEN_SENTENCE_GIVEN_SET_SIMPLE_GRAMMER_RULES* obj = static_cast<CHECK_GIVEN_SENTENCE_GIVEN_SET_SIMPLE_GRAMMER_RULES*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    
    v8::String::Utf8Value utf8(args.GetIsolate(), args[0]);
    string str = *utf8;  // Convert V8 string to std::string
    
    bool result = obj->C_G_S_G_S_S_G_R_f_gold(str);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, CHECK_GIVEN_SENTENCE_GIVEN_SET_SIMPLE_GRAMMER_RULES_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "CHECK_GIVEN_SENTENCE_GIVEN_SET_SIMPLE_GRAMMER_RULES").ToLocalChecked());
    
    tpl->PrototypeTemplate()->Set(isolate, "C_G_S_G_S_S_G_R_f_gold", v8::FunctionTemplate::New(isolate, CHECK_GIVEN_SENTENCE_GIVEN_SET_SIMPLE_GRAMMER_RULES_C_G_S_G_S_S_G_R_f_gold));
    
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "CHECK_GIVEN_SENTENCE_GIVEN_SET_SIMPLE_GRAMMER_RULES").ToLocalChecked(), constructor);
