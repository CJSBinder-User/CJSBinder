...
//
//[code snippet 1]
void SMALLEST_SUBSET_SUM_GREATER_ELEMENTS_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    SMALLEST_SUBSET_SUM_GREATER_ELEMENTS* obj = new SMALLEST_SUBSET_SUM_GREATER_ELEMENTS();
...
//
//[code snippet 2]
void SMALLEST_SUBSET_SUM_GREATER_ELEMENTS_S_S_S_G_E_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 2 || !args[0]->IsArray() || !args[1]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }

    // Extracting the array from the arguments
    ...
    SMALLEST_SUBSET_SUM_GREATER_ELEMENTS* obj = static_cast<SMALLEST_SUBSET_SUM_GREATER_ELEMENTS*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int result = obj->S_S_S_G_E_f_gold(arr, n);
...
//assumed that 'isolate' and 'context' have been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, SMALLEST_SUBSET_SUM_GREATER_ELEMENTS_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "SMALLEST_SUBSET_SUM_GREATER_ELEMENTS").ToLocalChecked());
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, SMALLEST_SUBSET_SUM_GREATER_ELEMENTS_S_S_S_G_E_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "S_S_S_G_E_f_gold", method_tpl);
    
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "SMALLEST_SUBSET_SUM_GREATER_ELEMENTS").ToLocalChecked(), constructor);
