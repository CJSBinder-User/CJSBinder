...
//
//[code snippet 1]
void FIND_LARGEST_D_IN_ARRAY_SUCH_THAT_A_B_C_D_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    FIND_LARGEST_D_IN_ARRAY_SUCH_THAT_A_B_C_D* obj = new FIND_LARGEST_D_IN_ARRAY_SUCH_THAT_A_B_C_D();
...
//
//[code snippet 2]
void FIND_LARGEST_D_IN_ARRAY_SUCH_THAT_A_B_C_D_F_L_D_I_A_S_T_A_B_C_D_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 2 || !args[0]->IsArray() || !args[1]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    FIND_LARGEST_D_IN_ARRAY_SUCH_THAT_A_B_C_D* obj = static_cast<FIND_LARGEST_D_IN_ARRAY_SUCH_THAT_A_B_C_D*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    ...
    // Convert V8 array to native C++ array
    int* S = new int[n];
    for (int i = 0; i < n; ++i) {
        v8::Local<v8::Value> element = inputArray->Get(args.GetIsolate()->GetCurrentContext(), i).ToLocalChecked();
        S[i] = element->IsInt32() ? element->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0) : 0;
    }

    int result = obj->F_L_D_I_A_S_T_A_B_C_D_f_gold(S, n);
    delete[] S; // clean up allocated memory
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, FIND_LARGEST_D_IN_ARRAY_SUCH_THAT_A_B_C_D_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "FIND_LARGEST_D_IN_ARRAY_SUCH_THAT_A_B_C_D").ToLocalChecked());
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, FIND_LARGEST_D_IN_ARRAY_SUCH_THAT_A_B_C_D_F_L_D_I_A_S_T_A_B_C_D_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "F_L_D_I_A_S_T_A_B_C_D_f_gold", method_tpl);
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "FIND_LARGEST_D_IN_ARRAY_SUCH_THAT_A_B_C_D").ToLocalChecked(), constructor);
