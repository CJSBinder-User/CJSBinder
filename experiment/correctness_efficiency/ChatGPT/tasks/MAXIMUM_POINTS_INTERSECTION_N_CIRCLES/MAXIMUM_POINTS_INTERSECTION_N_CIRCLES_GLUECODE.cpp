...
//
//[code snippet 1]
void MAXIMUM_POINTS_INTERSECTION_N_CIRCLES_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    MAXIMUM_POINTS_INTERSECTION_N_CIRCLES* obj = new MAXIMUM_POINTS_INTERSECTION_N_CIRCLES();
...
//
//[code snippet 2]
void MAXIMUM_POINTS_INTERSECTION_N_CIRCLES_M_P_I_N_C_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    MAXIMUM_POINTS_INTERSECTION_N_CIRCLES* obj = static_cast<MAXIMUM_POINTS_INTERSECTION_N_CIRCLES*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int n = args[0]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    int result = obj->M_P_I_N_C_f_gold(n);
...
//assumed that 'isolate' and 'context' have been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, MAXIMUM_POINTS_INTERSECTION_N_CIRCLES_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "MAXIMUM_POINTS_INTERSECTION_N_CIRCLES").ToLocalChecked());
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, MAXIMUM_POINTS_INTERSECTION_N_CIRCLES_M_P_I_N_C_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "M_P_I_N_C_f_gold", method_tpl);
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "MAXIMUM_POINTS_INTERSECTION_N_CIRCLES").ToLocalChecked(), constructor);
