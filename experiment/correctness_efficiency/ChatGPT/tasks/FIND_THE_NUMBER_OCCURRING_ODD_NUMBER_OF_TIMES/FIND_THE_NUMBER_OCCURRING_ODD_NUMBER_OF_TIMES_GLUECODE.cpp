...
//
//[code snippet 1]
void FIND_THE_NUMBER_OCCURRING_ODD_NUMBER_OF_TIMES_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    FIND_THE_NUMBER_OCCURRING_ODD_NUMBER_OF_TIMES* obj = new FIND_THE_NUMBER_OCCURRING_ODD_NUMBER_OF_TIMES();
...
//
//[code snippet 2]
void FIND_THE_NUMBER_OCCURRING_ODD_NUMBER_OF_TIMES_F_T_N_O_O_N_O_T_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 2 || !args[0]->IsArray() || !args[1]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    
    FIND_THE_NUMBER_OCCURRING_ODD_NUMBER_OF_TIMES* obj = static_cast<FIND_THE_NUMBER_OCCURRING_ODD_NUMBER_OF_TIMES*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    ...
    int result = obj->F_T_N_O_O_N_O_T_f_gold(arr, arr_size);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, FIND_THE_NUMBER_OCCURRING_ODD_NUMBER_OF_TIMES_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "FIND_THE_NUMBER_OCCURRING_ODD_NUMBER_OF_TIMES").ToLocalChecked());
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, FIND_THE_NUMBER_OCCURRING_ODD_NUMBER_OF_TIMES_F_T_N_O_O_N_O_T_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "F_T_N_O_O_N_O_T_f_gold", method_tpl);
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "FIND_THE_NUMBER_OCCURRING_ODD_NUMBER_OF_TIMES").ToLocalChecked(), constructor);
