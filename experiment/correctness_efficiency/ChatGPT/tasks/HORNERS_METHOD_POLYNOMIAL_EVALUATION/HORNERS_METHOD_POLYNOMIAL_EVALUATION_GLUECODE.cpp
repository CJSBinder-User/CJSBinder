...
//
//[code snippet 1]
void HORNERS_METHOD_POLYNOMIAL_EVALUATION_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    HORNERS_METHOD_POLYNOMIAL_EVALUATION* obj = new HORNERS_METHOD_POLYNOMIAL_EVALUATION();
...
//
//[code snippet 2]
void HORNERS_METHOD_POLYNOMIAL_EVALUATION_H_M_P_E_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 3 || !args[0]->IsArray() || !args[1]->IsInt32() || !args[2]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }

    // Extract the polynomial array
    v8::Local<v8::Array> polyArray = v8::Local<v8::Array>::Cast(args[0]);
    ...
    int* poly = new int[n];
    for (int i = 0; i < n; i++) {
        v8::Local<v8::Value> value = polyArray->Get(args.GetIsolate()->GetCurrentContext(), i).ToLocalChecked();
        poly[i] = value->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    }

    HORNERS_METHOD_POLYNOMIAL_EVALUATION* obj = static_cast<HORNERS_METHOD_POLYNOMIAL_EVALUATION*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int result = obj->H_M_P_E_f_gold(poly, n, x);
    
    delete[] poly; // Clean up the dynamically allocated array
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, HORNERS_METHOD_POLYNOMIAL_EVALUATION_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "HORNERS_METHOD_POLYNOMIAL_EVALUATION").ToLocalChecked());
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, HORNERS_METHOD_POLYNOMIAL_EVALUATION_H_M_P_E_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "H_M_P_E_f_gold", method_tpl);
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "HORNERS_METHOD_POLYNOMIAL_EVALUATION").ToLocalChecked(), constructor);
