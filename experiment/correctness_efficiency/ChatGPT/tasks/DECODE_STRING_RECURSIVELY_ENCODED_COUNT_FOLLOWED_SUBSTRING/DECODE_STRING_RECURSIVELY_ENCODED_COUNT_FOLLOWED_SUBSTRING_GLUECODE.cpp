...
//
//[code snippet 1]
void DECODE_STRING_RECURSIVELY_ENCODED_COUNT_FOLLOWED_SUBSTRING_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    DECODE_STRING_RECURSIVELY_ENCODED_COUNT_FOLLOWED_SUBSTRING* obj = new DECODE_STRING_RECURSIVELY_ENCODED_COUNT_FOLLOWED_SUBSTRING();
...
//
//[code snippet 2]
void DECODE_STRING_RECURSIVELY_ENCODED_COUNT_FOLLOWED_SUBSTRING_D_S_R_E_C_F_S_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsString()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    v8::String::Utf8Value str(args.GetIsolate(), args[0]->ToString(args.GetIsolate()->GetCurrentContext()).ToLocalChecked());
    DECODE_STRING_RECURSIVELY_ENCODED_COUNT_FOLLOWED_SUBSTRING* obj = static_cast<DECODE_STRING_RECURSIVELY_ENCODED_COUNT_FOLLOWED_SUBSTRING*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    std::string result = obj->D_S_R_E_C_F_S_f_gold(*str);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, DECODE_STRING_RECURSIVELY_ENCODED_COUNT_FOLLOWED_SUBSTRING_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "DECODE_STRING_RECURSIVELY_ENCODED_COUNT_FOLLOWED_SUBSTRING").ToLocalChecked());
    tpl->PrototypeTemplate()->Set(isolate, "D_S_R_E_C_F_S_f_gold", v8::FunctionTemplate::New(isolate, DECODE_STRING_RECURSIVELY_ENCODED_COUNT_FOLLOWED_SUBSTRING_D_S_R_E_C_F_S_f_gold));
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "DECODE_STRING_RECURSIVELY_ENCODED_COUNT_FOLLOWED_SUBSTRING").ToLocalChecked(), constructor);
