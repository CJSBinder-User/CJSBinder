...
//
//[code snippet 1]
void TRIANGULAR_MATCHSTICK_NUMBER_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    TRIANGULAR_MATCHSTICK_NUMBER* obj = new TRIANGULAR_MATCHSTICK_NUMBER();
...
//
//[code snippet 2]
void TRIANGULAR_MATCHSTICK_NUMBER_T_M_N_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    TRIANGULAR_MATCHSTICK_NUMBER* obj = static_cast<TRIANGULAR_MATCHSTICK_NUMBER*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int x = args[0]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    int result = obj->T_M_N_f_gold(x);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, TRIANGULAR_MATCHSTICK_NUMBER_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "TRIANGULAR_MATCHSTICK_NUMBER").ToLocalChecked());
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, TRIANGULAR_MATCHSTICK_NUMBER_T_M_N_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "T_M_N_f_gold", method_tpl);
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "TRIANGULAR_MATCHSTICK_NUMBER").ToLocalChecked(), constructor);
