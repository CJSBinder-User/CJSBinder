...
//
//[code snippet 1]
void LARGEST_SUBARRAY_WITH_EQUAL_NUMBER_OF_0S_AND_1S_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    LARGEST_SUBARRAY_WITH_EQUAL_NUMBER_OF_0S_AND_1S* obj = new LARGEST_SUBARRAY_WITH_EQUAL_NUMBER_OF_0S_AND_1S();
...
//
//[code snippet 2]
void LARGEST_SUBARRAY_WITH_EQUAL_NUMBER_OF_0S_AND_1S_L_S_W_E_N_O_0_A_1_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 2 || !args[0]->IsArray() || !args[1]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }

    LARGEST_SUBARRAY_WITH_EQUAL_NUMBER_OF_0S_AND_1S* obj = static_cast<LARGEST_SUBARRAY_WITH_EQUAL_NUMBER_OF_0S_AND_1S*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    ...
    int* arr = new int[n];
    for (int i = 0; i < n; i++) {
        v8::Local<v8::Value> element = jsArr->Get(args.GetIsolate()->GetCurrentContext(), i).ToLocalChecked();
        if (element->IsInt32()) {
            arr[i] = element->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
        }
    }
    
    int result = obj->L_S_W_E_N_O_0_A_1_f_gold(arr, n);
    delete[] arr; // Don't forget to free the dynamically allocated array
...
//assumed that 'isolate' and 'context' have been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, LARGEST_SUBARRAY_WITH_EQUAL_NUMBER_OF_0S_AND_1S_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "LARGEST_SUBARRAY_WITH_EQUAL_NUMBER_OF_0S_AND_1S").ToLocalChecked());
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, LARGEST_SUBARRAY_WITH_EQUAL_NUMBER_OF_0S_AND_1S_L_S_W_E_N_O_0_A_1_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "L_S_W_E_N_O_0_A_1_f_gold", method_tpl);
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "LARGEST_SUBARRAY_WITH_EQUAL_NUMBER_OF_0S_AND_1S").ToLocalChecked(), constructor);
