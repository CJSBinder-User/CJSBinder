...
//
//[code snippet 1]
void COUNT_INDEX_PAIRS_EQUAL_ELEMENTS_ARRAY_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    COUNT_INDEX_PAIRS_EQUAL_ELEMENTS_ARRAY* obj = new COUNT_INDEX_PAIRS_EQUAL_ELEMENTS_ARRAY();
...
//
//[code snippet 2]
void COUNT_INDEX_PAIRS_EQUAL_ELEMENTS_ARRAY_C_I_P_E_E_A_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 2 || !args[0]->IsArray() || !args[1]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }

    COUNT_INDEX_PAIRS_EQUAL_ELEMENTS_ARRAY* obj = static_cast<COUNT_INDEX_PAIRS_EQUAL_ELEMENTS_ARRAY*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    ...
    // Call the C++ function
    int result = obj->C_I_P_E_E_A_f_gold(arr, n);
...
//assumed that 'isolate' and 'context' have been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, COUNT_INDEX_PAIRS_EQUAL_ELEMENTS_ARRAY_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "COUNT_INDEX_PAIRS_EQUAL_ELEMENTS_ARRAY").ToLocalChecked());
    
    // Register the method
    tpl->PrototypeTemplate()->Set(isolate, "C_I_P_E_E_A_f_gold", v8::FunctionTemplate::New(isolate, COUNT_INDEX_PAIRS_EQUAL_ELEMENTS_ARRAY_C_I_P_E_E_A_f_gold));
    
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "COUNT_INDEX_PAIRS_EQUAL_ELEMENTS_ARRAY").ToLocalChecked(), constructor);
