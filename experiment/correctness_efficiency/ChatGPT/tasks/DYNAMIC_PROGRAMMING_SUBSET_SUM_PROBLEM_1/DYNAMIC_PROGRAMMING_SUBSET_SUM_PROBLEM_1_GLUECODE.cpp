...
//
//[code snippet 1]
void DYNAMIC_PROGRAMMING_SUBSET_SUM_PROBLEM_1_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    DYNAMIC_PROGRAMMING_SUBSET_SUM_PROBLEM_1* obj = new DYNAMIC_PROGRAMMING_SUBSET_SUM_PROBLEM_1();
...
//
//[code snippet 2]
void D_P_S_S_P_1_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    ...
    int sum = args[2]->Int32Value(args.GetIsolate()->GetCurrentContext()).ToChecked();

    int* set = new int[n];
    for (int i = 0; i < n; i++) {
        v8::Local<v8::Value> element = jsArray->Get(args.GetIsolate()->GetCurrentContext(), i).ToLocalChecked();
        set[i] = element->Int32Value(args.GetIsolate()->GetCurrentContext()).ToChecked();
    }

    DYNAMIC_PROGRAMMING_SUBSET_SUM_PROBLEM_1* obj = 
        static_cast<DYNAMIC_PROGRAMMING_SUBSET_SUM_PROBLEM_1*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());

    bool result = obj->D_P_S_S_P_1_f_gold(set, n, sum);
    delete[] set; // Clean up allocated array memory
...
//assumed that 'isolate' and 'context' have been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, DYNAMIC_PROGRAMMING_SUBSET_SUM_PROBLEM_1_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "DYNAMIC_PROGRAMMING_SUBSET_SUM_PROBLEM_1").ToLocalChecked());

    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, D_P_S_S_P_1_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "D_P_S_S_P_1_f_gold", method_tpl);

    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "DYNAMIC_PROGRAMMING_SUBSET_SUM_PROBLEM_1").ToLocalChecked(), constructor);
