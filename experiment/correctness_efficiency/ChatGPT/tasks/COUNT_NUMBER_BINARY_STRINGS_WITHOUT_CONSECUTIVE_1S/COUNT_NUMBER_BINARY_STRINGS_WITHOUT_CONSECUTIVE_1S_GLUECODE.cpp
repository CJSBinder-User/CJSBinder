...
//
//[code snippet 1]
void COUNT_NUMBER_BINARY_STRINGS_WITHOUT_CONSECUTIVE_1S_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    COUNT_NUMBER_BINARY_STRINGS_WITHOUT_CONSECUTIVE_1S* obj = new COUNT_NUMBER_BINARY_STRINGS_WITHOUT_CONSECUTIVE_1S();
...
//
//[code snippet 2]
void COUNT_NUMBER_BINARY_STRINGS_WITHOUT_CONSECUTIVE_1S_C_N_B_S_W_C_1_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    COUNT_NUMBER_BINARY_STRINGS_WITHOUT_CONSECUTIVE_1S* obj = static_cast<COUNT_NUMBER_BINARY_STRINGS_WITHOUT_CONSECUTIVE_1S*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int n = args[0]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    int result = obj->C_N_B_S_W_C_1_f_gold(n);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, COUNT_NUMBER_BINARY_STRINGS_WITHOUT_CONSECUTIVE_1S_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "COUNT_NUMBER_BINARY_STRINGS_WITHOUT_CONSECUTIVE_1S").ToLocalChecked());
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, COUNT_NUMBER_BINARY_STRINGS_WITHOUT_CONSECUTIVE_1S_C_N_B_S_W_C_1_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "C_N_B_S_W_C_1_f_gold", method_tpl);
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "COUNT_NUMBER_BINARY_STRINGS_WITHOUT_CONSECUTIVE_1S").ToLocalChecked(), constructor);
