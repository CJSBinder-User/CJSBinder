...
//
//[code snippet 1]
void CEILING_IN_A_SORTED_ARRAY_1_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    CEILING_IN_A_SORTED_ARRAY_1* obj = new CEILING_IN_A_SORTED_ARRAY_1();
...
//
//[code snippet 2]
void CEILING_IN_A_SORTED_ARRAY_1_C_I_A_S_A_1_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    ...
    CEILING_IN_A_SORTED_ARRAY_1* obj = static_cast<CEILING_IN_A_SORTED_ARRAY_1*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    ...
    int arr[jsArr->Length()];
    for (uint32_t i = 0; i < jsArr->Length(); i++) {
        v8::MaybeLocal<v8::Value> val = jsArr->Get(args.GetIsolate()->GetCurrentContext(), i);
        if (val.IsEmpty() || !val.ToLocalChecked()->IsInt32()) {
            args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
            return;
        }
        arr[i] = val.ToLocalChecked()->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    }
    
    // Call the member function
    int result = obj->C_I_A_S_A_1_f_gold(arr, low, high, x);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, CEILING_IN_A_SORTED_ARRAY_1_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "CEILING_IN_A_SORTED_ARRAY_1").ToLocalChecked());
    
    // Register the method
    tpl->PrototypeTemplate()->Set(isolate, "C_I_A_S_A_1_f_gold", v8::FunctionTemplate::New(isolate, CEILING_IN_A_SORTED_ARRAY_1_C_I_A_S_A_1_f_gold));
    
    // Create the constructor function
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    
    // Register the constructor in the global context
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "CEILING_IN_A_SORTED_ARRAY_1").ToLocalChecked(), constructor);
