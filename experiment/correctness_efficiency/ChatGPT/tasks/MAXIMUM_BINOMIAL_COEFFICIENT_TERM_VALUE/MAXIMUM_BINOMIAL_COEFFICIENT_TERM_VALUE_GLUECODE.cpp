...
//
//[code snippet 1]
void MAXIMUM_BINOMIAL_COEFFICIENT_TERM_VALUE_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    MAXIMUM_BINOMIAL_COEFFICIENT_TERM_VALUE* obj = new MAXIMUM_BINOMIAL_COEFFICIENT_TERM_VALUE();
...
//
//[code snippet 2]
void MAXIMUM_BINOMIAL_COEFFICIENT_TERM_VALUE_M_B_C_T_V_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    MAXIMUM_BINOMIAL_COEFFICIENT_TERM_VALUE* obj = static_cast<MAXIMUM_BINOMIAL_COEFFICIENT_TERM_VALUE*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int n = args[0]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    int result = obj->M_B_C_T_V_f_gold(n);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, MAXIMUM_BINOMIAL_COEFFICIENT_TERM_VALUE_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "MAXIMUM_BINOMIAL_COEFFICIENT_TERM_VALUE").ToLocalChecked());
    
    tpl->PrototypeTemplate()->Set(isolate, "M_B_C_T_V_f_gold", v8::FunctionTemplate::New(isolate, MAXIMUM_BINOMIAL_COEFFICIENT_TERM_VALUE_M_B_C_T_V_f_gold));

    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "MAXIMUM_BINOMIAL_COEFFICIENT_TERM_VALUE").ToLocalChecked(), constructor);
