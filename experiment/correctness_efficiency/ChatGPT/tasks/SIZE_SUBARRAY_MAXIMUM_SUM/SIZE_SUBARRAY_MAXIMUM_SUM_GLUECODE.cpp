...
//
//[code snippet 1]
void SIZE_SUBARRAY_MAXIMUM_SUM_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    SIZE_SUBARRAY_MAXIMUM_SUM* obj = new SIZE_SUBARRAY_MAXIMUM_SUM();
...
//
//[code snippet 2]
void SIZE_SUBARRAY_MAXIMUM_SUM_S_S_M_S_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    ...
    int* data = new int[size]; // Dynamically allocated to hold array data

    for (int i = 0; i < size; ++i) {
        v8::Local<v8::Value> element = array->Get(args.GetIsolate()->GetCurrentContext(), i).ToLocalChecked();
        data[i] = element->IsInt32() ? element->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0) : 0;
    }

    SIZE_SUBARRAY_MAXIMUM_SUM* obj = static_cast<SIZE_SUBARRAY_MAXIMUM_SUM*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int result = obj->S_S_M_S_f_gold(data, size);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, SIZE_SUBARRAY_MAXIMUM_SUM_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "SIZE_SUBARRAY_MAXIMUM_SUM").ToLocalChecked());
    tpl->PrototypeTemplate()->Set(isolate, "S_S_M_S_f_gold", v8::FunctionTemplate::New(isolate, SIZE_SUBARRAY_MAXIMUM_SUM_S_S_M_S_f_gold));
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "SIZE_SUBARRAY_MAXIMUM_SUM").ToLocalChecked(), constructor);
