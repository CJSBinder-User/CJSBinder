...
//
//[code snippet 1]
void COUNT_ROTATIONS_DIVISIBLE_4_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    COUNT_ROTATIONS_DIVISIBLE_4* obj = new COUNT_ROTATIONS_DIVISIBLE_4();
...
//
//[code snippet 2]
void COUNT_ROTATIONS_DIVISIBLE_4_C_R_D_4_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsString()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    COUNT_ROTATIONS_DIVISIBLE_4* obj = static_cast<COUNT_ROTATIONS_DIVISIBLE_4*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    
    v8::String::Utf8Value str(args.GetIsolate(), args[0]);
    std::string n(*str); // Convert V8 string to std::string

    int result = obj->C_R_D_4_f_gold(n);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, COUNT_ROTATIONS_DIVISIBLE_4_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "COUNT_ROTATIONS_DIVISIBLE_4").ToLocalChecked());
    tpl->PrototypeTemplate()->Set(isolate, "C_R_D_4_f_gold", v8::FunctionTemplate::New(isolate, COUNT_ROTATIONS_DIVISIBLE_4_C_R_D_4_f_gold));
    
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "COUNT_ROTATIONS_DIVISIBLE_4").ToLocalChecked(), constructor);
