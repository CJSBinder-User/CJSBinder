...
//
//[code snippet 1]
void HARDY_RAMANUJAN_THEOREM_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    HARDY_RAMANUJAN_THEOREM* obj = new HARDY_RAMANUJAN_THEOREM();
...
//
//[code snippet 2]
void HARDY_RAMANUJAN_THEOREM_H_R_T_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    HARDY_RAMANUJAN_THEOREM* obj = static_cast<HARDY_RAMANUJAN_THEOREM*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int n = args[0]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    int result = obj->H_R_T_f_gold(n);
...
//assumed that 'isolate' and 'context' have been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, HARDY_RAMANUJAN_THEOREM_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "HARDY_RAMANUJAN_THEOREM").ToLocalChecked());
    tpl->PrototypeTemplate()->Set(isolate, "H_R_T_f_gold", v8::FunctionTemplate::New(isolate, HARDY_RAMANUJAN_THEOREM_H_R_T_f_gold));
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "HARDY_RAMANUJAN_THEOREM").ToLocalChecked(), constructor);
