...
//
//[code snippet 1]
void SMALLEST_SUM_CONTIGUOUS_SUBARRAY_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    SMALLEST_SUM_CONTIGUOUS_SUBARRAY* obj = new SMALLEST_SUM_CONTIGUOUS_SUBARRAY();
...
//
//[code snippet 2]
void SMALLEST_SUM_CONTIGUOUS_SUBARRAY_S_S_C_S_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 2 || !args[0]->IsArray() || !args[1]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }

    CClass* obj = static_cast<SMALLEST_SUM_CONTIGUOUS_SUBARRAY*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());

    // Retrieve the C++ array from the JavaScript array argument
    ...
    int result = obj->S_S_C_S_f_gold(arr, n);
    
    delete[] arr; // Free allocated memory for the array
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, SMALLEST_SUM_CONTIGUOUS_SUBARRAY_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "SMALLEST_SUM_CONTIGUOUS_SUBARRAY").ToLocalChecked());

    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, SMALLEST_SUM_CONTIGUOUS_SUBARRAY_S_S_C_S_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "S_S_C_S_f_gold", method_tpl);

    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "SMALLEST_SUM_CONTIGUOUS_SUBARRAY").ToLocalChecked(), constructor);
