...
//
//[code snippet 1]
void MAXIMUM_SUM_SUBARRAY_REMOVING_ONE_ELEMENT_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    MAXIMUM_SUM_SUBARRAY_REMOVING_ONE_ELEMENT* obj = new MAXIMUM_SUM_SUBARRAY_REMOVING_ONE_ELEMENT();
...
//
//[code snippet 2]
void MAXIMUM_SUM_SUBARRAY_REMOVING_ONE_ELEMENT_M_S_S_R_O_E_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 2 || !args[0]->IsArray() || !args[1]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }

    // Retrieve the array and its length
    ...
    for (int i = 0; i < n; i++) {
        ...
        c_arr[i] = element->IsInt32() ? element->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0) : 0;
    }

    // Call the C++ method
    MAXIMUM_SUM_SUBARRAY_REMOVING_ONE_ELEMENT* obj = static_cast<MAXIMUM_SUM_SUBARRAY_REMOVING_ONE_ELEMENT*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int result = obj->M_S_S_R_O_E_f_gold(c_arr, n);
...
//assumed that 'isolate' and 'context' have been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, MAXIMUM_SUM_SUBARRAY_REMOVING_ONE_ELEMENT_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "MAXIMUM_SUM_SUBARRAY_REMOVING_ONE_ELEMENT").ToLocalChecked());
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, MAXIMUM_SUM_SUBARRAY_REMOVING_ONE_ELEMENT_M_S_S_R_O_E_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "M_S_S_R_O_E_f_gold", method_tpl);
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "MAXIMUM_SUM_SUBARRAY_REMOVING_ONE_ELEMENT").ToLocalChecked(), constructor);
