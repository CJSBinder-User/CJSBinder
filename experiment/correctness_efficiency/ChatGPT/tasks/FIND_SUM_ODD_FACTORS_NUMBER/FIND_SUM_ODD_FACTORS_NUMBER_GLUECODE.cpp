...
//
//[code snippet 1]
void FIND_SUM_ODD_FACTORS_NUMBER_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    FIND_SUM_ODD_FACTORS_NUMBER* obj = new FIND_SUM_ODD_FACTORS_NUMBER();
...
//
//[code snippet 2]
void FIND_SUM_ODD_FACTORS_NUMBER_F_S_O_F_N_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    FIND_SUM_ODD_FACTORS_NUMBER* obj = static_cast<FIND_SUM_ODD_FACTORS_NUMBER*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int n = args[0]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    int result = obj->F_S_O_F_N_f_gold(n);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, FIND_SUM_ODD_FACTORS_NUMBER_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "FIND_SUM_ODD_FACTORS_NUMBER").ToLocalChecked());
    tpl->PrototypeTemplate()->Set(isolate, "F_S_O_F_N_f_gold", v8::FunctionTemplate::New(isolate, FIND_SUM_ODD_FACTORS_NUMBER_F_S_O_F_N_f_gold));
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "FIND_SUM_ODD_FACTORS_NUMBER").ToLocalChecked(), constructor);
