...
//
//[code snippet 1]
void HOW_TO_BEGIN_WITH_COMPETITIVE_PROGRAMMING_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    HOW_TO_BEGIN_WITH_COMPETITIVE_PROGRAMMING* obj = new HOW_TO_BEGIN_WITH_COMPETITIVE_PROGRAMMING();
...
//
//[code snippet 2]
void HOW_TO_BEGIN_WITH_COMPETITIVE_PROGRAMMING_H_T_B_W_C_P_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 3 || 
        !args[0]->IsArray() || 
        !args[1]->IsInt32() || 
        !args[2]->IsInt32()) {
    ...
    v8::Local<v8::Array> arrJs = v8::Local<v8::Array>::Cast(args[0]);
    ...
    int* arr = new int[n];
    for (int i = 0; i < n; i++) {
        arr[i] = arrJs->Get(args.GetIsolate()->GetCurrentContext(), i).ToLocalChecked()->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    }

    HOW_TO_BEGIN_WITH_COMPETITIVE_PROGRAMMING* obj = static_cast<HOW_TO_BEGIN_WITH_COMPETITIVE_PROGRAMMING*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int result = obj->H_T_B_W_C_P_f_gold(arr, n, x);
...
// assumed that 'isolate' and 'context' have been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, HOW_TO_BEGIN_WITH_COMPETITIVE_PROGRAMMING_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "HOW_TO_BEGIN_WITH_COMPETITIVE_PROGRAMMING").ToLocalChecked());
    
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, HOW_TO_BEGIN_WITH_COMPETITIVE_PROGRAMMING_H_T_B_W_C_P_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "H_T_B_W_C_P_f_gold", method_tpl);

    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "HOW_TO_BEGIN_WITH_COMPETITIVE_PROGRAMMING").ToLocalChecked(), constructor);
