...
//
//[code snippet 1]
void MINIMUM_XOR_VALUE_PAIR_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    MINIMUM_XOR_VALUE_PAIR* obj = new MINIMUM_XOR_VALUE_PAIR();
...
//
//[code snippet 2]
void MINIMUM_XOR_VALUE_PAIR_M_X_V_P_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 2 || !args[0]->IsArray() || !args[1]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }

    // Extracting the array from JavaScript arguments
    ...
    int* arr = new int[n]; // You can change memory management as needed
    ...
    MINIMUM_XOR_VALUE_PAIR* obj = static_cast<MINIMUM_XOR_VALUE_PAIR*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int result = obj->M_X_V_P_f_gold(arr, n);
...
// assumed that 'isolate' and 'context' have been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, MINIMUM_XOR_VALUE_PAIR_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "MINIMUM_XOR_VALUE_PAIR").ToLocalChecked());
    tpl->PrototypeTemplate()->Set(isolate, "M_X_V_P_f_gold", v8::FunctionTemplate::New(isolate, MINIMUM_XOR_VALUE_PAIR_M_X_V_P_f_gold));
    
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "MINIMUM_XOR_VALUE_PAIR").ToLocalChecked(), constructor);
