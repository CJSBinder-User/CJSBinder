...
//
//[code snippet 1]
void STRING_CONTAINING_FIRST_LETTER_EVERY_WORD_GIVEN_STRING_SPACES_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    STRING_CONTAINING_FIRST_LETTER_EVERY_WORD_GIVEN_STRING_SPACES* obj = new STRING_CONTAINING_FIRST_LETTER_EVERY_WORD_GIVEN_STRING_SPACES();
...
//
//[code snippet 2]
void STRING_CONTAINING_FIRST_LETTER_EVERY_WORD_GIVEN_STRING_SPACES_S_C_F_L_E_W_G_S_S_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsString()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    v8::String::Utf8Value str(args.GetIsolate(), args[0]->ToString(args.GetIsolate()->GetCurrentContext()).ToLocalChecked());
    STRING_CONTAINING_FIRST_LETTER_EVERY_WORD_GIVEN_STRING_SPACES* obj = 
        static_cast<STRING_CONTAINING_FIRST_LETTER_EVERY_WORD_GIVEN_STRING_SPACES*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    
    // Call the C++ method
    string result = obj->S_C_F_L_E_W_G_S_S_f_gold(*str);
...
// assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, STRING_CONTAINING_FIRST_LETTER_EVERY_WORD_GIVEN_STRING_SPACES_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "STRING_CONTAINING_FIRST_LETTER_EVERY_WORD_GIVEN_STRING_SPACES").ToLocalChecked());
    
    // Register the method
    tpl->PrototypeTemplate()->Set(isolate, 
        "S_C_F_L_E_W_G_S_S_f_gold", 
        v8::FunctionTemplate::New(isolate, STRING_CONTAINING_FIRST_LETTER_EVERY_WORD_GIVEN_STRING_SPACES_S_C_F_L_E_W_G_S_S_f_gold)
    );

    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, 
        v8::String::NewFromUtf8(isolate, "STRING_CONTAINING_FIRST_LETTER_EVERY_WORD_GIVEN_STRING_SPACES").ToLocalChecked(), 
        constructor
    ).FromMaybe(false);
