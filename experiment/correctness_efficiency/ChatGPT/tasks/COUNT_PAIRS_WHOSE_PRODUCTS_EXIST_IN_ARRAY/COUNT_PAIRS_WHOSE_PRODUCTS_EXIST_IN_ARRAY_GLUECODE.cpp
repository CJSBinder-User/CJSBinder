...
//
//[code snippet 1]
void COUNT_PAIRS_WHOSE_PRODUCTS_EXIST_IN_ARRAY_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    COUNT_PAIRS_WHOSE_PRODUCTS_EXIST_IN_ARRAY* obj = new COUNT_PAIRS_WHOSE_PRODUCTS_EXIST_IN_ARRAY();
...
//
//[code snippet 2]
void COUNT_PAIRS_WHOSE_PRODUCTS_EXIST_IN_ARRAY_C_P_W_P_E_I_A_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 2 || !args[0]->IsArray() || !args[1]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }

    // Helper to convert v8 Array to C++ Array
    ...
    COUNT_PAIRS_WHOSE_PRODUCTS_EXIST_IN_ARRAY* obj = 
        static_cast<COUNT_PAIRS_WHOSE_PRODUCTS_EXIST_IN_ARRAY*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    
    int result = obj->C_P_W_P_E_I_A_f_gold(arr, n);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, COUNT_PAIRS_WHOSE_PRODUCTS_EXIST_IN_ARRAY_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "COUNT_PAIRS_WHOSE_PRODUCTS_EXIST_IN_ARRAY").ToLocalChecked());
    
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, COUNT_PAIRS_WHOSE_PRODUCTS_EXIST_IN_ARRAY_C_P_W_P_E_I_A_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "C_P_W_P_E_I_A_f_gold", method_tpl);

    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "COUNT_PAIRS_WHOSE_PRODUCTS_EXIST_IN_ARRAY").ToLocalChecked(), constructor);
