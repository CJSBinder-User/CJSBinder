...
//
//[code snippet 1]
void COUNT_SUBARRAYS_EQUAL_NUMBER_1S_0S_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    COUNT_SUBARRAYS_EQUAL_NUMBER_1S_0S* obj = new COUNT_SUBARRAYS_EQUAL_NUMBER_1S_0S();
...
//
//[code snippet 2]
void COUNT_SUBARRAYS_EQUAL_NUMBER_1S_0S_C_S_E_N_1_0_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 2 || !args[0]->IsArray() || !args[1]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }

    // Convert V8 array to C++ int array
    ...
    for (uint32_t i = 0; i < arr->Length(); i++) {
        ...
        if (value->IsInt32()) {
            c_arr.push_back(value->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0));
        }
    }

    COUNT_SUBARRAYS_EQUAL_NUMBER_1S_0S* obj = static_cast<COUNT_SUBARRAYS_EQUAL_NUMBER_1S_0S*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int result = obj->C_S_E_N_1_0_f_gold(c_arr.data(), n);
...
//assumed that 'isolate' and 'context' have been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, COUNT_SUBARRAYS_EQUAL_NUMBER_1S_0S_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "COUNT_SUBARRAYS_EQUAL_NUMBER_1S_0S").ToLocalChecked());
    tpl->PrototypeTemplate()->Set(isolate, "C_S_E_N_1_0_f_gold", v8::FunctionTemplate::New(isolate, COUNT_SUBARRAYS_EQUAL_NUMBER_1S_0S_C_S_E_N_1_0_f_gold));
    
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "COUNT_SUBARRAYS_EQUAL_NUMBER_1S_0S").ToLocalChecked(), constructor);
