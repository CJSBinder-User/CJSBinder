...
//
//[code snippet 1]
void COUNT_STRINGS_ADJACENT_CHARACTERS_DIFFERENCE_ONE_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    COUNT_STRINGS_ADJACENT_CHARACTERS_DIFFERENCE_ONE* obj = new COUNT_STRINGS_ADJACENT_CHARACTERS_DIFFERENCE_ONE();
...
//
//[code snippet 2]
void COUNT_STRINGS_ADJACENT_CHARACTERS_DIFFERENCE_ONE_C_S_A_C_D_O_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    COUNT_STRINGS_ADJACENT_CHARACTERS_DIFFERENCE_ONE* obj = static_cast<COUNT_STRINGS_ADJACENT_CHARACTERS_DIFFERENCE_ONE*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int n = args[0]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    int result = obj->C_S_A_C_D_O_f_gold(n);
...
// assumed that 'isolate' and 'context' have been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, COUNT_STRINGS_ADJACENT_CHARACTERS_DIFFERENCE_ONE_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "COUNT_STRINGS_ADJACENT_CHARACTERS_DIFFERENCE_ONE").ToLocalChecked());
    
    // Register the method
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, COUNT_STRINGS_ADJACENT_CHARACTERS_DIFFERENCE_ONE_C_S_A_C_D_O_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "C_S_A_C_D_O_f_gold", method_tpl);
    
    // Create the constructor function
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "COUNT_STRINGS_ADJACENT_CHARACTERS_DIFFERENCE_ONE").ToLocalChecked(), constructor);
