...
//
//[code snippet 1]
void PRINT_WORDS_STRING_REVERSE_ORDER_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    PRINT_WORDS_STRING_REVERSE_ORDER* obj = new PRINT_WORDS_STRING_REVERSE_ORDER();
...
//
//[code snippet 2]
void PRINT_WORDS_STRING_REVERSE_ORDER_P_W_S_R_O_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsString()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    PRINT_WORDS_STRING_REVERSE_ORDER* obj = static_cast<PRINT_WORDS_STRING_REVERSE_ORDER*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    
    v8::String::Utf8Value str(args.GetIsolate(), args[0]);
    std::string result = obj->P_W_S_R_O_f_gold(*str);
...
// assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, PRINT_WORDS_STRING_REVERSE_ORDER_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "PRINT_WORDS_STRING_REVERSE_ORDER").ToLocalChecked());
    
    tpl->PrototypeTemplate()->Set(isolate, "P_W_S_R_O_f_gold", v8::FunctionTemplate::New(isolate, PRINT_WORDS_STRING_REVERSE_ORDER_P_W_S_R_O_f_gold));
    
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "PRINT_WORDS_STRING_REVERSE_ORDER").ToLocalChecked(), constructor);
