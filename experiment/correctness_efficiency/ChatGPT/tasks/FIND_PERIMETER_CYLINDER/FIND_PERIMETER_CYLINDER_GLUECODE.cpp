...
//
//[code snippet 1]
void FIND_PERIMETER_CYLINDER_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    FIND_PERIMETER_CYLINDER* obj = new FIND_PERIMETER_CYLINDER();
...
//
//[code snippet 2]
void FIND_PERIMETER_CYLINDER_F_P_C_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 2 || !args[0]->IsInt32() || !args[1]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    FIND_PERIMETER_CYLINDER* obj = static_cast<FIND_PERIMETER_CYLINDER*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int diameter = args[0]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    int height = args[1]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    int result = obj->F_P_C_f_gold(diameter, height);
...
//assumed that 'isolate' and 'context' have been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, FIND_PERIMETER_CYLINDER_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "FIND_PERIMETER_CYLINDER").ToLocalChecked());
    
    tpl->PrototypeTemplate()->Set(isolate, "F_P_C_f_gold", v8::FunctionTemplate::New(isolate, FIND_PERIMETER_CYLINDER_F_P_C_f_gold));
    
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "FIND_PERIMETER_CYLINDER").ToLocalChecked(), constructor);
