...
//
//[code snippet 1]
void CHECK_NUMBER_IS_PERFECT_SQUARE_USING_ADDITIONSUBTRACTION_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    CHECK_NUMBER_IS_PERFECT_SQUARE_USING_ADDITIONSUBTRACTION* obj = new CHECK_NUMBER_IS_PERFECT_SQUARE_USING_ADDITIONSUBTRACTION();
...
//
//[code snippet 2]
void CHECK_NUMBER_IS_PERFECT_SQUARE_USING_ADDITIONSUBTRACTION_C_N_I_P_S_U_A_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    CHECK_NUMBER_IS_PERFECT_SQUARE_USING_ADDITIONSUBTRACTION* obj = static_cast<CHECK_NUMBER_IS_PERFECT_SQUARE_USING_ADDITIONSUBTRACTION*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int n = args[0]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    bool result = obj->C_N_I_P_S_U_A_f_gold(n);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, CHECK_NUMBER_IS_PERFECT_SQUARE_USING_ADDITIONSUBTRACTION_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "CHECK_NUMBER_IS_PERFECT_SQUARE_USING_ADDITIONSUBTRACTION").ToLocalChecked());
    tpl->PrototypeTemplate()->Set(isolate, "C_N_I_P_S_U_A_f_gold", v8::FunctionTemplate::New(isolate, CHECK_NUMBER_IS_PERFECT_SQUARE_USING_ADDITIONSUBTRACTION_C_N_I_P_S_U_A_f_gold));
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "CHECK_NUMBER_IS_PERFECT_SQUARE_USING_ADDITIONSUBTRACTION").ToLocalChecked(), constructor);
