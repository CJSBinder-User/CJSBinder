...
//
//[code snippet 1]
void TILING_WITH_DOMINOES_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    TILING_WITH_DOMINOES* obj = new TILING_WITH_DOMINOES();
...
//
//[code snippet 2]
void TILING_WITH_DOMINOES_T_W_D_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    TILING_WITH_DOMINOES* obj = static_cast<TILING_WITH_DOMINOES*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int n = args[0]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    int result = obj->T_W_D_f_gold(n);
...
//assumed that 'isolate' and 'context' have been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, TILING_WITH_DOMINOES_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "TILING_WITH_DOMINOES").ToLocalChecked());
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, TILING_WITH_DOMINOES_T_W_D_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "T_W_D_f_gold", method_tpl);
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "TILING_WITH_DOMINOES").ToLocalChecked(), constructor);
