...
//
//[code snippet 1]
void MAXIMUM_CONSECUTIVE_NUMBERS_PRESENT_ARRAY_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    MAXIMUM_CONSECUTIVE_NUMBERS_PRESENT_ARRAY* obj = new MAXIMUM_CONSECUTIVE_NUMBERS_PRESENT_ARRAY();
...
//
//[code snippet 2]
void MAXIMUM_CONSECUTIVE_NUMBERS_PRESENT_ARRAY_M_C_N_P_A_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 2 || !args[0]->IsArray() || !args[1]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }

    // Extract the array from JavaScript and convert it to C++ array
    ...
    MAXIMUM_CONSECUTIVE_NUMBERS_PRESENT_ARRAY* obj = static_cast<MAXIMUM_CONSECUTIVE_NUMBERS_PRESENT_ARRAY*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int result = obj->M_C_N_P_A_f_gold(arr, n);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, MAXIMUM_CONSECUTIVE_NUMBERS_PRESENT_ARRAY_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "MAXIMUM_CONSECUTIVE_NUMBERS_PRESENT_ARRAY").ToLocalChecked());
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, MAXIMUM_CONSECUTIVE_NUMBERS_PRESENT_ARRAY_M_C_N_P_A_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "M_C_N_P_A_f_gold", method_tpl);
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "MAXIMUM_CONSECUTIVE_NUMBERS_PRESENT_ARRAY").ToLocalChecked(), constructor);
