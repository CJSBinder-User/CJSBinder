...
//
//[code snippet 1]
void SQUARE_PYRAMIDAL_NUMBER_SUM_SQUARES_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    SQUARE_PYRAMIDAL_NUMBER_SUM_SQUARES* obj = new SQUARE_PYRAMIDAL_NUMBER_SUM_SQUARES();
...
//
//[code snippet 2]
void SQUARE_PYRAMIDAL_NUMBER_SUM_SQUARES_S_P_N_S_S_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    SQUARE_PYRAMIDAL_NUMBER_SUM_SQUARES* obj = static_cast<SQUARE_PYRAMIDAL_NUMBER_SUM_SQUARES*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int s = args[0]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    int result = obj->S_P_N_S_S_f_gold(s);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, SQUARE_PYRAMIDAL_NUMBER_SUM_SQUARES_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "SQUARE_PYRAMIDAL_NUMBER_SUM_SQUARES").ToLocalChecked());
    
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, SQUARE_PYRAMIDAL_NUMBER_SUM_SQUARES_S_P_N_S_S_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "S_P_N_S_S_f_gold", method_tpl);
    
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "SQUARE_PYRAMIDAL_NUMBER_SUM_SQUARES").ToLocalChecked(), constructor);
