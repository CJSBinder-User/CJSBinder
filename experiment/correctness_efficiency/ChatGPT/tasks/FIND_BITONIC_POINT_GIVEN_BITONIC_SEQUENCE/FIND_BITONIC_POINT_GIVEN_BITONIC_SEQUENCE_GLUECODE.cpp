...
//
//[code snippet 1]
void FIND_BITONIC_POINT_GIVEN_BITONIC_SEQUENCE_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    FIND_BITONIC_POINT_GIVEN_BITONIC_SEQUENCE* obj = new FIND_BITONIC_POINT_GIVEN_BITONIC_SEQUENCE();
...
//
//[code snippet 2]
void FIND_BITONIC_POINT_GIVEN_BITONIC_SEQUENCE_F_B_P_G_B_S_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 3 || !args[0]->IsArray() || !args[1]->IsInt32() || !args[2]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }

    FIND_BITONIC_POINT_GIVEN_BITONIC_SEQUENCE* obj = static_cast<FIND_BITONIC_POINT_GIVEN_BITONIC_SEQUENCE*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    ...
    int left = args[1]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    int right = args[2]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);

    int result = obj->F_B_P_G_B_S_f_gold(arr, left, right);
    args.GetReturnValue().Set(v8::Integer::New(args.GetIsolate(), result));

    delete[] arr; // Cleanup allocated memory
}
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, FIND_BITONIC_POINT_GIVEN_BITONIC_SEQUENCE_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "FIND_BITONIC_POINT_GIVEN_BITONIC_SEQUENCE").ToLocalChecked());
    tpl->PrototypeTemplate()->Set(isolate, "F_B_P_G_B_S_f_gold", v8::FunctionTemplate::New(isolate, FIND_BITONIC_POINT_GIVEN_BITONIC_SEQUENCE_F_B_P_G_B_S_f_gold));
    
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "FIND_BITONIC_POINT_GIVEN_BITONIC_SEQUENCE").ToLocalChecked(), constructor);
