...
//
//[code snippet 1]
void NON_REPEATING_ELEMENT_1_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    NON_REPEATING_ELEMENT_1* obj = new NON_REPEATING_ELEMENT_1();
...
//
//[code snippet 2]
void NON_REPEATING_ELEMENT_1_N_R_E_1_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 2 || !args[0]->IsArray() || !args[1]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }

    // Convert the input JavaScript array to C++ int array
    ...
    NON_REPEATING_ELEMENT_1* obj = static_cast<NON_REPEATING_ELEMENT_1*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int result = obj->N_R_E_1_f_gold(arr, n);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, NON_REPEATING_ELEMENT_1_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "NON_REPEATING_ELEMENT_1").ToLocalChecked());
    
    tpl->PrototypeTemplate()->Set(isolate, "N_R_E_1_f_gold", v8::FunctionTemplate::New(isolate, NON_REPEATING_ELEMENT_1_N_R_E_1_f_gold));
    
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "NON_REPEATING_ELEMENT_1").ToLocalChecked(), constructor);
