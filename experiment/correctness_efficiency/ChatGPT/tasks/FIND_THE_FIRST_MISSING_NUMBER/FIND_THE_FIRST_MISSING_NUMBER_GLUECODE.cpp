...
//
//[code snippet 1]
void FIND_THE_FIRST_MISSING_NUMBER_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    FIND_THE_FIRST_MISSING_NUMBER* obj = new FIND_THE_FIRST_MISSING_NUMBER();
...
//
//[code snippet 2]
void FIND_THE_FIRST_MISSING_NUMBER_F_T_F_M_N_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 3 || 
        !args[0]->IsArray() || 
        !args[1]->IsInt32() || 
        !args[2]->IsInt32()) {
    ...
    FIND_THE_FIRST_MISSING_NUMBER* obj = static_cast<FIND_THE_FIRST_MISSING_NUMBER*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());

    v8::Local<v8::Array> jsArray = v8::Local<v8::Array>::Cast(args[0]);
    int start = args[1]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    int end = args[2]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);

    // Convert V8 array to native C++ array
    int* array = new int[jsArray->Length()];
    for (unsigned int i = 0; i < jsArray->Length(); i++) {
        array[i] = jsArray->Get(args.GetIsolate()->GetCurrentContext(), i).ToLocalChecked()->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    }

    int result = obj->F_T_F_M_N_f_gold(array, start, end);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, FIND_THE_FIRST_MISSING_NUMBER_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "FIND_THE_FIRST_MISSING_NUMBER").ToLocalChecked());
    tpl->PrototypeTemplate()->Set(isolate, "F_T_F_M_N_f_gold", v8::FunctionTemplate::New(isolate, FIND_THE_FIRST_MISSING_NUMBER_F_T_F_M_N_f_gold));

    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "FIND_THE_FIRST_MISSING_NUMBER").ToLocalChecked(), constructor);
