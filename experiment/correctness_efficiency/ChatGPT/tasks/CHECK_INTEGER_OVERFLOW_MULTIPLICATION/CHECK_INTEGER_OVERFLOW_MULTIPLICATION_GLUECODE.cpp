...
//
//[code snippet 1]
void CHECK_INTEGER_OVERFLOW_MULTIPLICATION_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    CHECK_INTEGER_OVERFLOW_MULTIPLICATION* obj = new CHECK_INTEGER_OVERFLOW_MULTIPLICATION();
...
//
//[code snippet 2]
void CHECK_INTEGER_OVERFLOW_MULTIPLICATION_C_I_O_M_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 2 || 
        !args[0]->IsBigInt() || 
        !args[1]->IsBigInt()) {
    ...
    CHECK_INTEGER_OVERFLOW_MULTIPLICATION* obj = static_cast<CHECK_INTEGER_OVERFLOW_MULTIPLICATION*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    
    long long a = args[0]->ToBigInt(args.GetIsolate()->GetCurrentContext()).ToLocalChecked()->Int64Value();
    long long b = args[1]->ToBigInt(args.GetIsolate()->GetCurrentContext()).ToLocalChecked()->Int64Value();
    
    bool result = obj->C_I_O_M_f_gold(a, b);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, CHECK_INTEGER_OVERFLOW_MULTIPLICATION_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "CHECK_INTEGER_OVERFLOW_MULTIPLICATION").ToLocalChecked());
    
    tpl->PrototypeTemplate()->Set(isolate, "C_I_O_M_f_gold", v8::FunctionTemplate::New(isolate, CHECK_INTEGER_OVERFLOW_MULTIPLICATION_C_I_O_M_f_gold));
    
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "CHECK_INTEGER_OVERFLOW_MULTIPLICATION").ToLocalChecked(), constructor);
