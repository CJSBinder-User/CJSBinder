...
//
//[code snippet 1]
void TEMPLE_OFFERINGS_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    TEMPLE_OFFERINGS* obj = new TEMPLE_OFFERINGS();
...
//
//[code snippet 2]
void TEMPLE_OFFERINGS_T_O_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 2 || !args[0]->IsInt32() || !args[1]->IsArray()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }

    TEMPLE_OFFERINGS* obj = static_cast<TEMPLE_OFFERINGS*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int n = args[0]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    
    v8::Local<v8::Array> templeHeightArray = v8::Local<v8::Array>::Cast(args[1]);
    int* templeHeight = new int[n];
    for (int i = 0; i < n; i++) {
        v8::Local<v8::Value> elem = templeHeightArray->Get(args.GetIsolate()->GetCurrentContext(), i).ToLocalChecked();
        templeHeight[i] = elem->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    }

    int result = obj->T_O_f_gold(n, templeHeight);
    delete[] templeHeight; // Clean up after using the array
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, TEMPLE_OFFERINGS_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "TEMPLE_OFFERINGS").ToLocalChecked());
    
    tpl->PrototypeTemplate()->Set(isolate, "T_O_f_gold", v8::FunctionTemplate::New(isolate, TEMPLE_OFFERINGS_T_O_f_gold));
    
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "TEMPLE_OFFERINGS").ToLocalChecked(), constructor);
