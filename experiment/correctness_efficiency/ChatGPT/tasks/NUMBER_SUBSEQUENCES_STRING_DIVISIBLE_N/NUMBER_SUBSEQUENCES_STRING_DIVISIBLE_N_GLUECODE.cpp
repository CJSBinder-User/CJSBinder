...
//
//[code snippet 1]
void NUMBER_SUBSEQUENCES_STRING_DIVISIBLE_N_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    NUMBER_SUBSEQUENCES_STRING_DIVISIBLE_N* obj = new NUMBER_SUBSEQUENCES_STRING_DIVISIBLE_N();
...
//
//[code snippet 2]
void NUMBER_SUBSEQUENCES_STRING_DIVISIBLE_N_N_S_S_D_N_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 2 || !args[0]->IsString() || !args[1]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    
    // Convert the first argument to a C++ string
    v8::String::Utf8Value utf8String(args.GetIsolate(), args[0]->ToString(args.GetIsolate()->GetCurrentContext()).ToLocalChecked());
    std::string str(*utf8String);
    
    // Retrieve the integer parameter
    NUMBER_SUBSEQUENCES_STRING_DIVISIBLE_N* obj = static_cast<NUMBER_SUBSEQUENCES_STRING_DIVISIBLE_N*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int n = args[1]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    
    // Call the method and return the result
    int result = obj->N_S_S_D_N_f_gold(str, n);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, NUMBER_SUBSEQUENCES_STRING_DIVISIBLE_N_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "NUMBER_SUBSEQUENCES_STRING_DIVISIBLE_N").ToLocalChecked());
    
    // Register the method
    tpl->PrototypeTemplate()->Set(isolate, "N_S_S_D_N_f_gold", v8::FunctionTemplate::New(isolate, NUMBER_SUBSEQUENCES_STRING_DIVISIBLE_N_N_S_S_D_N_f_gold));
    
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "NUMBER_SUBSEQUENCES_STRING_DIVISIBLE_N").ToLocalChecked(), constructor);
