...
//
//[code snippet 1]
void COST_BALANCE_PARANTHESES_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    COST_BALANCE_PARANTHESES* obj = new COST_BALANCE_PARANTHESES();
...
//
//[code snippet 2]
void COST_BALANCE_PARANTHESES_C_B_P_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsString()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }

    // Convert the argument from a V8 string to a C++ string
    v8::String::Utf8Value str(args.GetIsolate(), args[0]);
    std::string s(*str);

    COST_BALANCE_PARANTHESES* obj = static_cast<COST_BALANCE_PARANTHESES*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int result = obj->C_B_P_f_gold(s);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, COST_BALANCE_PARANTHESES_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "COST_BALANCE_PARANTHESES").ToLocalChecked());
    tpl->PrototypeTemplate()->Set(isolate, "C_B_P_f_gold", v8::FunctionTemplate::New(isolate, COST_BALANCE_PARANTHESES_C_B_P_f_gold));
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "COST_BALANCE_PARANTHESES").ToLocalChecked(), constructor);
