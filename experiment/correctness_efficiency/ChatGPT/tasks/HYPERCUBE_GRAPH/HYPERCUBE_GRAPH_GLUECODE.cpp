...
//
//[code snippet 1]
void HYPERCUBE_GRAPH_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    HYPERCUBE_GRAPH* obj = new HYPERCUBE_GRAPH();
...
//
//[code snippet 2]
void HYPERCUBE_GRAPH_H_G_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    HYPERCUBE_GRAPH* obj = static_cast<HYPERCUBE_GRAPH*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int n = args[0]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    int result = obj->H_G_f_gold(n);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, HYPERCUBE_GRAPH_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "HYPERCUBE_GRAPH").ToLocalChecked());
    tpl->PrototypeTemplate()->Set(isolate, "H_G_f_gold", v8::FunctionTemplate::New(isolate, HYPERCUBE_GRAPH_H_G_f_gold));
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "HYPERCUBE_GRAPH").ToLocalChecked(), constructor);
