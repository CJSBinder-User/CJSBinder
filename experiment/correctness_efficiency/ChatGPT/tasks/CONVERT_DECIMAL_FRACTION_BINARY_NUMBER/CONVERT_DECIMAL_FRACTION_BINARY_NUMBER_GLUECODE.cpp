...
//
//[code snippet 1]
void CONVERT_DECIMAL_FRACTION_BINARY_NUMBER_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    CONVERT_DECIMAL_FRACTION_BINARY_NUMBER* obj = new CONVERT_DECIMAL_FRACTION_BINARY_NUMBER();
...
//
//[code snippet 2]
void CONVERT_DECIMAL_FRACTION_BINARY_NUMBER_C_D_F_B_N_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 2 || !args[0]->IsNumber() || !args[1]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    
    CONVERT_DECIMAL_FRACTION_BINARY_NUMBER* obj = static_cast<CONVERT_DECIMAL_FRACTION_BINARY_NUMBER*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    double num = args[0]->NumberValue(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    int k_prec = args[1]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    
    std::string result = obj->C_D_F_B_N_f_gold(num, k_prec);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, CONVERT_DECIMAL_FRACTION_BINARY_NUMBER_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "CONVERT_DECIMAL_FRACTION_BINARY_NUMBER").ToLocalChecked());
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, CONVERT_DECIMAL_FRACTION_BINARY_NUMBER_C_D_F_B_N_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "C_D_F_B_N_f_gold", method_tpl);
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "CONVERT_DECIMAL_FRACTION_BINARY_NUMBER").ToLocalChecked(), constructor);
