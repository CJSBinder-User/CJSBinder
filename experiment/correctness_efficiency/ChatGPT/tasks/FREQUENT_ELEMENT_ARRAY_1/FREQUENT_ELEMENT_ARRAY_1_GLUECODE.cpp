...
//
//[code snippet 1]
void FREQUENT_ELEMENT_ARRAY_1_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    FREQUENT_ELEMENT_ARRAY_1* obj = new FREQUENT_ELEMENT_ARRAY_1();
...
//
//[code snippet 2]
void FREQUENT_ELEMENT_ARRAY_1_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 2 || !args[0]->IsArray() || !args[1]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    
    // Retrieve the array from the JavaScript side
    ...
    // Convert the JS array to a C++ array
    int* arr = new int[n];
    for (int i = 0; i < n; i++) {
        v8::Local<v8::Value> val = jsArray->Get(args.GetIsolate()->GetCurrentContext(), i).ToLocalChecked();
        arr[i] = val->IsInt32() ? val->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0) : 0;
    }

    FREQUENT_ELEMENT_ARRAY_1* obj = static_cast<FREQUENT_ELEMENT_ARRAY_1*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int result = obj->F_E_A_1_f_gold(arr, n);

    delete[] arr;  // Clean up to prevent memory leaks
...
// assumed that 'isolate' and 'context' have been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, FREQUENT_ELEMENT_ARRAY_1_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "FREQUENT_ELEMENT_ARRAY_1").ToLocalChecked());
    tpl->PrototypeTemplate()->Set(isolate, "F_E_A_1_f_gold", v8::FunctionTemplate::New(isolate, FREQUENT_ELEMENT_ARRAY_1_f_gold));
    
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "FREQUENT_ELEMENT_ARRAY_1").ToLocalChecked(), constructor);
