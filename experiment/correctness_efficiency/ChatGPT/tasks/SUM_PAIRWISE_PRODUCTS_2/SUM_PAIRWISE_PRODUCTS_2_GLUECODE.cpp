...
//
//[code snippet 1]
void SUM_PAIRWISE_PRODUCTS_2_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    SUM_PAIRWISE_PRODUCTS_2* obj = new SUM_PAIRWISE_PRODUCTS_2();
...
//
//[code snippet 2]
void SUM_PAIRWISE_PRODUCTS_2_S_P_P_2_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    SUM_PAIRWISE_PRODUCTS_2* obj = static_cast<SUM_PAIRWISE_PRODUCTS_2*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int n = args[0]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    long long result = obj->S_P_P_2_f_gold(n);
    args.GetReturnValue().Set(v8::Number::New(args.GetIsolate(), result)); // long long as Number
}
...
// assumed that 'isolate' and 'context' have been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, SUM_PAIRWISE_PRODUCTS_2_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "SUM_PAIRWISE_PRODUCTS_2").ToLocalChecked());
    
    tpl->PrototypeTemplate()->Set(isolate, "S_P_P_2_f_gold", v8::FunctionTemplate::New(isolate, SUM_PAIRWISE_PRODUCTS_2_S_P_P_2_f_gold));

    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "SUM_PAIRWISE_PRODUCTS_2").ToLocalChecked(), constructor);
