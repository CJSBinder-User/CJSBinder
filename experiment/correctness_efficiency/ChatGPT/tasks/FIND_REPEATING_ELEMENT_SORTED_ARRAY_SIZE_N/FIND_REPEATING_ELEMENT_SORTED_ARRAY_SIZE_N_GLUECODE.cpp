...
//
//[code snippet 1]
void FIND_REPEATING_ELEMENT_SORTED_ARRAY_SIZE_N_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    FIND_REPEATING_ELEMENT_SORTED_ARRAY_SIZE_N* obj = new FIND_REPEATING_ELEMENT_SORTED_ARRAY_SIZE_N();
...
//
//[code snippet 2]
void FIND_REPEATING_ELEMENT_SORTED_ARRAY_SIZE_N_f_R_E_S_A_S_N_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 3 || 
        !args[0]->IsArray() || 
        !args[1]->IsInt32() || 
        !args[2]->IsInt32()) {
    ...
    FIND_REPEATING_ELEMENT_SORTED_ARRAY_SIZE_N* obj = static_cast<FIND_REPEATING_ELEMENT_SORTED_ARRAY_SIZE_N*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int result = obj->F_R_E_S_A_S_N_f_gold(arr, low, high);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, FIND_REPEATING_ELEMENT_SORTED_ARRAY_SIZE_N_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "FIND_REPEATING_ELEMENT_SORTED_ARRAY_SIZE_N").ToLocalChecked());
    
    // Register the method
    tpl->PrototypeTemplate()->Set(isolate, "F_R_E_S_A_S_N_f_gold", v8::FunctionTemplate::New(isolate, FIND_REPEATING_ELEMENT_SORTED_ARRAY_SIZE_N_f_R_E_S_A_S_N_f_gold));

    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "FIND_REPEATING_ELEMENT_SORTED_ARRAY_SIZE_N").ToLocalChecked(), constructor);
