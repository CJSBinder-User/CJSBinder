...
//
//[code snippet 1]
void K_TH_DISTINCT_OR_NON_REPEATING_ELEMENT_IN_AN_ARRAY_1_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    K_TH_DISTINCT_OR_NON_REPEATING_ELEMENT_IN_AN_ARRAY_1* obj = new K_TH_DISTINCT_OR_NON_REPEATING_ELEMENT_IN_AN_ARRAY_1();
...
//
//[code snippet 2]
void K_T_D_O_N_R_E_I_A_A_1_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (maybeValue.IsEmpty() || !maybeValue.ToLocalChecked()->IsInt32()) {
            delete[] arr;  // Clean up to avoid memory leak
            args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
            return;
        }
        arr[i] = maybeValue.ToLocalChecked()->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    }

    K_TH_DISTINCT_OR_NON_REPEATING_ELEMENT_IN_AN_ARRAY_1* obj = static_cast<K_TH_DISTINCT_OR_NON_REPEATING_ELEMENT_IN_AN_ARRAY_1*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int result = obj->K_T_D_O_N_R_E_I_A_A_1_f_gold(arr, n, k);
...
//assumed that 'isolate' and 'context' have been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, K_TH_DISTINCT_OR_NON_REPEATING_ELEMENT_IN_AN_ARRAY_1_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "K_TH_DISTINCT_OR_NON_REPEATING_ELEMENT_IN_AN_ARRAY_1").ToLocalChecked());
    
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, K_T_D_O_N_R_E_I_A_A_1_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "K_T_D_O_N_R_E_I_A_A_1_f_gold", method_tpl);
    
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "K_TH_DISTINCT_OR_NON_REPEATING_ELEMENT_IN_AN_ARRAY_1").ToLocalChecked(), constructor);
