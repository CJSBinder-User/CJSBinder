...
//
//[code snippet 1]
void FIND_PAIR_MAXIMUM_GCD_ARRAY_1_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    FIND_PAIR_MAXIMUM_GCD_ARRAY_1* obj = new FIND_PAIR_MAXIMUM_GCD_ARRAY_1();
...
//
//[code snippet 2]
void FIND_PAIR_MAXIMUM_GCD_ARRAY_1_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        cpp_arr[i] = value->IsInt32() ? value->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0) : 0;
    }

    FIND_PAIR_MAXIMUM_GCD_ARRAY_1* obj = static_cast<FIND_PAIR_MAXIMUM_GCD_ARRAY_1*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int result = obj->F_P_M_G_A_1_f_gold(cpp_arr, n);
    
    args.GetReturnValue().Set(v8::Integer::New(args.GetIsolate(), result));
    delete[] cpp_arr; // Clean up allocated memory
}
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, FIND_PAIR_MAXIMUM_GCD_ARRAY_1_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "FIND_PAIR_MAXIMUM_GCD_ARRAY_1").ToLocalChecked());
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, FIND_PAIR_MAXIMUM_GCD_ARRAY_1_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "F_P_M_G_A_1_f_gold", method_tpl);
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "FIND_PAIR_MAXIMUM_GCD_ARRAY_1").ToLocalChecked(), constructor);
