...
//
//[code snippet 1]
void PROGRAM_CHECK_INPUT_INTEGER_STRING_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    PROGRAM_CHECK_INPUT_INTEGER_STRING* obj = new PROGRAM_CHECK_INPUT_INTEGER_STRING();
...
//
//[code snippet 2]
void PROGRAM_CHECK_INPUT_INTEGER_STRING_P_C_I_I_S_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsString()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    
    v8::String::Utf8Value utf8(args.GetIsolate(), args[0]->ToString(args.GetIsolate()->GetCurrentContext()).ToLocalChecked());
    std::string s(*utf8);
    
    PROGRAM_CHECK_INPUT_INTEGER_STRING* obj = static_cast<PROGRAM_CHECK_INPUT_INTEGER_STRING*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    bool result = obj->P_C_I_I_S_f_gold(s);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, PROGRAM_CHECK_INPUT_INTEGER_STRING_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "PROGRAM_CHECK_INPUT_INTEGER_STRING").ToLocalChecked());

    // Set up the prototype method
    tpl->PrototypeTemplate()->Set(isolate, "P_C_I_I_S_f_gold", v8::FunctionTemplate::New(isolate, PROGRAM_CHECK_INPUT_INTEGER_STRING_P_C_I_I_S_f_gold));

    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "PROGRAM_CHECK_INPUT_INTEGER_STRING").ToLocalChecked(), constructor);
