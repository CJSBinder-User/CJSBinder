...
//
//[code snippet 1]
void HOW_TO_COMPUTE_MOD_OF_A_BIG_NUMBER_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    HOW_TO_COMPUTE_MOD_OF_A_BIG_NUMBER* obj = new HOW_TO_COMPUTE_MOD_OF_A_BIG_NUMBER();
...
//
//[code snippet 2]
void H_T_C_M_O_A_B_N_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 2 || !args[0]->IsString() || !args[1]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    
    HOW_TO_COMPUTE_MOD_OF_A_BIG_NUMBER* obj = static_cast<HOW_TO_COMPUTE_MOD_OF_A_BIG_NUMBER*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());

    // Convert input string argument to std::string
    v8::String::Utf8Value utf8_string(args.GetIsolate(), args[0]);
    std::string num = std::string(*utf8_string);

    int a = args[1]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);

    int result = obj->H_T_C_M_O_A_B_N_f_gold(num, a);
...
// assumed that 'isolate' and 'context' have been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, HOW_TO_COMPUTE_MOD_OF_A_BIG_NUMBER_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "HOW_TO_COMPUTE_MOD_OF_A_BIG_NUMBER").ToLocalChecked());
    
    tpl->PrototypeTemplate()->Set(isolate, "H_T_C_M_O_A_B_N_f_gold", v8::FunctionTemplate::New(isolate, H_T_C_M_O_A_B_N_f_gold));
    
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "HOW_TO_COMPUTE_MOD_OF_A_BIG_NUMBER").ToLocalChecked(), constructor);
