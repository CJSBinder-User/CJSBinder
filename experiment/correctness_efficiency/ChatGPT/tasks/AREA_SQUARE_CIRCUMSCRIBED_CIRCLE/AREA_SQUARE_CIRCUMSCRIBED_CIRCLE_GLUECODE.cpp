...
//
//[code snippet 1]
void AREA_SQUARE_CIRCUMSCRIBED_CIRCLE_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    AREA_SQUARE_CIRCUMSCRIBED_CIRCLE* obj = new AREA_SQUARE_CIRCUMSCRIBED_CIRCLE();
...
//
//[code snippet 2]
void AREA_SQUARE_CIRCUMSCRIBED_CIRCLE_A_S_C_C_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    AREA_SQUARE_CIRCUMSCRIBED_CIRCLE* obj = static_cast<AREA_SQUARE_CIRCUMSCRIBED_CIRCLE*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int r = args[0]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    int result = obj->A_S_C_C_f_gold(r);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, AREA_SQUARE_CIRCUMSCRIBED_CIRCLE_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "AREA_SQUARE_CIRCUMSCRIBED_CIRCLE").ToLocalChecked());
    tpl->PrototypeTemplate()->Set(isolate, "A_S_C_C_f_gold", v8::FunctionTemplate::New(isolate, AREA_SQUARE_CIRCUMSCRIBED_CIRCLE_A_S_C_C_f_gold));
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "AREA_SQUARE_CIRCUMSCRIBED_CIRCLE").ToLocalChecked(), constructor);
