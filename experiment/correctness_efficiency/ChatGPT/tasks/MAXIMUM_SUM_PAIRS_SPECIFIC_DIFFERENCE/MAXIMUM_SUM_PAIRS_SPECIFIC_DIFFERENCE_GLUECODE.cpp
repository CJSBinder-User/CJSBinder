...
//
//[code snippet 1]
void MAXIMUM_SUM_PAIRS_SPECIFIC_DIFFERENCE_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    MAXIMUM_SUM_PAIRS_SPECIFIC_DIFFERENCE* obj = new MAXIMUM_SUM_PAIRS_SPECIFIC_DIFFERENCE();
...
//
//[code snippet 2]
void MAXIMUM_SUM_PAIRS_SPECIFIC_DIFFERENCE_M_S_P_S_D_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 3 || 
        !args[0]->IsArray() || 
        !args[1]->IsInt32() || 
        !args[2]->IsInt32()) {
    ...
    int* arr = new int[N]; // Create a C++ array
    for (int i = 0; i < N; ++i) {
    ...
    MAXIMUM_SUM_PAIRS_SPECIFIC_DIFFERENCE* obj = static_cast<MAXIMUM_SUM_PAIRS_SPECIFIC_DIFFERENCE*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int result = obj->M_S_P_S_D_f_gold(arr, N, K);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, MAXIMUM_SUM_PAIRS_SPECIFIC_DIFFERENCE_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "MAXIMUM_SUM_PAIRS_SPECIFIC_DIFFERENCE").ToLocalChecked());

    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, MAXIMUM_SUM_PAIRS_SPECIFIC_DIFFERENCE_M_S_P_S_D_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "M_S_P_S_D_f_gold", method_tpl);

    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "MAXIMUM_SUM_PAIRS_SPECIFIC_DIFFERENCE").ToLocalChecked(), constructor);
