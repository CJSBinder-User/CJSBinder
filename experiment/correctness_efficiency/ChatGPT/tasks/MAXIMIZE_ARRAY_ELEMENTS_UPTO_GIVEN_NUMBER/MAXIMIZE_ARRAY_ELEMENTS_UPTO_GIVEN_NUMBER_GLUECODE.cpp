...
//
//[code snippet 1]
void MAXIMIZE_ARRAY_ELEMENTS_UPTO_GIVEN_NUMBER_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    MAXIMIZE_ARRAY_ELEMENTS_UPTO_GIVEN_NUMBER* obj = new MAXIMIZE_ARRAY_ELEMENTS_UPTO_GIVEN_NUMBER();
...
//
//[code snippet 2]
void MAXIMIZE_ARRAY_ELEMENTS_UPTO_GIVEN_NUMBER_M_A_E_U_G_N_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 4 || !args[0]->IsArray() || !args[1]->IsInt32() || !args[2]->IsInt32() || !args[3]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    MAXIMIZE_ARRAY_ELEMENTS_UPTO_GIVEN_NUMBER* obj = static_cast<MAXIMIZE_ARRAY_ELEMENTS_UPTO_GIVEN_NUMBER*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    
    // Convert V8 array to C++ array
    v8::Local<v8::Array> array = v8::Local<v8::Array>::Cast(args[0]);
    int n = args[1]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    int num = args[2]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    int maxLimit = args[3]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    
    int* arr = new int[n];
    for (int i = 0; i < n; i++) {
        arr[i] = array->Get(args.GetIsolate()->GetCurrentContext(), i).ToLocalChecked()->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    }
    
    int result = obj->M_A_E_U_G_N_f_gold(arr, n, num, maxLimit);
...
//[code snippet 3]
    ...
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, MAXIMIZE_ARRAY_ELEMENTS_UPTO_GIVEN_NUMBER_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "MAXIMIZE_ARRAY_ELEMENTS_UPTO_GIVEN_NUMBER").ToLocalChecked());
    
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, MAXIMIZE_ARRAY_ELEMENTS_UPTO_GIVEN_NUMBER_M_A_E_U_G_N_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "M_A_E_U_G_N_f_gold", method_tpl);
    
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "MAXIMIZE_ARRAY_ELEMENTS_UPTO_GIVEN_NUMBER").ToLocalChecked(), constructor);
