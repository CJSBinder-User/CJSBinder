...
//
//[code snippet 1]
void FIND_SUBARRAY_WITH_GIVEN_SUM_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    FIND_SUBARRAY_WITH_GIVEN_SUM* obj = new FIND_SUBARRAY_WITH_GIVEN_SUM();
...
//
//[code snippet 2]
void FIND_SUBARRAY_WITH_GIVEN_SUM_F_S_W_G_S_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 3 || !args[0]->IsArray() || !args[1]->IsInt32() || !args[2]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }

    // Extract the array from the JS arguments
    ...
    // Create the C++ object and invoke the method
    FIND_SUBARRAY_WITH_GIVEN_SUM* obj = static_cast<FIND_SUBARRAY_WITH_GIVEN_SUM*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int result = obj->F_S_W_G_S_f_gold(arr, n, sum);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, FIND_SUBARRAY_WITH_GIVEN_SUM_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "FIND_SUBARRAY_WITH_GIVEN_SUM").ToLocalChecked());
    tpl->PrototypeTemplate()->Set(isolate, "F_S_W_G_S_f_gold", v8::FunctionTemplate::New(isolate, FIND_SUBARRAY_WITH_GIVEN_SUM_F_S_W_G_S_f_gold));
    
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "FIND_SUBARRAY_WITH_GIVEN_SUM").ToLocalChecked(), constructor);
