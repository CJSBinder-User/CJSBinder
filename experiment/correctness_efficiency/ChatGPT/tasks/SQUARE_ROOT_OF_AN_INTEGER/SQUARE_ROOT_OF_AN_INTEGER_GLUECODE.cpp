...
//
//[code snippet 1]
void SQUARE_ROOT_OF_AN_INTEGER_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    SQUARE_ROOT_OF_AN_INTEGER* obj = new SQUARE_ROOT_OF_AN_INTEGER();
...
//
//[code snippet 2]
void SQUARE_ROOT_OF_AN_INTEGER_S_R_O_A_I_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    SQUARE_ROOT_OF_AN_INTEGER* obj = static_cast<SQUARE_ROOT_OF_AN_INTEGER*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int x = args[0]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    int result = obj->S_R_O_A_I_f_gold(x);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, SQUARE_ROOT_OF_AN_INTEGER_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "SQUARE_ROOT_OF_AN_INTEGER").ToLocalChecked());
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, SQUARE_ROOT_OF_AN_INTEGER_S_R_O_A_I_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "S_R_O_A_I_f_gold", method_tpl);
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "SQUARE_ROOT_OF_AN_INTEGER").ToLocalChecked(), constructor);
