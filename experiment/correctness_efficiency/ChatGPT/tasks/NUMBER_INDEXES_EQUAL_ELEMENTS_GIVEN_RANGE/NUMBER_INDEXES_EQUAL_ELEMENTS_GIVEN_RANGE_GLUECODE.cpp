...
//
//[code snippet 1]
void NUMBER_INDEXES_EQUAL_ELEMENTS_GIVEN_RANGE_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    NUMBER_INDEXES_EQUAL_ELEMENTS_GIVEN_RANGE* obj = new NUMBER_INDEXES_EQUAL_ELEMENTS_GIVEN_RANGE();
...
//
//[code snippet 2]
void NUMBER_INDEXES_EQUAL_ELEMENTS_GIVEN_RANGE_N_I_E_E_G_R_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 4 
        || !args[0]->IsArray() 
        || !args[1]->IsInt32() 
        || !args[2]->IsInt32() 
    ...
    CClass* obj = static_cast<NUMBER_INDEXES_EQUAL_ELEMENTS_GIVEN_RANGE*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());

    // 1. Convert the array from JavaScript to C++
    ...
    int l = args[2]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    int r = args[3]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);

    // 2. Prepare the C++ array (assuming it can hold n elements)
    int* a = new int[n];
    for (int i = 0; i < n; i++) {
    ...
    // 3. Call the C++ method
    int result = obj->N_I_E_E_G_R_f_gold(a, n, l, r);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, NUMBER_INDEXES_EQUAL_ELEMENTS_GIVEN_RANGE_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "NUMBER_INDEXES_EQUAL_ELEMENTS_GIVEN_RANGE").ToLocalChecked());
    
    // Wrap the method
    tpl->PrototypeTemplate()->Set(isolate, "N_I_E_E_G_R_f_gold", v8::FunctionTemplate::New(isolate, NUMBER_INDEXES_EQUAL_ELEMENTS_GIVEN_RANGE_N_I_E_E_G_R_f_gold));
    
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "NUMBER_INDEXES_EQUAL_ELEMENTS_GIVEN_RANGE").ToLocalChecked(), constructor);
