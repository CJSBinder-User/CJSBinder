...
//
//[code snippet 1]
void DIVIDE_LARGE_NUMBER_REPRESENTED_STRING_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    DIVIDE_LARGE_NUMBER_REPRESENTED_STRING* obj = new DIVIDE_LARGE_NUMBER_REPRESENTED_STRING();
...
//
//[code snippet 2]
void DIVIDE_LARGE_NUMBER_REPRESENTED_STRING_D_L_N_R_S_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    ...
    // Extract arguments
    v8::String::Utf8Value utf8_number(args.GetIsolate(), args[0]);
    std::string number(*utf8_number);
    int divisor = args[1]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);

    // Retrieve the C++ object
    DIVIDE_LARGE_NUMBER_REPRESENTED_STRING* obj = static_cast<DIVIDE_LARGE_NUMBER_REPRESENTED_STRING*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());

    // Call the C++ method
    std::string result = obj->D_L_N_R_S_f_gold(number, divisor);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, DIVIDE_LARGE_NUMBER_REPRESENTED_STRING_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "DIVIDE_LARGE_NUMBER_REPRESENTED_STRING").ToLocalChecked());
    
    // Register the method
    tpl->PrototypeTemplate()->Set(isolate, "D_L_N_R_S_f_gold", v8::FunctionTemplate::New(isolate, DIVIDE_LARGE_NUMBER_REPRESENTED_STRING_D_L_N_R_S_f_gold));

    // Create the constructor and register it in the global context
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "DIVIDE_LARGE_NUMBER_REPRESENTED_STRING").ToLocalChecked(), constructor);
