...
//
//[code snippet 1]
void COUNTING_PAIRS_PERSON_CAN_FORM_PAIR_ONE_1_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    COUNTING_PAIRS_PERSON_CAN_FORM_PAIR_ONE_1* obj = new COUNTING_PAIRS_PERSON_CAN_FORM_PAIR_ONE_1();
...
//
//[code snippet 2]
void COUNTING_PAIRS_PERSON_CAN_FORM_PAIR_ONE_1_C_P_P_C_F_P_O_1_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    COUNTING_PAIRS_PERSON_CAN_FORM_PAIR_ONE_1* obj = static_cast<COUNTING_PAIRS_PERSON_CAN_FORM_PAIR_ONE_1*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int x = args[0]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    int result = obj->C_P_P_C_F_P_O_1_f_gold(x);
...
//assumed that 'isolate' and 'context' have been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, COUNTING_PAIRS_PERSON_CAN_FORM_PAIR_ONE_1_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "COUNTING_PAIRS_PERSON_CAN_FORM_PAIR_ONE_1").ToLocalChecked());
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, COUNTING_PAIRS_PERSON_CAN_FORM_PAIR_ONE_1_C_P_P_C_F_P_O_1_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "C_P_P_C_F_P_O_1_f_gold", method_tpl);
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "COUNTING_PAIRS_PERSON_CAN_FORM_PAIR_ONE_1").ToLocalChecked(), constructor);
