...
//
//[code snippet 1]
void MINIMUM_XOR_VALUE_PAIR_1_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    MINIMUM_XOR_VALUE_PAIR_1* obj = new MINIMUM_XOR_VALUE_PAIR_1();
...
//
//[code snippet 2]
void MINIMUM_XOR_VALUE_PAIR_1_M_X_V_P_1_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 2 || !args[0]->IsArray() || !args[1]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }

    // Retrieve the array from the JavaScript context
    v8::Local<v8::Array> arr = v8::Local<v8::Array>::Cast(args[0]);
    int n = args[1]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);

    // Prepare a C++ array to hold the values
    int* c_arr = new int[n];
    for (int i = 0; i < n; i++) {
    ...
    MINIMUM_XOR_VALUE_PAIR_1* obj = static_cast<MINIMUM_XOR_VALUE_PAIR_1*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int result = obj->M_X_V_P_1_f_gold(c_arr, n);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, MINIMUM_XOR_VALUE_PAIR_1_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "MINIMUM_XOR_VALUE_PAIR_1").ToLocalChecked());
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, MINIMUM_XOR_VALUE_PAIR_1_M_X_V_P_1_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "M_X_V_P_1_f_gold", method_tpl);
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "MINIMUM_XOR_VALUE_PAIR_1").ToLocalChecked(), constructor);
