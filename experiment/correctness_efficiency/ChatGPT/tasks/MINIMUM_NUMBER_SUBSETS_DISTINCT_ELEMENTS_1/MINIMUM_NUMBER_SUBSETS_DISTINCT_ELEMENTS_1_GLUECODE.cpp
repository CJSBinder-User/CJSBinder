...
//
//[code snippet 1]
void MINIMUM_NUMBER_SUBSETS_DISTINCT_ELEMENTS_1_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    MINIMUM_NUMBER_SUBSETS_DISTINCT_ELEMENTS_1* obj = new MINIMUM_NUMBER_SUBSETS_DISTINCT_ELEMENTS_1();
...
//
//[code snippet 2]
void MINIMUM_NUMBER_SUBSETS_DISTINCT_ELEMENTS_1_M_N_S_D_E_1_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    ...
    int* c_arr = new int[n];
    for (int i = 0; i < n; ++i) {
        v8::Local<v8::Value> elem = arr->Get(args.GetIsolate()->GetCurrentContext(), i).ToLocalChecked();
        c_arr[i] = elem->IsInt32() ? elem->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0) : 0;
    }

    MINIMUM_NUMBER_SUBSETS_DISTINCT_ELEMENTS_1* obj = static_cast<MINIMUM_NUMBER_SUBSETS_DISTINCT_ELEMENTS_1*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int result = obj->M_N_S_D_E_1_f_gold(c_arr, n);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, MINIMUM_NUMBER_SUBSETS_DISTINCT_ELEMENTS_1_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "MINIMUM_NUMBER_SUBSETS_DISTINCT_ELEMENTS_1").ToLocalChecked());
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, MINIMUM_NUMBER_SUBSETS_DISTINCT_ELEMENTS_1_M_N_S_D_E_1_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "M_N_S_D_E_1_f_gold", method_tpl);
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "MINIMUM_NUMBER_SUBSETS_DISTINCT_ELEMENTS_1").ToLocalChecked(), constructor);
