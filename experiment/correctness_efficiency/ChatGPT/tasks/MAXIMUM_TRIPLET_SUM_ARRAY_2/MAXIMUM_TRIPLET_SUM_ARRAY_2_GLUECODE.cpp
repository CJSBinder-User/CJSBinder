...
//
//[code snippet 1]
void MAXIMUM_TRIPLET_SUM_ARRAY_2_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    MAXIMUM_TRIPLET_SUM_ARRAY_2* obj = new MAXIMUM_TRIPLET_SUM_ARRAY_2();
...
//
//[code snippet 2]
void MAXIMUM_TRIPLET_SUM_ARRAY_2_M_T_S_A_2_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 2 || !args[0]->IsArray() || !args[1]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }

    // Retrieve the array from the arguments
    ...
    // Call the C++ method
    MAXIMUM_TRIPLET_SUM_ARRAY_2* obj = static_cast<MAXIMUM_TRIPLET_SUM_ARRAY_2*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int result = obj->M_T_S_A_2_f_gold(arr, n);
...
// assumed that 'isolate' and 'context' have been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, MAXIMUM_TRIPLET_SUM_ARRAY_2_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "MAXIMUM_TRIPLET_SUM_ARRAY_2").ToLocalChecked());
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, MAXIMUM_TRIPLET_SUM_ARRAY_2_M_T_S_A_2_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "M_T_S_A_2_f_gold", method_tpl);
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "MAXIMUM_TRIPLET_SUM_ARRAY_2").ToLocalChecked(), constructor);
