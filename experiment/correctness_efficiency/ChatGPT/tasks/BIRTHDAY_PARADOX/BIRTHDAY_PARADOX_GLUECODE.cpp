...
//
//[code snippet 1]
void BIRTHDAY_PARADOX_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    BIRTHDAY_PARADOX* obj = new BIRTHDAY_PARADOX();
...
//
//[code snippet 2]
void BIRTHDAY_PARADOX_B_P_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsNumber()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    BIRTHDAY_PARADOX* obj = static_cast<BIRTHDAY_PARADOX*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    double p = args[0]->NumberValue(args.GetIsolate()->GetCurrentContext()).FromMaybe(0.0);
    int result = obj->B_P_f_gold(p);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, BIRTHDAY_PARADOX_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "BIRTHDAY_PARADOX").ToLocalChecked());
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, BIRTHDAY_PARADOX_B_P_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "B_P_f_gold", method_tpl);
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "BIRTHDAY_PARADOX").ToLocalChecked(), constructor);
