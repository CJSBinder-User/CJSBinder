...
//
//[code snippet 1]
void MAXIMUM_DISTANCE_TWO_OCCURRENCES_ELEMENT_ARRAY_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    MAXIMUM_DISTANCE_TWO_OCCURRENCES_ELEMENT_ARRAY* obj = new MAXIMUM_DISTANCE_TWO_OCCURRENCES_ELEMENT_ARRAY();
...
//
//[code snippet 2]
void MAXIMUM_DISTANCE_TWO_OCCURRENCES_ELEMENT_ARRAY_M_D_T_O_E_A_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 2 || !args[0]->IsArray() || !args[1]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    
    MAXIMUM_DISTANCE_TWO_OCCURRENCES_ELEMENT_ARRAY* obj = static_cast<MAXIMUM_DISTANCE_TWO_OCCURRENCES_ELEMENT_ARRAY*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    ...
    int* arr = new int[n];
    for (int i = 0; i < n; ++i) {
        v8::Local<v8::Value> value = js_array->Get(args.GetIsolate()->GetCurrentContext(), i).ToLocalChecked();
        arr[i] = value->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    }
    
    int result = obj->M_D_T_O_E_A_f_gold(arr, n);
...
//Assumed that 'isolate' and 'context' have been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, MAXIMUM_DISTANCE_TWO_OCCURRENCES_ELEMENT_ARRAY_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "MAXIMUM_DISTANCE_TWO_OCCURRENCES_ELEMENT_ARRAY").ToLocalChecked());
    
    tpl->PrototypeTemplate()->Set(isolate, "M_D_T_O_E_A_f_gold", v8::FunctionTemplate::New(isolate, MAXIMUM_DISTANCE_TWO_OCCURRENCES_ELEMENT_ARRAY_M_D_T_O_E_A_f_gold));
    
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "MAXIMUM_DISTANCE_TWO_OCCURRENCES_ELEMENT_ARRAY").ToLocalChecked(), constructor);
