...
//
//[code snippet 1]
void POSITION_OF_RIGHTMOST_SET_BIT_1_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    POSITION_OF_RIGHTMOST_SET_BIT_1* obj = new POSITION_OF_RIGHTMOST_SET_BIT_1();
...
//
//[code snippet 2]
void POSITION_OF_RIGHTMOST_SET_BIT_1_P_O_R_S_B_1_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    POSITION_OF_RIGHTMOST_SET_BIT_1* obj = static_cast<POSITION_OF_RIGHTMOST_SET_BIT_1*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int n = args[0]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    int result = obj->P_O_R_S_B_1_f_gold(n);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, POSITION_OF_RIGHTMOST_SET_BIT_1_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "POSITION_OF_RIGHTMOST_SET_BIT_1").ToLocalChecked());
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, POSITION_OF_RIGHTMOST_SET_BIT_1_P_O_R_S_B_1_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "P_O_R_S_B_1_f_gold", method_tpl);
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "POSITION_OF_RIGHTMOST_SET_BIT_1").ToLocalChecked(), constructor);
