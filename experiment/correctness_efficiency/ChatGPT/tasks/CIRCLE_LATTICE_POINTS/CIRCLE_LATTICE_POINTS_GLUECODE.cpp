...
//
//[code snippet 1]
void CIRCLE_LATTICE_POINTS_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    CIRCLE_LATTICE_POINTS* obj = new CIRCLE_LATTICE_POINTS();
...
//
//[code snippet 2]
void CIRCLE_LATTICE_POINTS_C_L_P_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    CIRCLE_LATTICE_POINTS* obj = static_cast<CIRCLE_LATTICE_POINTS*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int r = args[0]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    int result = obj->C_L_P_f_gold(r);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, CIRCLE_LATTICE_POINTS_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "CIRCLE_LATTICE_POINTS").ToLocalChecked());

    tpl->PrototypeTemplate()->Set(isolate, "C_L_P_f_gold", v8::FunctionTemplate::New(isolate, CIRCLE_LATTICE_POINTS_C_L_P_f_gold));
    
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "CIRCLE_LATTICE_POINTS").ToLocalChecked(), constructor);
