...
//
//[code snippet 1]
void MAXIMUM_PRODUCT_SUBSET_ARRAY_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    MAXIMUM_PRODUCT_SUBSET_ARRAY* obj = new MAXIMUM_PRODUCT_SUBSET_ARRAY();
...
//
//[code snippet 2]
void MAXIMUM_PRODUCT_SUBSET_ARRAY_M_P_S_A_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 2 || !args[0]->IsArray() || !args[1]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    MAXIMUM_PRODUCT_SUBSET_ARRAY* obj = static_cast<MAXIMUM_PRODUCT_SUBSET_ARRAY*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    ...
    for (int i = 0; i < n; i++) {
        ...
        a[i] = element->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0); // Convert JS number to C++ int
    }

    int result = obj->M_P_S_A_f_gold(a.data(), n);
...
//assumed that 'isolate' and 'context' have been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, MAXIMUM_PRODUCT_SUBSET_ARRAY_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "MAXIMUM_PRODUCT_SUBSET_ARRAY").ToLocalChecked());
    
    tpl->PrototypeTemplate()->Set(isolate, "M_P_S_A_f_gold", v8::FunctionTemplate::New(isolate, MAXIMUM_PRODUCT_SUBSET_ARRAY_M_P_S_A_f_gold));

    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "MAXIMUM_PRODUCT_SUBSET_ARRAY").ToLocalChecked(), constructor);
