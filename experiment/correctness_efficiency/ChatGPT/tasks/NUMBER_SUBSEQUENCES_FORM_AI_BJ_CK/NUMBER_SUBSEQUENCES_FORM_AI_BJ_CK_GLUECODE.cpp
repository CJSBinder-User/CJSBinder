...
//
//[code snippet 1]
void NUMBER_SUBSEQUENCES_FORM_AI_BJ_CK_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    NUMBER_SUBSEQUENCES_FORM_AI_BJ_CK* obj = new NUMBER_SUBSEQUENCES_FORM_AI_BJ_CK();
...
//
//[code snippet 2]
void NUMBER_SUBSEQUENCES_FORM_AI_BJ_CK_N_S_F_A_B_C_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsString()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    CStyleString s = *v8::String::Utf8Value(args.GetIsolate(), args[0]);
    NUMBER_SUBSEQUENCES_FORM_AI_BJ_CK* obj = static_cast<NUMBER_SUBSEQUENCES_FORM_AI_BJ_CK*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int result = obj->N_S_F_A_B_C_f_gold(s);
...
//assumed that 'isolate' and 'context' have been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, NUMBER_SUBSEQUENCES_FORM_AI_BJ_CK_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "NUMBER_SUBSEQUENCES_FORM_AI_BJ_CK").ToLocalChecked());
    tpl->PrototypeTemplate()->Set(isolate, "N_S_F_A_B_C_f_gold", v8::FunctionTemplate::New(isolate, NUMBER_SUBSEQUENCES_FORM_AI_BJ_CK_N_S_F_A_B_C_f_gold));
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "NUMBER_SUBSEQUENCES_FORM_AI_BJ_CK").ToLocalChecked(), constructor);
