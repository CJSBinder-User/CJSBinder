...
//
//[code snippet 1]
void MAXIMIZE_VOLUME_CUBOID_GIVEN_SUM_SIDES_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    MAXIMIZE_VOLUME_CUBOID_GIVEN_SUM_SIDES* obj = new MAXIMIZE_VOLUME_CUBOID_GIVEN_SUM_SIDES();
...
//
//[code snippet 2]
void MAXIMIZE_VOLUME_CUBOID_GIVEN_SUM_SIDES_M_V_C_G_S_S_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    MAXIMIZE_VOLUME_CUBOID_GIVEN_SUM_SIDES* obj = static_cast<MAXIMIZE_VOLUME_CUBOID_GIVEN_SUM_SIDES*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int s = args[0]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    int result = obj->M_V_C_G_S_S_f_gold(s);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, MAXIMIZE_VOLUME_CUBOID_GIVEN_SUM_SIDES_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "MAXIMIZE_VOLUME_CUBOID_GIVEN_SUM_SIDES").ToLocalChecked());
    
    tpl->PrototypeTemplate()->Set(isolate, "M_V_C_G_S_S_f_gold", v8::FunctionTemplate::New(isolate, MAXIMIZE_VOLUME_CUBOID_GIVEN_SUM_SIDES_M_V_C_G_S_S_f_gold));
    
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "MAXIMIZE_VOLUME_CUBOID_GIVEN_SUM_SIDES").ToLocalChecked(), constructor);
