...
//
//[code snippet 1]
void DECODE_MEDIAN_STRING_ORIGINAL_STRING_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    DECODE_MEDIAN_STRING_ORIGINAL_STRING* obj = new DECODE_MEDIAN_STRING_ORIGINAL_STRING();
...
//
//[code snippet 2]
void DECODE_MEDIAN_STRING_ORIGINAL_STRING_D_M_S_O_S_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsString()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }

    // Convert the first argument to std::string
    v8::String::Utf8Value utf8_value(args.GetIsolate(), args[0]);
    std::string s(*utf8_value);

    DECODE_MEDIAN_STRING_ORIGINAL_STRING* obj = static_cast<DECODE_MEDIAN_STRING_ORIGINAL_STRING*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    std::string result = obj->D_M_S_O_S_f_gold(s);
    
    // Convert the result back to a JavaScript string
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, DECODE_MEDIAN_STRING_ORIGINAL_STRING_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "DECODE_MEDIAN_STRING_ORIGINAL_STRING").ToLocalChecked());
    
    // Register the method
    tpl->PrototypeTemplate()->Set(isolate, "D_M_S_O_S_f_gold", v8::FunctionTemplate::New(isolate, DECODE_MEDIAN_STRING_ORIGINAL_STRING_D_M_S_O_S_f_gold));

    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "DECODE_MEDIAN_STRING_ORIGINAL_STRING").ToLocalChecked(), constructor);
