...
//
//[code snippet 1]
void K_TH_LARGEST_SUM_CONTIGUOUS_SUBARRAY_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    K_TH_LARGEST_SUM_CONTIGUOUS_SUBARRAY* obj = new K_TH_LARGEST_SUM_CONTIGUOUS_SUBARRAY();
...
//
//[code snippet 2]
void K_T_L_S_C_S_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 3 || !args[0]->IsArray() || !args[1]->IsInt32() || !args[2]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }

    K_TH_LARGEST_SUM_CONTIGUOUS_SUBARRAY* obj = static_cast<K_TH_LARGEST_SUM_CONTIGUOUS_SUBARRAY*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    ...
    int* arr = new int[n];
    for (int i = 0; i < n; ++i) {
        arr[i] = js_array->Get(args.GetIsolate()->GetCurrentContext(), i).ToLocalChecked()->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    }

    int result = obj->K_T_L_S_C_S_f_gold(arr, n, k);
    delete[] arr;  // Clean up
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, K_TH_LARGEST_SUM_CONTIGUOUS_SUBARRAY_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "K_TH_LARGEST_SUM_CONTIGUOUS_SUBARRAY").ToLocalChecked());

    // Register the method
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, K_T_L_S_C_S_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "K_T_L_S_C_S_f_gold", method_tpl);

    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "K_TH_LARGEST_SUM_CONTIGUOUS_SUBARRAY").ToLocalChecked(), constructor);
