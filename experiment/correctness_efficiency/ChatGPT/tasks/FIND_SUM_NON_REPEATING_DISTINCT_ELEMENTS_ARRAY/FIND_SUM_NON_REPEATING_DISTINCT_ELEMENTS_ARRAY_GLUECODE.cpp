...
//
//[code snippet 1]
void FIND_SUM_NON_REPEATING_DISTINCT_ELEMENTS_ARRAY_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    FIND_SUM_NON_REPEATING_DISTINCT_ELEMENTS_ARRAY* obj = new FIND_SUM_NON_REPEATING_DISTINCT_ELEMENTS_ARRAY();
...
//
//[code snippet 2]
void FIND_SUM_NON_REPEATING_DISTINCT_ELEMENTS_ARRAY_F_S_N_R_D_E_A_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        arr[i] = value->IsInt32() ? value->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0) : 0; // Default to 0 if not an integer
    }

    FIND_SUM_NON_REPEATING_DISTINCT_ELEMENTS_ARRAY* obj = static_cast<FIND_SUM_NON_REPEATING_DISTINCT_ELEMENTS_ARRAY*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int result = obj->F_S_N_R_D_E_A_f_gold(arr, n);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, FIND_SUM_NON_REPEATING_DISTINCT_ELEMENTS_ARRAY_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "FIND_SUM_NON_REPEATING_DISTINCT_ELEMENTS_ARRAY").ToLocalChecked());
    
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, FIND_SUM_NON_REPEATING_DISTINCT_ELEMENTS_ARRAY_F_S_N_R_D_E_A_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "F_S_N_R_D_E_A_f_gold", method_tpl);
    
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "FIND_SUM_NON_REPEATING_DISTINCT_ELEMENTS_ARRAY").ToLocalChecked(), constructor);
