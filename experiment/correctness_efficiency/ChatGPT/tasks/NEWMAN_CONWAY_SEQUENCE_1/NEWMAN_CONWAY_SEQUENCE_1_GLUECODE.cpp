...
//
//[code snippet 1]
void NEWMAN_CONWAY_SEQUENCE_1_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    NEWMAN_CONWAY_SEQUENCE_1* obj = new NEWMAN_CONWAY_SEQUENCE_1();
...
//
//[code snippet 2]
void NEWMAN_CONWAY_SEQUENCE_1_N_C_S_1_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    NEWMAN_CONWAY_SEQUENCE_1* obj = static_cast<NEWMAN_CONWAY_SEQUENCE_1*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int n = args[0]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    int result = obj->N_C_S_1_f_gold(n);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, NEWMAN_CONWAY_SEQUENCE_1_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "NEWMAN_CONWAY_SEQUENCE_1").ToLocalChecked());
    
    // Add the method to the prototype
    tpl->PrototypeTemplate()->Set(isolate, "N_C_S_1_f_gold", v8::FunctionTemplate::New(isolate, NEWMAN_CONWAY_SEQUENCE_1_N_C_S_1_f_gold));
    
    // Create the constructor
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    
    // Register the constructor in the global context
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "NEWMAN_CONWAY_SEQUENCE_1").ToLocalChecked(), constructor);
