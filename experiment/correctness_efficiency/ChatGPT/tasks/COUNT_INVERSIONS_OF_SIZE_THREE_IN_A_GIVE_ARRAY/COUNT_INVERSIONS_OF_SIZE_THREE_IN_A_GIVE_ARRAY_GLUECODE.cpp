...
//
//[code snippet 1]
void COUNT_INVERSIONS_OF_SIZE_THREE_IN_A_GIVE_ARRAY_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    COUNT_INVERSIONS_OF_SIZE_THREE_IN_A_GIVE_ARRAY* obj = new COUNT_INVERSIONS_OF_SIZE_THREE_IN_A_GIVE_ARRAY();
...
//
//[code snippet 2]
void COUNT_INVERSIONS_OF_SIZE_THREE_IN_A_GIVE_ARRAY_C_I_O_S_T_I_A_G_A_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        arr[i] = element->IsInt32() ? element->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0) : 0; // Default to 0 if not an integer
    }

    COUNT_INVERSIONS_OF_SIZE_THREE_IN_A_GIVE_ARRAY* obj = static_cast<COUNT_INVERSIONS_OF_SIZE_THREE_IN_A_GIVE_ARRAY*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int result = obj->C_I_O_S_T_I_A_G_A_f_gold(arr, n);
...
// assumed that 'isolate' and 'context' have been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, COUNT_INVERSIONS_OF_SIZE_THREE_IN_A_GIVE_ARRAY_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "COUNT_INVERSIONS_OF_SIZE_THREE_IN_A_GIVE_ARRAY").ToLocalChecked());
    
    // Register method
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, COUNT_INVERSIONS_OF_SIZE_THREE_IN_A_GIVE_ARRAY_C_I_O_S_T_I_A_G_A_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "C_I_O_S_T_I_A_G_A_f_gold", method_tpl);
    
    // Create constructor
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "COUNT_INVERSIONS_OF_SIZE_THREE_IN_A_GIVE_ARRAY").ToLocalChecked(), constructor);
