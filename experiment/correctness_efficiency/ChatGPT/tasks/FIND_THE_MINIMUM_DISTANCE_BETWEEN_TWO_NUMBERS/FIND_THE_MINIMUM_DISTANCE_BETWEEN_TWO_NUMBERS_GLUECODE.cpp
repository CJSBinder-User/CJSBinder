...
//
//[code snippet 1]
void FIND_THE_MINIMUM_DISTANCE_BETWEEN_TWO_NUMBERS_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    FIND_THE_MINIMUM_DISTANCE_BETWEEN_TWO_NUMBERS* obj = new FIND_THE_MINIMUM_DISTANCE_BETWEEN_TWO_NUMBERS();
...
//
//[code snippet 2]
void FIND_THE_MINIMUM_DISTANCE_BETWEEN_TWO_NUMBERS_F_T_M_D_B_T_N_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 4 
        || !args[1]->IsInt32() 
        || !args[2]->IsInt32() 
        || !args[3]->IsInt32Array()) {
    ...
    FIND_THE_MINIMUM_DISTANCE_BETWEEN_TWO_NUMBERS* obj = static_cast<FIND_THE_MINIMUM_DISTANCE_BETWEEN_TWO_NUMBERS*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    ...
    v8::Local<v8::Int32Array> arr = v8::Local<v8::Int32Array>::Cast(args[3]);
    int* c_arr = new int[n];

    // Copy the content of the Int32Array to a C++ array
    for (int i = 0; i < n; ++i) {
    ...
    int result = obj->F_T_M_D_B_T_N_f_gold(c_arr, n, x, y);
    delete[] c_arr; // Clean up the allocated array
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, FIND_THE_MINIMUM_DISTANCE_BETWEEN_TWO_NUMBERS_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "FIND_THE_MINIMUM_DISTANCE_BETWEEN_TWO_NUMBERS").ToLocalChecked());
    
    // Bind the method
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, FIND_THE_MINIMUM_DISTANCE_BETWEEN_TWO_NUMBERS_F_T_M_D_B_T_N_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "F_T_M_D_B_T_N_f_gold", method_tpl);
    
    // Create the constructor function
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "FIND_THE_MINIMUM_DISTANCE_BETWEEN_TWO_NUMBERS").ToLocalChecked(), constructor);
