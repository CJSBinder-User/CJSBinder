...
//
//[code snippet 1]
void CALCULATING_FACTORIALS_USING_STIRLING_APPROXIMATION_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    CALCULATING_FACTORIALS_USING_STIRLING_APPROXIMATION* obj = new CALCULATING_FACTORIALS_USING_STIRLING_APPROXIMATION();
...
//
//[code snippet 2]
void CALCULATING_FACTORIALS_USING_STIRLING_APPROXIMATION_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    CALCULATING_FACTORIALS_USING_STIRLING_APPROXIMATION* obj = static_cast<CALCULATING_FACTORIALS_USING_STIRLING_APPROXIMATION*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int n = args[0]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    long result = obj->C_F_U_S_A_f_gold(n);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, CALCULATING_FACTORIALS_USING_STIRLING_APPROXIMATION_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "CALCULATING_FACTORIALS_USING_STIRLING_APPROXIMATION").ToLocalChecked());
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, CALCULATING_FACTORIALS_USING_STIRLING_APPROXIMATION_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "C_F_U_S_A_f_gold", method_tpl);
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "CALCULATING_FACTORIALS_USING_STIRLING_APPROXIMATION").ToLocalChecked(), constructor);
