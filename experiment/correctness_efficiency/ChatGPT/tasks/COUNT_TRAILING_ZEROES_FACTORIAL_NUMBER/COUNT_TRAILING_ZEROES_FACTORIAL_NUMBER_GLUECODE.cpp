...
//
//[code snippet 1]
void COUNT_TRAILING_ZEROES_FACTORIAL_NUMBER_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    COUNT_TRAILING_ZEROES_FACTORIAL_NUMBER* obj = new COUNT_TRAILING_ZEROES_FACTORIAL_NUMBER();
...
//
//[code snippet 2]
void COUNT_TRAILING_ZEROES_FACTORIAL_NUMBER_C_T_Z_F_N_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    COUNT_TRAILING_ZEROES_FACTORIAL_NUMBER* obj = static_cast<COUNT_TRAILING_ZEROES_FACTORIAL_NUMBER*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int n = args[0]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    int result = obj->C_T_Z_F_N_f_gold(n);
...
//assumed that 'isolate' and 'context' have been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, COUNT_TRAILING_ZEROES_FACTORIAL_NUMBER_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "COUNT_TRAILING_ZEROES_FACTORIAL_NUMBER").ToLocalChecked());
    
    // Register the method
    tpl->PrototypeTemplate()->Set(isolate, "C_T_Z_F_N_f_gold", v8::FunctionTemplate::New(isolate, COUNT_TRAILING_ZEROES_FACTORIAL_NUMBER_C_T_Z_F_N_f_gold));
    
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "COUNT_TRAILING_ZEROES_FACTORIAL_NUMBER").ToLocalChecked(), constructor);
