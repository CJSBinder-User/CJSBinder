...
//
//[code snippet 1]
void MAXIMUM_TRIPLET_SUM_ARRAY_1_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    MAXIMUM_TRIPLET_SUM_ARRAY_1* obj = new MAXIMUM_TRIPLET_SUM_ARRAY_1();
...
//
//[code snippet 2]
void MAXIMUM_TRIPLET_SUM_ARRAY_1_M_T_S_A_1_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        arr[i] = value->IsInt32() ? value->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0) : 0;  // Fallback to 0 for non-int values
    }
    
    MAXIMUM_TRIPLET_SUM_ARRAY_1* obj = static_cast<MAXIMUM_TRIPLET_SUM_ARRAY_1*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int result = obj->M_T_S_A_1_f_gold(arr, n);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, MAXIMUM_TRIPLET_SUM_ARRAY_1_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "MAXIMUM_TRIPLET_SUM_ARRAY_1").ToLocalChecked());

    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, MAXIMUM_TRIPLET_SUM_ARRAY_1_M_T_S_A_1_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "M_T_S_A_1_f_gold", method_tpl);
    
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "MAXIMUM_TRIPLET_SUM_ARRAY_1").ToLocalChecked(), constructor);
