...
//
//[code snippet 1]
void EVEN_FIBONACCI_NUMBERS_SUM_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    EVEN_FIBONACCI_NUMBERS_SUM* obj = new EVEN_FIBONACCI_NUMBERS_SUM();
...
//
//[code snippet 2]
void EVEN_FIBONACCI_NUMBERS_SUM_E_F_N_S_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    EVEN_FIBONACCI_NUMBERS_SUM* obj = static_cast<EVEN_FIBONACCI_NUMBERS_SUM*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int limit = args[0]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    int result = obj->E_F_N_S_f_gold(limit);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, EVEN_FIBONACCI_NUMBERS_SUM_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "EVEN_FIBONACCI_NUMBERS_SUM").ToLocalChecked());
    
    // Registering the method
    tpl->PrototypeTemplate()->Set(isolate, "E_F_N_S_f_gold", v8::FunctionTemplate::New(isolate, EVEN_FIBONACCI_NUMBERS_SUM_E_F_N_S_f_gold));
    
    // Create the constructor function
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    
    // Attach the constructor to the global context
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "EVEN_FIBONACCI_NUMBERS_SUM").ToLocalChecked(), constructor);
