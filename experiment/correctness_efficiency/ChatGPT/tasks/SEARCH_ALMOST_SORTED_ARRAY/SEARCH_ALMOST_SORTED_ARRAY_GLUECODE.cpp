...
//
//[code snippet 1]
void SEARCH_ALMOST_SORTED_ARRAY_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    SEARCH_ALMOST_SORTED_ARRAY* obj = new SEARCH_ALMOST_SORTED_ARRAY();
...
//
//[code snippet 2]
void SEARCH_ALMOST_SORTED_ARRAY_S_A_S_A_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 3 || 
        !args[0]->IsArray() || 
        !args[1]->IsInt32() || 
        !args[2]->IsInt32()) {
    ...
    for (int i = 0; i < size; ++i) {
        arr[i] = inputArray->Get(args.GetIsolate()->GetCurrentContext(), i).ToLocalChecked()->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    }

    SEARCH_ALMOST_SORTED_ARRAY* obj = static_cast<SEARCH_ALMOST_SORTED_ARRAY*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int result = obj->S_A_S_A_f_gold(arr, l, r);
...
//assumed that 'isolate' and 'context' have been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, SEARCH_ALMOST_SORTED_ARRAY_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "SEARCH_ALMOST_SORTED_ARRAY").ToLocalChecked());
    
    // Defining the method
    tpl->PrototypeTemplate()->Set(isolate, "S_A_S_A_f_gold", v8::FunctionTemplate::New(isolate, SEARCH_ALMOST_SORTED_ARRAY_S_A_S_A_f_gold));
    
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "SEARCH_ALMOST_SORTED_ARRAY").ToLocalChecked(), constructor);
