...
//
//[code snippet 1]
void POLICEMEN_CATCH_THIEVES_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    POLICEMEN_CATCH_THIEVES* obj = new POLICEMEN_CATCH_THIEVES();
...
//
//[code snippet 2]
void POLICEMEN_CATCH_THIEVES_P_C_T_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 3 || !args[1]->IsInt32() || !args[2]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }

    POLICEMEN_CATCH_THIEVES* obj = static_cast<POLICEMEN_CATCH_THIEVES*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());

    // Convert char array (C-style string)
    ...
    int result = obj->P_C_T_f_gold(const_cast<char*>(arr), n, k);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, POLICEMEN_CATCH_THIEVES_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "POLICEMEN_CATCH_THIEVES").ToLocalChecked());
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, POLICEMEN_CATCH_THIEVES_P_C_T_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "P_C_T_f_gold", method_tpl);
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "POLICEMEN_CATCH_THIEVES").ToLocalChecked(), constructor);
