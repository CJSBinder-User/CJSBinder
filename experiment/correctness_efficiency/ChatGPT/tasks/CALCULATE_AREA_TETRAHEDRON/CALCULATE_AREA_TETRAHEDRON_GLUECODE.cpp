...
//
//[code snippet 1]
void CALCULATE_AREA_TETRAHEDRON_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    CALCULATE_AREA_TETRAHEDRON* obj = new CALCULATE_AREA_TETRAHEDRON();
...
//
//[code snippet 2]
void CALCULATE_AREA_TETRAHEDRON_C_A_T_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsNumber()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    CALCULATE_AREA_TETRAHEDRON* obj = static_cast<CALCULATE_AREA_TETRAHEDRON*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    double side = args[0]->NumberValue(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    double result = obj->C_A_T_f_gold(side);
...
// assumed that 'isolate' and 'context' have been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, CALCULATE_AREA_TETRAHEDRON_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "CALCULATE_AREA_TETRAHEDRON").ToLocalChecked());
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, CALCULATE_AREA_TETRAHEDRON_C_A_T_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "C_A_T_f_gold", method_tpl);
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "CALCULATE_AREA_TETRAHEDRON").ToLocalChecked(), constructor);
