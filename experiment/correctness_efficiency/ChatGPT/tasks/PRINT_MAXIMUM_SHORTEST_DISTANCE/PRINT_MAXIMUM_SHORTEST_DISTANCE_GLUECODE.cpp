...
//
//[code snippet 1]
void PRINT_MAXIMUM_SHORTEST_DISTANCE_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    PRINT_MAXIMUM_SHORTEST_DISTANCE* obj = new PRINT_MAXIMUM_SHORTEST_DISTANCE();
...
//
//[code snippet 2]
void PRINT_MAXIMUM_SHORTEST_DISTANCE_P_M_S_D_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (maybeValue.IsEmpty() || !maybeValue.ToLocal(&a[i]) || !a[i]->IsInt32()) {
            args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
            return;
        }
        a[i] = maybeValue.ToLocalChecked()->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    }

    PRINT_MAXIMUM_SHORTEST_DISTANCE* obj = static_cast<PRINT_MAXIMUM_SHORTEST_DISTANCE*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int result = obj->P_M_S_D_f_gold(a.data(), n, k);
...
//assumed that 'isolate' and 'context' have been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, PRINT_MAXIMUM_SHORTEST_DISTANCE_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "PRINT_MAXIMUM_SHORTEST_DISTANCE").ToLocalChecked());
    
    tpl->PrototypeTemplate()->Set(isolate, "P_M_S_D_f_gold", v8::FunctionTemplate::New(isolate, PRINT_MAXIMUM_SHORTEST_DISTANCE_P_M_S_D_f_gold));

    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "PRINT_MAXIMUM_SHORTEST_DISTANCE").ToLocalChecked(), constructor);
