...
//
//[code snippet 1]
void FIND_ROTATION_COUNT_ROTATED_SORTED_ARRAY_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    FIND_ROTATION_COUNT_ROTATED_SORTED_ARRAY* obj = new FIND_ROTATION_COUNT_ROTATED_SORTED_ARRAY();
...
//
//[code snippet 2]
void FIND_ROTATION_COUNT_ROTATED_SORTED_ARRAY_F_R_C_R_S_A_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        cpp_arr[i] = element->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    }

    FIND_ROTATION_COUNT_ROTATED_SORTED_ARRAY* obj = static_cast<FIND_ROTATION_COUNT_ROTATED_SORTED_ARRAY*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int result = obj->F_R_C_R_S_A_f_gold(cpp_arr, n);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, FIND_ROTATION_COUNT_ROTATED_SORTED_ARRAY_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "FIND_ROTATION_COUNT_ROTATED_SORTED_ARRAY").ToLocalChecked());
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, FIND_ROTATION_COUNT_ROTATED_SORTED_ARRAY_F_R_C_R_S_A_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "F_R_C_R_S_A_f_gold", method_tpl);
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "FIND_ROTATION_COUNT_ROTATED_SORTED_ARRAY").ToLocalChecked(), constructor);
