...
//
//[code snippet 1]
void MAXIMUM_SUM_ALTERNATING_SUBSEQUENCE_SUM_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    MAXIMUM_SUM_ALTERNATING_SUBSEQUENCE_SUM* obj = new MAXIMUM_SUM_ALTERNATING_SUBSEQUENCE_SUM();
...
//
//[code snippet 2]
void MAXIMUM_SUM_ALTERNATING_SUBSEQUENCE_SUM_M_S_A_S_S_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (element->IsInt32()) {
            ...
            delete[] arr; // Cleanup if non-integer value found
            args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
            return;
        }
    }

    MAXIMUM_SUM_ALTERNATING_SUBSEQUENCE_SUM* obj = static_cast<MAXIMUM_SUM_ALTERNATING_SUBSEQUENCE_SUM*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int result = obj->M_S_A_S_S_f_gold(arr, n);
    delete[] arr; // Cleanup
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, MAXIMUM_SUM_ALTERNATING_SUBSEQUENCE_SUM_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "MAXIMUM_SUM_ALTERNATING_SUBSEQUENCE_SUM").ToLocalChecked());

    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, MAXIMUM_SUM_ALTERNATING_SUBSEQUENCE_SUM_M_S_A_S_S_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "M_S_A_S_S_f_gold", method_tpl);

    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "MAXIMUM_SUM_ALTERNATING_SUBSEQUENCE_SUM").ToLocalChecked(), constructor);
