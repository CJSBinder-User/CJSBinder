...
//
//[code snippet 1]
void MAXIMUM_NUMBER_SEGMENTS_LENGTHS_B_C_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    MAXIMUM_NUMBER_SEGMENTS_LENGTHS_B_C* obj = new MAXIMUM_NUMBER_SEGMENTS_LENGTHS_B_C();
...
//
//[code snippet 2]
void MAXIMUM_NUMBER_SEGMENTS_LENGTHS_B_C_M_N_S_L_B_C_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 4 || 
        !args[0]->IsInt32() || 
        !args[1]->IsInt32() || 
        !args[2]->IsInt32() || 
    ...
    MAXIMUM_NUMBER_SEGMENTS_LENGTHS_B_C* obj = static_cast<MAXIMUM_NUMBER_SEGMENTS_LENGTHS_B_C*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int n = args[0]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    int a = args[1]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    int b = args[2]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    int c = args[3]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    int result = obj->M_N_S_L_B_C_f_gold(n, a, b, c);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, MAXIMUM_NUMBER_SEGMENTS_LENGTHS_B_C_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "MAXIMUM_NUMBER_SEGMENTS_LENGTHS_B_C").ToLocalChecked());
    
    // Registering the method
    tpl->PrototypeTemplate()->Set(isolate, "M_N_S_L_B_C_f_gold", v8::FunctionTemplate::New(isolate, MAXIMUM_NUMBER_SEGMENTS_LENGTHS_B_C_M_N_S_L_B_C_f_gold));
    
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "MAXIMUM_NUMBER_SEGMENTS_LENGTHS_B_C").ToLocalChecked(), constructor);
