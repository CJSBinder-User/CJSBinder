...
//
//[code snippet 1]
void COUNT_NUMBERS_CAN_CONSTRUCTED_USING_TWO_NUMBERS_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    COUNT_NUMBERS_CAN_CONSTRUCTED_USING_TWO_NUMBERS* obj = new COUNT_NUMBERS_CAN_CONSTRUCTED_USING_TWO_NUMBERS();
...
//
//[code snippet 2]
void COUNT_NUMBERS_CAN_CONSTRUCTED_USING_TWO_NUMBERS_C_N_C_C_U_T_N_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 3 || !args[0]->IsInt32() || !args[1]->IsInt32() || !args[2]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    COUNT_NUMBERS_CAN_CONSTRUCTED_USING_TWO_NUMBERS* obj = static_cast<COUNT_NUMBERS_CAN_CONSTRUCTED_USING_TWO_NUMBERS*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    ...
    int result = obj->C_N_C_C_U_T_N_f_gold(n, x, y);  // Call the C++ method
    args.GetReturnValue().Set(v8::Integer::New(args.GetIsolate(), result));  // Return the result to JavaScript
}
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, COUNT_NUMBERS_CAN_CONSTRUCTED_USING_TWO_NUMBERS_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "COUNT_NUMBERS_CAN_CONSTRUCTED_USING_TWO_NUMBERS").ToLocalChecked());
    
    // Exposing the method
    tpl->PrototypeTemplate()->Set(isolate, "C_N_C_C_U_T_N_f_gold", v8::FunctionTemplate::New(isolate, COUNT_NUMBERS_CAN_CONSTRUCTED_USING_TWO_NUMBERS_C_N_C_C_U_T_N_f_gold));
    
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "COUNT_NUMBERS_CAN_CONSTRUCTED_USING_TWO_NUMBERS").ToLocalChecked(), constructor);
