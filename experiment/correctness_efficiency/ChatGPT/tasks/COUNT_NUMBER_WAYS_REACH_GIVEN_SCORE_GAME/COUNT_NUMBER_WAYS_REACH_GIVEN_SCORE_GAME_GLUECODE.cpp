...
//
//[code snippet 1]
void COUNT_NUMBER_WAYS_REACH_GIVEN_SCORE_GAME_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    COUNT_NUMBER_WAYS_REACH_GIVEN_SCORE_GAME* obj = new COUNT_NUMBER_WAYS_REACH_GIVEN_SCORE_GAME();
...
//
//[code snippet 2]
void COUNT_NUMBER_WAYS_REACH_GIVEN_SCORE_GAME_C_N_W_R_G_S_G_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    COUNT_NUMBER_WAYS_REACH_GIVEN_SCORE_GAME* obj = static_cast<COUNT_NUMBER_WAYS_REACH_GIVEN_SCORE_GAME*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int n = args[0]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    int result = obj->C_N_W_R_G_S_G_f_gold(n);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, COUNT_NUMBER_WAYS_REACH_GIVEN_SCORE_GAME_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "COUNT_NUMBER_WAYS_REACH_GIVEN_SCORE_GAME").ToLocalChecked());
    tpl->PrototypeTemplate()->Set(isolate, "C_N_W_R_G_S_G_f_gold", v8::FunctionTemplate::New(isolate, COUNT_NUMBER_WAYS_REACH_GIVEN_SCORE_GAME_C_N_W_R_G_S_G_f_gold));
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "COUNT_NUMBER_WAYS_REACH_GIVEN_SCORE_GAME").ToLocalChecked(), constructor);
