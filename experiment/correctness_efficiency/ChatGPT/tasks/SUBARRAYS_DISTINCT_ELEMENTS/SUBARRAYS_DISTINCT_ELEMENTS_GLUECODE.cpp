...
//
//[code snippet 1]
void SUBARRAYS_DISTINCT_ELEMENTS_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    SUBARRAYS_DISTINCT_ELEMENTS* obj = new SUBARRAYS_DISTINCT_ELEMENTS();
...
//
//[code snippet 2]
void SUBARRAYS_DISTINCT_ELEMENTS_S_D_E_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        nativeArr[i] = value->IsInt32() ? value->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0) : 0;
    }

    SUBARRAYS_DISTINCT_ELEMENTS* obj = static_cast<SUBARRAYS_DISTINCT_ELEMENTS*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int result = obj->S_D_E_f_gold(nativeArr, n);
    
    delete[] nativeArr; // Clean up the dynamically allocated array
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, SUBARRAYS_DISTINCT_ELEMENTS_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "SUBARRAYS_DISTINCT_ELEMENTS").ToLocalChecked());
    tpl->PrototypeTemplate()->Set(isolate, "S_D_E_f_gold", v8::FunctionTemplate::New(isolate, SUBARRAYS_DISTINCT_ELEMENTS_S_D_E_f_gold));
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "SUBARRAYS_DISTINCT_ELEMENTS").ToLocalChecked(), constructor);
