...
//
//[code snippet 1]
void PROGRAM_OCTAL_DECIMAL_CONVERSION_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    PROGRAM_OCTAL_DECIMAL_CONVERSION* obj = new PROGRAM_OCTAL_DECIMAL_CONVERSION();
...
//
//[code snippet 2]
void PROGRAM_OCTAL_DECIMAL_CONVERSION_P_O_D_C_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    PROGRAM_OCTAL_DECIMAL_CONVERSION* obj = static_cast<PROGRAM_OCTAL_DECIMAL_CONVERSION*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int n = args[0]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    int result = obj->P_O_D_C_f_gold(n);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, PROGRAM_OCTAL_DECIMAL_CONVERSION_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "PROGRAM_OCTAL_DECIMAL_CONVERSION").ToLocalChecked());
    tpl->PrototypeTemplate()->Set(isolate, "P_O_D_C_f_gold", v8::FunctionTemplate::New(isolate, PROGRAM_OCTAL_DECIMAL_CONVERSION_P_O_D_C_f_gold));
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "PROGRAM_OCTAL_DECIMAL_CONVERSION").ToLocalChecked(), constructor);
