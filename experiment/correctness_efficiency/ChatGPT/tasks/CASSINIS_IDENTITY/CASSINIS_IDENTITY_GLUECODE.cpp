...
//
//[code snippet 1]
void CASSINIS_IDENTITY_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    CASSINIS_IDENTITY* obj = new CASSINIS_IDENTITY();
...
//
//[code snippet 2]
void CASSINIS_IDENTITY_C_I_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    CASSINIS_IDENTITY* obj = static_cast<CASSINIS_IDENTITY*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int n = args[0]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    int result = obj->C_I_f_gold(n);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, CASSINIS_IDENTITY_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "CASSINIS_IDENTITY").ToLocalChecked());
    
    tpl->PrototypeTemplate()->Set(isolate, "C_I_f_gold", v8::FunctionTemplate::New(isolate, CASSINIS_IDENTITY_C_I_f_gold));

    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "CASSINIS_IDENTITY").ToLocalChecked(), constructor);
