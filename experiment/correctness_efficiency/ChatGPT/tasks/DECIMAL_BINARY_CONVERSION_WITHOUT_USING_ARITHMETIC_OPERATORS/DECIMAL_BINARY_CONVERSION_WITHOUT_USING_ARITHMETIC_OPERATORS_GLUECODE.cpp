...
//
//[code snippet 1]
void DECIMAL_BINARY_CONVERSION_WITHOUT_USING_ARITHMETIC_OPERATORS_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    DECIMAL_BINARY_CONVERSION_WITHOUT_USING_ARITHMETIC_OPERATORS* obj = new DECIMAL_BINARY_CONVERSION_WITHOUT_USING_ARITHMETIC_OPERATORS();
...
//
//[code snippet 2]
void DECIMAL_BINARY_CONVERSION_WITHOUT_USING_ARITHMETIC_OPERATORS_D_B_C_W_U_A_O_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    DECIMAL_BINARY_CONVERSION_WITHOUT_USING_ARITHMETIC_OPERATORS* obj = static_cast<DECIMAL_BINARY_CONVERSION_WITHOUT_USING_ARITHMETIC_OPERATORS*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int n = args[0]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    string result = obj->D_B_C_W_U_A_O_f_gold(n);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, DECIMAL_BINARY_CONVERSION_WITHOUT_USING_ARITHMETIC_OPERATORS_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "DECIMAL_BINARY_CONVERSION_WITHOUT_USING_ARITHMETIC_OPERATORS").ToLocalChecked());
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, DECIMAL_BINARY_CONVERSION_WITHOUT_USING_ARITHMETIC_OPERATORS_D_B_C_W_U_A_O_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "D_B_C_W_U_A_O_f_gold", method_tpl);
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "DECIMAL_BINARY_CONVERSION_WITHOUT_USING_ARITHMETIC_OPERATORS").ToLocalChecked(), constructor);
