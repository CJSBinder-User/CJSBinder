...
//
// [code snippet 1]
void COUNT_BINARY_DIGIT_NUMBERS_SMALLER_N_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    COUNT_BINARY_DIGIT_NUMBERS_SMALLER_N* obj = new COUNT_BINARY_DIGIT_NUMBERS_SMALLER_N();
...
//
// [code snippet 2]
void COUNT_BINARY_DIGIT_NUMBERS_SMALLER_N_C_B_D_N_S_N_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    COUNT_BINARY_DIGIT_NUMBERS_SMALLER_N* obj = static_cast<COUNT_BINARY_DIGIT_NUMBERS_SMALLER_N*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int N = args[0]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    int result = obj->C_B_D_N_S_N_f_gold(N);
...
// assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, COUNT_BINARY_DIGIT_NUMBERS_SMALLER_N_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "COUNT_BINARY_DIGIT_NUMBERS_SMALLER_N").ToLocalChecked());
    
    // Register the method
    tpl->PrototypeTemplate()->Set(isolate, "C_B_D_N_S_N_f_gold", v8::FunctionTemplate::New(isolate, COUNT_BINARY_DIGIT_NUMBERS_SMALLER_N_C_B_D_N_S_N_f_gold));
    
    // Create the constructor function
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    
    // Register the class in the global context
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "COUNT_BINARY_DIGIT_NUMBERS_SMALLER_N").ToLocalChecked(), constructor);
