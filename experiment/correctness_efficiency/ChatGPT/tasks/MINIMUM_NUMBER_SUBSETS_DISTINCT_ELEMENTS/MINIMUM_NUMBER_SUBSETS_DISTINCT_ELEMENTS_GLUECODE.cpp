...
//
//[code snippet 1]
void MINIMUM_NUMBER_SUBSETS_DISTINCT_ELEMENTS_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    MINIMUM_NUMBER_SUBSETS_DISTINCT_ELEMENTS* obj = new MINIMUM_NUMBER_SUBSETS_DISTINCT_ELEMENTS();
...
//
//[code snippet 2]
void MINIMUM_NUMBER_SUBSETS_DISTINCT_ELEMENTS_M_N_S_D_E_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    ...
    // Assuming ar is a pointer to an array of integers
    int* ar = new int[n];
    
    for (int i = 0; i < n; i++) {
        v8::Local<v8::Value> value = array->Get(args.GetIsolate()->GetCurrentContext(), i).ToLocalChecked();
        ar[i] = value->IsInt32() ? value->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0) : 0;
    }

    MINIMUM_NUMBER_SUBSETS_DISTINCT_ELEMENTS* obj = static_cast<MINIMUM_NUMBER_SUBSETS_DISTINCT_ELEMENTS*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int result = obj->M_N_S_D_E_f_gold(ar, n);
...
//assumed that 'isolate' and 'context' have been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, MINIMUM_NUMBER_SUBSETS_DISTINCT_ELEMENTS_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "MINIMUM_NUMBER_SUBSETS_DISTINCT_ELEMENTS").ToLocalChecked());
    
    // Registering the method
    tpl->PrototypeTemplate()->Set(isolate, "M_N_S_D_E_f_gold", v8::FunctionTemplate::New(isolate, MINIMUM_NUMBER_SUBSETS_DISTINCT_ELEMENTS_M_N_S_D_E_f_gold));
    
    // Create the constructor function
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "MINIMUM_NUMBER_SUBSETS_DISTINCT_ELEMENTS").ToLocalChecked(), constructor);
