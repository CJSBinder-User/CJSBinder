...
//
//[code snippet 1]
void COUNT_POSSIBLE_WAYS_TO_CONSTRUCT_BUILDINGS_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    COUNT_POSSIBLE_WAYS_TO_CONSTRUCT_BUILDINGS* obj = new COUNT_POSSIBLE_WAYS_TO_CONSTRUCT_BUILDINGS();
...
//
//[code snippet 2]
void COUNT_POSSIBLE_WAYS_TO_CONSTRUCT_BUILDINGS_C_P_W_T_C_B_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    COUNT_POSSIBLE_WAYS_TO_CONSTRUCT_BUILDINGS* obj = static_cast<COUNT_POSSIBLE_WAYS_TO_CONSTRUCT_BUILDINGS*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int N = args[0]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    int result = obj->C_P_W_T_C_B_f_gold(N);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, COUNT_POSSIBLE_WAYS_TO_CONSTRUCT_BUILDINGS_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "COUNT_POSSIBLE_WAYS_TO_CONSTRUCT_BUILDINGS").ToLocalChecked());
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, COUNT_POSSIBLE_WAYS_TO_CONSTRUCT_BUILDINGS_C_P_W_T_C_B_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "C_P_W_T_C_B_f_gold", method_tpl);
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "COUNT_POSSIBLE_WAYS_TO_CONSTRUCT_BUILDINGS").ToLocalChecked(), constructor);
