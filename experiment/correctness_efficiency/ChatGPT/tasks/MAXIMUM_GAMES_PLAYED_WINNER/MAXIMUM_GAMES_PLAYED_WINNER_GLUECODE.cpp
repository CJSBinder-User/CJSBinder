...
//
//[code snippet 1]
void MAXIMUM_GAMES_PLAYED_WINNER_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    MAXIMUM_GAMES_PLAYED_WINNER* obj = new MAXIMUM_GAMES_PLAYED_WINNER();
...
//
//[code snippet 2]
void MAXIMUM_GAMES_PLAYED_WINNER_M_G_P_W_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    MAXIMUM_GAMES_PLAYED_WINNER* obj = static_cast<MAXIMUM_GAMES_PLAYED_WINNER*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int N = args[0]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    int result = obj->M_G_P_W_f_gold(N);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, MAXIMUM_GAMES_PLAYED_WINNER_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "MAXIMUM_GAMES_PLAYED_WINNER").ToLocalChecked());
    tpl->PrototypeTemplate()->Set(isolate, "M_G_P_W_f_gold", v8::FunctionTemplate::New(isolate, MAXIMUM_GAMES_PLAYED_WINNER_M_G_P_W_f_gold));
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "MAXIMUM_GAMES_PLAYED_WINNER").ToLocalChecked(), constructor);
