...
//
//[code snippet 1]
void OVERLAPPING_SUM_TWO_ARRAY_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    OVERLAPPING_SUM_TWO_ARRAY* obj = new OVERLAPPING_SUM_TWO_ARRAY();
...
//
//[code snippet 2]
void OVERLAPPING_SUM_TWO_ARRAY_O_S_T_A_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 3 || !args[0]->IsArray() || !args[1]->IsArray() || !args[2]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    
    // Extracting the arrays from JavaScript
    ...
    for (uint32_t i = 0; i < A->Length(); i++) {
        a_vec.push_back(A->Get(args.GetIsolate()->GetCurrentContext(), i).ToLocalChecked()->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0));
    }

    for (uint32_t i = 0; i < B->Length(); i++) {
        b_vec.push_back(B->Get(args.GetIsolate()->GetCurrentContext(), i).ToLocalChecked()->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0));
    }

    // Create C++ array pointers
    int* A_ptr = a_vec.data();
    int* B_ptr = b_vec.data();

    // Call the C++ method
    OVERLAPPING_SUM_TWO_ARRAY* obj = static_cast<OVERLAPPING_SUM_TWO_ARRAY*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int result = obj->O_S_T_A_f_gold(A_ptr, B_ptr, n);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, OVERLAPPING_SUM_TWO_ARRAY_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "OVERLAPPING_SUM_TWO_ARRAY").ToLocalChecked());
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, OVERLAPPING_SUM_TWO_ARRAY_O_S_T_A_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "O_S_T_A_f_gold", method_tpl);
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "OVERLAPPING_SUM_TWO_ARRAY").ToLocalChecked(), constructor);
