...
//
//[code snippet 1]
void AREA_OF_A_HEXAGON_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    AREA_OF_A_HEXAGON* obj = new AREA_OF_A_HEXAGON();
...
//
//[code snippet 2]
void AREA_OF_A_HEXAGON_A_O_A_H_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsNumber()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    AREA_OF_A_HEXAGON* obj = static_cast<AREA_OF_A_HEXAGON*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    double s = args[0]->NumberValue(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    double result = obj->A_O_A_H_f_gold(s);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, AREA_OF_A_HEXAGON_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "AREA_OF_A_HEXAGON").ToLocalChecked());
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, AREA_OF_A_HEXAGON_A_O_A_H_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "A_O_A_H_f_gold", method_tpl);
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "AREA_OF_A_HEXAGON").ToLocalChecked(), constructor);
