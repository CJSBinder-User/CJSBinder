...
//
//[code snippet 1]
void TAIL_RECURSION_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
...
//
//[code snippet 2]
void TAIL_RECURSION_T_R_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsUint32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    TAIL_RECURSION* obj = static_cast<TAIL_RECURSION*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    unsigned int n = args[0]->Uint32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    unsigned int result = obj->T_R_f_gold(n);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, TAIL_RECURSION_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "TAIL_RECURSION").ToLocalChecked());
  
    // Set up method wrapper
    tpl->PrototypeTemplate()->Set(isolate, "T_R_f_gold", v8::FunctionTemplate::New(isolate, TAIL_RECURSION_T_R_f_gold));

    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "TAIL_RECURSION").ToLocalChecked(), constructor);
