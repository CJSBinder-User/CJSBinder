...
//
//[code snippet 1]
void MINIMUM_INSERTIONS_SORT_ARRAY_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    MINIMUM_INSERTIONS_SORT_ARRAY* obj = new MINIMUM_INSERTIONS_SORT_ARRAY();
...
//
//[code snippet 2]
void MINIMUM_INSERTIONS_SORT_ARRAY_M_I_S_A_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 2 || !args[0]->IsArray() || !args[1]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    
    MINIMUM_INSERTIONS_SORT_ARRAY* obj = static_cast<MINIMUM_INSERTIONS_SORT_ARRAY*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    
    // Extracting array from JS arguments
    ...
    for (int i = 0; i < N; ++i) {
        ...
        arr[i] = value.ToLocalChecked()->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    }

    int result = obj->M_I_S_A_f_gold(arr, N);
    
    // Clean up the C++ array
    delete[] arr;
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, MINIMUM_INSERTIONS_SORT_ARRAY_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "MINIMUM_INSERTIONS_SORT_ARRAY").ToLocalChecked());
    tpl->PrototypeTemplate()->Set(isolate, "M_I_S_A_f_gold", v8::FunctionTemplate::New(isolate, MINIMUM_INSERTIONS_SORT_ARRAY_M_I_S_A_f_gold));
    
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "MINIMUM_INSERTIONS_SORT_ARRAY").ToLocalChecked(), constructor);
