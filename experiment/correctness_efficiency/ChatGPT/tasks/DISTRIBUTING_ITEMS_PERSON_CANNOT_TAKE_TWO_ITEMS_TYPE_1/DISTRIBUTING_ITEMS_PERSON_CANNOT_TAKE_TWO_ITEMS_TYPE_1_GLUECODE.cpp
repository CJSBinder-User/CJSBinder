...
//
//[code snippet 1]
void DISTRIBUTING_ITEMS_PERSON_CANNOT_TAKE_TWO_ITEMS_TYPE_1_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    DISTRIBUTING_ITEMS_PERSON_CANNOT_TAKE_TWO_ITEMS_TYPE_1* obj = new DISTRIBUTING_ITEMS_PERSON_CANNOT_TAKE_TWO_ITEMS_TYPE_1();
...
//
//[code snippet 2]
void DISTRIBUTING_ITEMS_PERSON_CANNOT_TAKE_TWO_ITEMS_TYPE_1_D_I_P_C_T_T_I_T_1_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    ...
    int n = args[1]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromJust();
    int k = args[2]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromJust();
    
    // Convert JS array to C++ array
    int* arr = new int[n];
    for (int i = 0; i < n; i++) {
        v8::Local<v8::Value> element = jsArray->Get(args.GetIsolate()->GetCurrentContext(), i).ToLocalChecked();
        arr[i] = element->Int32Value(args.GetIsolate()->GetCurrentContext()).FromJust();
    }

    DISTRIBUTING_ITEMS_PERSON_CANNOT_TAKE_TWO_ITEMS_TYPE_1* obj = 
        static_cast<DISTRIBUTING_ITEMS_PERSON_CANNOT_TAKE_TWO_ITEMS_TYPE_1*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    
    bool result = obj->D_I_P_C_T_T_I_T_1_f_gold(arr, n, k);
...
// assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, DISTRIBUTING_ITEMS_PERSON_CANNOT_TAKE_TWO_ITEMS_TYPE_1_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "DISTRIBUTING_ITEMS_PERSON_CANNOT_TAKE_TWO_ITEMS_TYPE_1").ToLocalChecked());
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, DISTRIBUTING_ITEMS_PERSON_CANNOT_TAKE_TWO_ITEMS_TYPE_1_D_I_P_C_T_T_I_T_1_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "D_I_P_C_T_T_I_T_1_f_gold", method_tpl);
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "DISTRIBUTING_ITEMS_PERSON_CANNOT_TAKE_TWO_ITEMS_TYPE_1").ToLocalChecked(), constructor);
