...
//
//[code snippet 1]
void COUNT_DIGITS_FACTORIAL_SET_2_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    COUNT_DIGITS_FACTORIAL_SET_2* obj = new COUNT_DIGITS_FACTORIAL_SET_2();
...
//
//[code snippet 2]
void COUNT_DIGITS_FACTORIAL_SET_2_C_D_F_S_2_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    COUNT_DIGITS_FACTORIAL_SET_2* obj = static_cast<COUNT_DIGITS_FACTORIAL_SET_2*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int n = args[0]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    long long result = obj->C_D_F_S_2_f_gold(n);
...
// assumed that 'isolate' and 'context' have been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, COUNT_DIGITS_FACTORIAL_SET_2_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "COUNT_DIGITS_FACTORIAL_SET_2").ToLocalChecked());
    tpl->PrototypeTemplate()->Set(isolate, "C_D_F_S_2_f_gold", v8::FunctionTemplate::New(isolate, COUNT_DIGITS_FACTORIAL_SET_2_C_D_F_S_2_f_gold));
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "COUNT_DIGITS_FACTORIAL_SET_2").ToLocalChecked(), constructor);
