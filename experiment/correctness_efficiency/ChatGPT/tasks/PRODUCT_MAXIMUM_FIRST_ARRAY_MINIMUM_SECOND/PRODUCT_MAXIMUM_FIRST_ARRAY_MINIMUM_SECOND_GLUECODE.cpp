...
//
//[code snippet 1]
void PRODUCT_MAXIMUM_FIRST_ARRAY_MINIMUM_SECOND_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    PRODUCT_MAXIMUM_FIRST_ARRAY_MINIMUM_SECOND* obj = new PRODUCT_MAXIMUM_FIRST_ARRAY_MINIMUM_SECOND();
...
//
//[code snippet 2]
void PRODUCT_MAXIMUM_FIRST_ARRAY_MINIMUM_SECOND_P_M_F_A_M_S_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 4 || 
        !args[0]->IsArray() || 
        !args[1]->IsArray() || 
        !args[2]->IsInt32() || 
    ...
    PRODUCT_MAXIMUM_FIRST_ARRAY_MINIMUM_SECOND* obj = static_cast<PRODUCT_MAXIMUM_FIRST_ARRAY_MINIMUM_SECOND*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    
    // Convert V8 arrays to C++ arrays
    int* c_arr1 = new int[n1];
    int* c_arr2 = new int[n2];
    ...
    int result = obj->P_M_F_A_M_S_f_gold(c_arr1, c_arr2, n1, n2);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, PRODUCT_MAXIMUM_FIRST_ARRAY_MINIMUM_SECOND_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "PRODUCT_MAXIMUM_FIRST_ARRAY_MINIMUM_SECOND").ToLocalChecked());
    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, PRODUCT_MAXIMUM_FIRST_ARRAY_MINIMUM_SECOND_P_M_F_A_M_S_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "P_M_F_A_M_S_f_gold", method_tpl);
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "PRODUCT_MAXIMUM_FIRST_ARRAY_MINIMUM_SECOND").ToLocalChecked(), constructor);
