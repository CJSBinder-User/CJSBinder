...
//
//[code snippet 1]
void PROGRAM_FOR_FACTORIAL_OF_A_NUMBER_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    PROGRAM_FOR_FACTORIAL_OF_A_NUMBER* obj = new PROGRAM_FOR_FACTORIAL_OF_A_NUMBER();
...
//
//[code snippet 2]
void PROGRAM_FOR_FACTORIAL_OF_A_NUMBER_P_F_F_O_A_N_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsUint32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    PROGRAM_FOR_FACTORIAL_OF_A_NUMBER* obj = static_cast<PROGRAM_FOR_FACTORIAL_OF_A_NUMBER*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    unsigned int n = args[0]->Uint32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    unsigned int result = obj->P_F_F_O_A_N_f_gold(n);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, PROGRAM_FOR_FACTORIAL_OF_A_NUMBER_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "PROGRAM_FOR_FACTORIAL_OF_A_NUMBER").ToLocalChecked());
    
    tpl->PrototypeTemplate()->Set(isolate, "P_F_F_O_A_N_f_gold", v8::FunctionTemplate::New(isolate, PROGRAM_FOR_FACTORIAL_OF_A_NUMBER_P_F_F_O_A_N_f_gold));
    
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "PROGRAM_FOR_FACTORIAL_OF_A_NUMBER").ToLocalChecked(), constructor);
