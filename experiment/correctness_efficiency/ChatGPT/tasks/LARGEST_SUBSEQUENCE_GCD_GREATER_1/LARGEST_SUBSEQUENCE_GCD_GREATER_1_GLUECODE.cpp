...
//
//[code snippet 1]
void LARGEST_SUBSEQUENCE_GCD_GREATER_1_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    LARGEST_SUBSEQUENCE_GCD_GREATER_1* obj = new LARGEST_SUBSEQUENCE_GCD_GREATER_1();
...
//
//[code snippet 2]
void LARGEST_SUBSEQUENCE_GCD_GREATER_1_L_S_G_G_1_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 2 || !args[0]->IsArray() || !args[1]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }

    LARGEST_SUBSEQUENCE_GCD_GREATER_1* obj = static_cast<LARGEST_SUBSEQUENCE_GCD_GREATER_1*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    ...
    int result = obj->L_S_G_G_1_f_gold(arr, n);
...
//assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, LARGEST_SUBSEQUENCE_GCD_GREATER_1_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "LARGEST_SUBSEQUENCE_GCD_GREATER_1").ToLocalChecked());

    v8::Local<v8::FunctionTemplate> method_tpl = v8::FunctionTemplate::New(isolate, LARGEST_SUBSEQUENCE_GCD_GREATER_1_L_S_G_G_1_f_gold);
    tpl->PrototypeTemplate()->Set(isolate, "L_S_G_G_1_f_gold", method_tpl);

    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "LARGEST_SUBSEQUENCE_GCD_GREATER_1").ToLocalChecked(), constructor);
