...
//
//[code snippet 1]
void MAXIMUM_VALUE_CHOICE_EITHER_DIVIDING_CONSIDERING_constructor(const v8::FunctionCallbackInfo<v8::Value>& args) {
    MAXIMUM_VALUE_CHOICE_EITHER_DIVIDING_CONSIDERING* obj = new MAXIMUM_VALUE_CHOICE_EITHER_DIVIDING_CONSIDERING();
...
//
//[code snippet 2]
void MAXIMUM_VALUE_CHOICE_EITHER_DIVIDING_CONSIDERING_M_V_C_E_D_C_f_gold(const v8::FunctionCallbackInfo<v8::Value>& args) {
    if (args.Length() < 1 || !args[0]->IsInt32()) {
        args.GetReturnValue().Set(v8::Undefined(args.GetIsolate()));
        return;
    }
    MAXIMUM_VALUE_CHOICE_EITHER_DIVIDING_CONSIDERING* obj = static_cast<MAXIMUM_VALUE_CHOICE_EITHER_DIVIDING_CONSIDERING*>(v8::External::Cast(*args.This()->GetInternalField(0))->Value());
    int n = args[0]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(0);
    int result = obj->M_V_C_E_D_C_f_gold(n);
...
// assumed that 'isolate' and 'context' has been initialized
    v8::Local<v8::FunctionTemplate> tpl = v8::FunctionTemplate::New(isolate, MAXIMUM_VALUE_CHOICE_EITHER_DIVIDING_CONSIDERING_constructor);
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->SetClassName(v8::String::NewFromUtf8(isolate, "MAXIMUM_VALUE_CHOICE_EITHER_DIVIDING_CONSIDERING").ToLocalChecked());
    
    tpl->PrototypeTemplate()->Set(isolate, "M_V_C_E_D_C_f_gold", v8::FunctionTemplate::New(isolate, MAXIMUM_VALUE_CHOICE_EITHER_DIVIDING_CONSIDERING_M_V_C_E_D_C_f_gold));
    
    v8::Local<v8::Function> constructor = tpl->GetFunction(context).ToLocalChecked();
    context->Global()->Set(context, v8::String::NewFromUtf8(isolate, "MAXIMUM_VALUE_CHOICE_EITHER_DIVIDING_CONSIDERING").ToLocalChecked(), constructor);
